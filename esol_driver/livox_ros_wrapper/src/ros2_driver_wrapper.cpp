...
void RosDriverWrapper::CreatePubSub()
{
  ...
  // This interface will register the callback WaitDataPacketFromHwRx that will receive and publish
  // the raw data to the ROS system.
  ...
  // 4. WaitRosConfig. It registers the ROS services that will be called to forward the configure
  // requests to the sensor. ToDo: Next Phase

  // 5. RosLidarDataSubscriber (a nodelet in ROS1). It subscribes to a specified ROS topic to listen
  //    to the data packets generated by the HwRxInterface, or the data received from previously
  //    recorded log data.
...
void RosDriverWrapper::RosLidarDataSubscriber(const livox_msgs::msg::LidarScan::SharedPtr scan_msg)
{
  ...
  // 7. RosPublishPointCloud. It will publish the final point cloud generated by the
  // GeneratePointCloud module.
