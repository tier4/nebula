...
{
  uint16_t pl_vmon_vin_p;
  uint8_t fpga_pl_kernal_temperature;
  uint8_t fpga_ps_kernal_temperature;
  uint8_t window_temperature;
};

struct MemsDeviceInfo
{
  uint16_t battery_volt;
  uint8_t lidar_fault_status;
  uint8_t lidar_roi_status;
};
struct BpRubyDeviceInfo
{
  uint16_t apd_temperature;
  uint16_t main_bot_temperature;
};

struct HeliosDeviceInfo
{
...
struct AllDeviceInfo : BpRubyDeviceInfo, HeliosDeviceInfo, MemsDeviceInfo, FlashDeviceInfo
{
  void operator=(AllDeviceInfo & deviceInfo)
  {
    ...
    this->apd_temperature = deviceInfo.apd_temperature;
    this->main_bot_temperature = deviceInfo.main_bot_temperature;
    ...
    this->battery_volt = deviceInfo.battery_volt;
    this->lidar_fault_status = deviceInfo.lidar_fault_status;
    this->lidar_roi_status = deviceInfo.lidar_roi_status;
    this->pl_vmon_vin_p = deviceInfo.pl_vmon_vin_p;
    this->fpga_pl_kernal_temperature = deviceInfo.fpga_pl_kernal_temperature;
    this->fpga_ps_kernal_temperature = deviceInfo.fpga_ps_kernal_temperature;
    this->window_temperature = deviceInfo.window_temperature;
  }
  uint8_t sn[6];
  uint8_t mac[6];
  uint16_t rpm;
...
struct FlashDeviceCalculateParamsInfo
{
  float pl_vmon_vin_p_val;
};

struct MemsDeviceCalculateParamsInfo
{
  float battery_volt_val;
};

struct HeliosDeviceCalculateParamsInfo
{
...
struct BpRubyDeviceCalculateParamsInfo
{
  float apd_temperature_val;
  float main_bot_temperature_val;
};

struct AllDeviceCalculateParamsInfo : AllDeviceInfo
{
  float bot_fpga_temperature_val;
  float main_fpga_temperature_val;
  HeliosDeviceCalculateParamsInfo heliosDeviceCalculateParamsInfo;
  BpRubyDeviceCalculateParamsInfo bpRubyDeviceCalculateParamsInfo;
  MemsDeviceCalculateParamsInfo memsDeviceCalculateParamsInfo;
  FlashDeviceCalculateParamsInfo flashDeviceCalculateParamsInfo;
};
//
// define M_PI
//
