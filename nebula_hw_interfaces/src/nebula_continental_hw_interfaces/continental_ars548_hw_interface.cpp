...
void ContinentalARS548HwInterface::ProcessSensorStatusPacket(const std::vector<uint8_t> & buffer)
{
  ...
  radar_status_.timestamp_seconds = sensor_status_packet_.stamp.timestamp_seconds.value();

  if (sensor_status_packet_.stamp.timestamp_sync_status == 1) {
    radar_status_.timestamp_sync_status = "SYNC_OK";
  } else if (sensor_status_packet_.stamp.timestamp_sync_status == 2) {
    radar_status_.timestamp_sync_status = "NEVER_SYNC";
  } else if (sensor_status_packet_.stamp.timestamp_sync_status == 3) {
    radar_status_.timestamp_sync_status = "SYNC_LOST";
  } else {
    radar_status_.timestamp_sync_status = "INVALID_VALUE";
  }

  radar_status_.sw_version_major = sensor_status_packet_.sw_version_major;
  radar_status_.sw_version_minor = sensor_status_packet_.sw_version_minor;
  radar_status_.sw_version_patch = sensor_status_packet_.sw_version_patch;

  radar_status_.plug_orientation = sensor_status_packet_.status.plug_orientation == 0 ? "PLUG_RIGHT"
                                   : sensor_status_packet_.status.plug_orientation == 1
                                     ? "PLUG_LEFT"
                                     : "INVALID_VALUE";

  radar_status_.max_distance = sensor_status_packet_.status.maximum_distance.value();

  radar_status_.longitudinal = sensor_status_packet_.status.longitudinal.value();
  radar_status_.lateral = sensor_status_packet_.status.lateral.value();
  radar_status_.vertical = sensor_status_packet_.status.vertical.value();
  radar_status_.yaw = sensor_status_packet_.status.yaw.value();

  radar_status_.pitch = sensor_status_packet_.status.pitch.value();
  ;
  radar_status_.length = sensor_status_packet_.status.length.value();
  ;
  radar_status_.width = sensor_status_packet_.status.width.value();
  radar_status_.height = sensor_status_packet_.status.height.value();
  radar_status_.wheel_base = sensor_status_packet_.status.wheelbase.value();

  if (sensor_status_packet_.status.frequency_slot == 0) {
    radar_status_.frequency_slot = "0:Low (76.23 GHz)";
  } else if (sensor_status_packet_.status.frequency_slot == 1) {
    radar_status_.frequency_slot = "1:Mid (76.48 GHz)";
  } else if (sensor_status_packet_.status.frequency_slot == 2) {
    radar_status_.frequency_slot = "2:High (76.73 GHz)";
  } else {
    radar_status_.frequency_slot = "INVALID VALUE";
  }

  radar_status_.cycle_time = sensor_status_packet_.status.cycle_time;
  radar_status_.time_slot = sensor_status_packet_.status.time_slot;

  radar_status_.hcc = sensor_status_packet_.status.hcc == 1 ? "Worldwide"
                      : sensor_status_packet_.status.hcc == 2
                        ? "Japan"
                        : ("INVALID VALUE=" + std::to_string(sensor_status_packet_.status.hcc));

  radar_status_.power_save_standstill =
    sensor_status_packet_.status.powersave_standstill == 0   ? "Off"
    : sensor_status_packet_.status.powersave_standstill == 1 ? "On"
                                                             : "INVALID VALUE";

  std::stringstream ss0, ss1;
  ss0 << std::to_string(sensor_status_packet_.status.sensor_ip_address00) << "."
      << std::to_string(sensor_status_packet_.status.sensor_ip_address01) << "."
      << std::to_string(sensor_status_packet_.status.sensor_ip_address02) << "."
      << std::to_string(sensor_status_packet_.status.sensor_ip_address03);
  radar_status_.sensor_ip_address0 = ss0.str();

  ss1 << std::to_string(sensor_status_packet_.status.sensor_ip_address10) << "."
      << std::to_string(sensor_status_packet_.status.sensor_ip_address11) << "."
      << std::to_string(sensor_status_packet_.status.sensor_ip_address12) << "."
      << std::to_string(sensor_status_packet_.status.sensor_ip_address13);
  radar_status_.sensor_ip_address1 = ss1.str();

  radar_status_.configuration_counter = sensor_status_packet_.configuration_counter;

  radar_status_.longitudinal_velocity_status =
    sensor_status_packet_.longitudinal_velocity_status == 0   ? "VDY_OK"
    : sensor_status_packet_.longitudinal_velocity_status == 1 ? "VDY_NOTOK"
                                                              : "INVALID VALUE";

  radar_status_.longitudinal_acceleration_status =
    sensor_status_packet_.longitudinal_acceleration_status == 0   ? "VDY_OK"
    : sensor_status_packet_.longitudinal_acceleration_status == 1 ? "VDY_NOTOK"
                                                                  : "INVALID VALUE";

  radar_status_.lateral_acceleration_status =
    sensor_status_packet_.lateral_acceleration_status == 0   ? "VDY_OK"
    : sensor_status_packet_.lateral_acceleration_status == 1 ? "VDY_NOTOK"
                                                             : "INVALID VALUE";

  radar_status_.yaw_rate_status = sensor_status_packet_.yaw_rate_status == 0   ? "VDY_OK"
                                  : sensor_status_packet_.yaw_rate_status == 1 ? "VDY_NOTOK"
                                                                               : "INVALID VALUE";

  radar_status_.steering_angle_status = sensor_status_packet_.steering_angle_status == 0 ? "VDY_OK"
                                        : sensor_status_packet_.steering_angle_status == 1
                                          ? "VDY_NOTOK"
                                          : "INVALID VALUE";

  radar_status_.driving_direction_status =
    sensor_status_packet_.driving_direction_status == 0   ? "VDY_OK"
    : sensor_status_packet_.driving_direction_status == 1 ? "VDY_NOTOK"
                                                          : "INVALID VALUE";

  radar_status_.characteristic_speed_status =
    sensor_status_packet_.characteristic_speed_status == 0   ? "VDY_OK"
    : sensor_status_packet_.characteristic_speed_status == 1 ? "VDY_NOTOK"
                                                             : "INVALID VALUE";

  if (sensor_status_packet_.radar_status == 0) {
    radar_status_.radar_status = "STATE_INIT";
  } else if (sensor_status_packet_.radar_status == 1) {
    radar_status_.radar_status = "STATE_OK";
  } else if (sensor_status_packet_.radar_status == 2) {
    radar_status_.radar_status = "STATE_INVALID";
  } else {
    radar_status_.radar_status = "INVALID VALUE";
  }

  if (sensor_status_packet_.voltage_status == 0) {
    radar_status_.voltage_status = "Ok";
  }
  if (sensor_status_packet_.voltage_status & 0x01) {
    radar_status_.voltage_status += "Current under voltage";
  }
  if (sensor_status_packet_.voltage_status & 0x02) {
    radar_status_.voltage_status = "Past under voltage";
  }
  if (sensor_status_packet_.voltage_status & 0x03) {
    radar_status_.voltage_status = "Current over voltage";
  }
  if (sensor_status_packet_.voltage_status & 0x04) {
  ...
  if (sensor_status_packet_.temperature_status & 0x01) {
    radar_status_.temperature_status += "Current under temperature";
  }
  if (sensor_status_packet_.temperature_status & 0x02) {
    radar_status_.temperature_status += "Past under temperature";
  }
  if (sensor_status_packet_.temperature_status & 0x03) {
    radar_status_.temperature_status += "Current over temperature";
  }
  if (sensor_status_packet_.temperature_status & 0x04) {
    radar_status_.temperature_status += "Past over temperature";
  }

  const uint8_t & blockage_status0 = sensor_status_packet_.blockage_status & 0x0f;
  const uint8_t & blockage_status1 = (sensor_status_packet_.blockage_status & 0xf0) >> 4;
