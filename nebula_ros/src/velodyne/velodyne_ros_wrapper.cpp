  ...
  , decoder_wrapper_()
{
  ...
  // declaration
  parameter_event_cb_ =
      add_on_set_parameters_callback(std::bind(&VelodyneRosWrapper::OnParameterChange, this, std::placeholders::_1));
}
...
Status VelodyneRosWrapper::ValidateAndSetConfig(std::shared_ptr<const drivers::VelodyneSensorConfiguration>& new_config)
{
...
void VelodyneRosWrapper::ReceiveScanMessageCallback(std::unique_ptr<velodyne_msgs::msg::VelodyneScan> scan_msg)
{
  if (hw_interface_wrapper_)
  {
...
rcl_interfaces::msg::SetParametersResult VelodyneRosWrapper::OnParameterChange(const std::vector<rclcpp::Parameter>& p)
{
