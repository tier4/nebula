...
void HesaiHwMonitorWrapper::InitializeHesaiDiagnostics()
{
  ...
  diagnostics_updater_.add("hesai_temperature", this, &HesaiHwMonitorWrapper::HesaiCheckTemperature);
  ...
  fetch_diagnostics_timer_ =
      parent_node_->create_wall_timer(std::chrono::milliseconds(diag_span_), std::move(fetch_diag_from_sensor));
...
std::string HesaiHwMonitorWrapper::GetPtreeValue(boost::property_tree::ptree* pt, const std::string& key)
{
...
void HesaiHwMonitorWrapper::OnHesaiStatusTimer()
{
  ...
  {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("HesaiHwMonitorWrapper::OnHesaiStatusTimer(std::system_error)"),
                        error.what());
  }
  catch (const boost::system::system_error& error)
  {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("HesaiHwMonitorWrapper::OnHesaiStatusTimer(boost::system::system_error)"),
                        error.what());
  }
...
void HesaiHwMonitorWrapper::OnHesaiLidarMonitorTimerHttp()
{
  ...
  try
  {
    hw_interface_->GetLidarMonitorAsyncHttp([this](const std::string& str) {
      ...
      current_lidar_monitor_time_.reset(new rclcpp::Time(parent_node_->get_clock()->now()));
      current_lidar_monitor_tree_ = std::make_unique<boost::property_tree::ptree>(hw_interface_->ParseJson(str));
    });
  }
  catch (const std::system_error& error)
  {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("HesaiHwMonitorWrapper::OnHesaiLidarMonitorTimerHttp(std::system_error)"),
                        error.what());
  }
  catch (const boost::system::system_error& error)
  {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("HesaiHwMonitorWrapper::OnHesaiLidarMonitorTimerHttp(boost::system::system_"
                                           "error)"),
                        error.what());
  }
...
void HesaiHwMonitorWrapper::OnHesaiLidarMonitorTimer()
{
  ...
  {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("HesaiHwMonitorWrapper::OnHesaiLidarMonitorTimer(std::system_error)"),
                        error.what());
  }
  catch (const boost::system::system_error& error)
  {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("HesaiHwMonitorWrapper::OnHesaiLidarMonitorTimer(boost::system::system_"
                                           "error)"),
                        error.what());
  }
...
void HesaiHwMonitorWrapper::HesaiCheckStatus(diagnostic_updater::DiagnosticStatusWrapper& diagnostics)
{
  ...
  if (current_status_)
  {
    ...
    diagnostics.add("total_operation_time", std::to_string(current_status_->total_operation_time.value()));
...
void HesaiHwMonitorWrapper::HesaiCheckTemperature(diagnostic_updater::DiagnosticStatusWrapper& diagnostics)
{
  ...
  if (current_status_)
  {
    ...
    {
      diagnostics.add(temperature_names_[i],
...
void HesaiHwMonitorWrapper::HesaiCheckVoltageHttp(diagnostic_updater::DiagnosticStatusWrapper& diagnostics)
{
  std::scoped_lock lock(mtx_lidar_monitor_);
...
void HesaiHwMonitorWrapper::HesaiCheckVoltage(diagnostic_updater::DiagnosticStatusWrapper& diagnostics)
{
  std::scoped_lock lock(mtx_lidar_monitor_);
  if (current_monitor_)
  {
    uint8_t level = diagnostic_msgs::msg::DiagnosticStatus::OK;
    std::vector<std::string> msg;
    diagnostics.add("input_voltage", GetFixedPrecisionString(current_monitor_->input_voltage.value() * 0.01, 3) + " V");
    diagnostics.add("input_current", GetFixedPrecisionString(current_monitor_->input_current.value() * 0.01, 3) + " m"
                                                                                                                  "A");
    diagnostics.add("input_power", GetFixedPrecisionString(current_monitor_->input_power.value() * 0.01, 3) + " W");
