  ...
  diagnostic_updater::DiagnosticStatusWrapper & diagnostics)
{
  ...
  if (current_status_) {
    ...
    for (size_t i = 0; i < std::size(current_status_->temperature); i++) {
      diagnostics.add(
        temperature_names_[i], GetFixedPrecisionString(current_status_->temperature[i].value() * 0.01));
    }
    diagnostics.summary(level, boost::algorithm::join(msg, ", "));
  } else {
    diagnostics.summary(diagnostic_msgs::msg::DiagnosticStatus::WARN, "No data available");
...
void HesaiRosWrapper::HesaiCheckVoltage(diagnostic_updater::DiagnosticStatusWrapper & diagnostics)
{
  ...
  if (current_monitor_) {
    ...
    std::vector<std::string> msg;
    diagnostics.add(
      "input_voltage", GetFixedPrecisionString(current_monitor_->input_voltage.value() * 0.01) + " V");
    diagnostics.add(
      "input_current", GetFixedPrecisionString(current_monitor_->input_current.value() * 0.01) + " mA");
    diagnostics.add(
      "input_power", GetFixedPrecisionString(current_monitor_->input_power.value() * 0.01) + " W");
