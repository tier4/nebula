  ...
  driver_ptr_(new drivers::RobosenseDriver(config, calibration))
{
  ...
  if (hw_interface_)
  {
    ...
    packets_pub_ =
        parent_node->create_publisher<robosense_msgs::msg::RobosenseScan>("robosense_packets", rclcpp::SensorDataQoS());
  }
  ...
  nebula_points_pub_ = parent_node->create_publisher<sensor_msgs::msg::PointCloud2>("robosense_points", pointcloud_qos);
...
void RobosenseDecoderWrapper::ProcessCloudPacket(std::unique_ptr<nebula_msgs::msg::NebulaPacket> packet_msg) {
...
void RobosenseDecoderWrapper::OnConfigChange(const std::shared_ptr<const nebula::drivers::RobosenseSensorConfiguration>& new_config) {
  std::lock_guard lock(mtx_driver_ptr_);
