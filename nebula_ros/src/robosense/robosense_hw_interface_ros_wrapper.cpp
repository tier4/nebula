...
namespace ros
{
RobosenseHwInterfaceRosWrapper::RobosenseHwInterfaceRosWrapper(const rclcpp::NodeOptions & options)
: rclcpp::Node("robosense_hw_interface_ros_wrapper", options), hw_interface_()
{
  ...
  if (this->setup_sensor) {
    set_param_res_ = this->add_on_set_parameters_callback(
      std::bind(&RobosenseHwInterfaceRosWrapper::paramCallback, this, std::placeholders::_1));
  }
#endif
  StreamStart();
}
...
Status RobosenseHwInterfaceRosWrapper::InitializeHwInterface(  // todo: don't think this is needed
...
rcl_interfaces::msg::SetParametersResult RobosenseHwInterfaceRosWrapper::paramCallback(
  const std::vector<rclcpp::Parameter> & p)
{
  ...
  if (
    ...
    get_param(p, "lidar_ip", new_param.sensor_ip) || get_param(p, "frame_id", new_param.frame_id) ||
    get_param(p, "data_port", new_param.data_port) ||
    get_param(p, "difop_port", new_param.difop_port) ||
...
RobosenseHwInterfaceRosWrapper::updateParameters()
{
  std::scoped_lock lock(mtx_config_);
  ...
  auto results = set_parameters(
    {rclcpp::Parameter("sensor_model", os_sensor_model.str()),
     ...
     rclcpp::Parameter("lidar_ip", sensor_configuration_.sensor_ip),
     rclcpp::Parameter("frame_id", sensor_configuration_.frame_id),
     rclcpp::Parameter("data_port", sensor_configuration_.data_port),
     rclcpp::Parameter("difop_port", sensor_configuration_.difop_port),
