...
{
HesaiRosOfflineExtractBagDecode::HesaiRosOfflineExtractBagDecode(
...
Status HesaiRosOfflineExtractBagDecode::initialize_driver(
...
Status HesaiRosOfflineExtractBagDecode::get_status()
{
  return wrapper_status_;
}

Status HesaiRosOfflineExtractBagDecode::get_parameters(
  drivers::HesaiSensorConfiguration & sensor_configuration,
  drivers::HesaiCalibrationConfiguration & calibration_configuration,
  drivers::HesaiCorrection & correction_configuration)
{
  ...
  output_topic_ = this->declare_parameter<std::string>("output_topic", "", param_read_only());
...
Status HesaiRosOfflineExtractBagDecode::read_bag()
{
  ...
  while (reader.has_next()) {
    ...
    for (auto & pkt : extracted_msg.packets) {
      ...
      auto pointcloud = std::get<0>(pointcloud_ts);  // This is a pcl::PointCloud<PointXYZIRCAEDT>;
      ...
      if (!bag_writer) {
        ...
        bag_writer->create_topic(
          {output_topic_, "sensor_msgs/msg/PointCloud2", rmw_get_serialization_format(), ""});
      }
      ...
      // Create ROS2 Pointcloud and Write to ROSBag
      {
        ...
        cloud_bag_msg->topic_name = output_topic_;
