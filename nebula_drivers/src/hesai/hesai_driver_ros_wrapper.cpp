...
: rclcpp::Node(node_name, options)
{
  ...
  InitializeDriver(std::const_pointer_cast<drivers::CloudConfigurationBase>(cloud_cfg_ptr),
  ...
  pandar_points_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("pandar_points", rclcpp::SensorDataQoS());
}

Status HesaiDriverRosWrapper::StreamStart() {

  return Status::OK;
}

Status HesaiDriverRosWrapper::StreamStop() {
  return Status::OK;
}

Status HesaiDriverRosWrapper::Shutdown() {
  return Status::OK;
}

Status HesaiDriverRosWrapper::ReceiveScanMsgCallback(const pandar_msgs::msg::PandarScan::SharedPtr scan_msg)
{
