...
public:
  ...
  std::string post(
    const std::string & url, const std::string & data, int timeout_seconds) const override
  {
    auto full_url = url_combine(base_url_, url);
    return execute_request(
      RequestType::TYPE_POST, full_url, timeout_seconds, data.c_str(), data.size());
  }

  std::string post(
    const std::string & url, const std::vector<uint8_t> & data, int timeout_seconds) const override
  {
    auto full_url = url_combine(base_url_, url);
    return execute_request(
    ...
    const std::string & url, const std::string & data, int timeout_seconds) const override
  {
    auto full_url = url_combine(base_url_, url);
    return execute_request(
      RequestType::TYPE_PUT, full_url, timeout_seconds, data.c_str(), data.size());
  }
...
private:
  ...
  std::string execute_request(
    RequestType type, const std::string & url, int timeout_seconds, const char * data = nullptr,
    size_t data_size = 0, int attempts = 3, int retry_delay_ms = 500) const
  {
    ...
    throw std::runtime_error(
      std::string(
        "CurlClient::execute_request failed for url: [" + url + "] with the code: [" +
        std::to_string(http_code) + std::string("] - and return: ") + buffer_));
  }

  static size_t write_memory_callback(
    void * contents, size_t element_size, size_t elements_count, void * user_pointer)
  {
