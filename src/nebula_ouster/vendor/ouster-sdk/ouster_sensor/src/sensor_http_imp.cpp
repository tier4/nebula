...
std::string SensorHttpImp::get_config_params(bool active, int timeout_sec) const
{
  if (firmware_version() < MINIMUM_STAGING_VERSION) {
    ...
    return get(
      std::string("api/v1/" + api_prefix_ + "sensor/cmd/get_config_param?args=") + config_type,
      timeout_sec);
  }
  auto config_type = active ? "false" : "true";
  return get(
    std::string("api/v1/" + api_prefix_ + "sensor/config?staging=") + config_type, timeout_sec);
}

void SensorHttpImp::set_config_param(
  const std::string & key, const std::string & value, int timeout_sec) const
{
  if (firmware_version() < MINIMUM_STAGING_VERSION) {
    auto encoded_value = http_client_->encode(value);  // encode config params
    auto url =
      "api/v1/" + api_prefix_ + "sensor/cmd/set_config_param?args=" + key + "+" + encoded_value;
...
{
  if (firmware_version() < MINIMUM_STAGING_VERSION) {
    return get("api/v1/" + api_prefix_ + "sensor/cmd/get_config_param?args=staged", timeout_sec);
  }
...
void SensorHttpImp::save_config_params(int timeout_sec) const
{
  ...
  std::string json = "{}";
  http_client_->post(
    "api/v1/" + api_prefix_ + "sensor/config?persist=true&reinit=false", json, timeout_sec);
}
  ...
  int timeout_sec, nonstd::optional<std::string> original_destination_in) const
{
  ...
  if (original_destination_in.has_value()) {
    ...
    original_destination = SensorConfig(get_config_params(true, timeout_sec)).udp_dest.value_or("");
  }
  ...
  root.dump(json);
  http_client_->post(
    "api/v1/" + api_prefix_ + "sensor/config?reinit=False&persist=False", json, timeout_sec);
...
UserDataAndPolicy SensorHttpImp::get_user_data_and_policy(int timeout_sec) const
{
  ...
  try {
    ...
    return {
      root["policy"].as<std::string>() != "clear_on_config_delete",
...
void SensorHttpImp::set_user_data(
  const std::string & data, bool keep_on_config_delete, int timeout_sec) const
{
  if (is_vlp()) {
    throw std::runtime_error("user data API not supported on this FW version");
  }
...
std::vector<uint8_t> SensorHttpImp::get_zone_monitor_config_zip(bool staged, int timeout_sec) const
{
  ...
  std::string str = http_client_->get(
    std::string("api/v1/" + api_prefix_ + "zone_monitor/") + (staged ? "staged" : "active") +
      "/zip",
    timeout_sec);
  ...
  const std::vector<uint8_t> & zip_archive, int timeout_sec) const
{
  ...
  http_client_->post("api/v1/" + api_prefix_ + "zone_monitor/staged/zip", zip_archive, timeout_sec);
  ...
  const std::vector<uint8_t> & zone_ids, int timeout_sec) const
{
  ...
  http_client_->post(
    "api/v1/" + api_prefix_ + "zone_monitor/live_ids", jsoncons::json(zone_ids).to_string(),
    timeout_sec);
}
...
void SensorHttpImp::execute(
  const std::string & url, const std::string & validation, int timeout_sec) const
{
  auto result = get(url, timeout_sec);
  if (result != validation) {
    throw std::runtime_error(
      "SensorHttpImp::execute failed! url: " + url + " returned [" + result + "], expected [" +
      validation + "]");
  }
}
...
void SensorHttpImp::set_static_ip(
  const std::string & ip_address, const std::string & gateway_address, int timeout_sec) const
{
...
void SensorHttpImp_2_4_or_3::set_user_data(
  const std::string & /*data*/, bool /*keep_on_config_delete*/, int /*timeout_sec*/) const
{
  throw std::runtime_error("user data API not supported on this FW version");
}
