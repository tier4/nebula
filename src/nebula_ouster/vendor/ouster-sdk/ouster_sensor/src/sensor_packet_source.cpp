...
bool parse_and_validate_metadata(
  const jsoncons::json & json_data, SensorInfo & sensor_info, ValidatorIssues & issues);
}
namespace sensor
{
...
SOCKET mtp_data_socket(
  int port, const std::set<std::string> & udp_dest_hosts, const std::string & mtp_dest_host = "",
  bool reuse_ports = true);
bool set_config(
  SensorHttp & sensor_http, const SensorConfig & config, uint8_t config_flags, int timeout_sec);

void add_socket_to_groups(
  SOCKET sock_fd, const std::set<std::string> & udp_dest_hosts,
  const std::string & mtp_dest_host = "")
{
  ...
  for (const auto & udp_dest_host : udp_dest_hosts) {
    ...
    if (
      setsockopt(
        sock_fd, IPPROTO_IP, IP_ADD_MEMBERSHIP, reinterpret_cast<char *>(&mreq), sizeof(mreq)) !=
      0) {
...
SensorInfo Sensor::fetch_metadata(int timeout) const
{
  ...
  if (http_client_ptr->firmware_version() >= ouster::sdk::core::Version{3, 2, 0}) {
    try {
      ...
      result.zone_set = std::move(ZoneSet(std::move(zone_set_config_bytes)));
...
static std::vector<Sensor> calculate_sensors(
  const std::vector<std::string> & sources, SensorPacketSourceOptions & options)
{
  ...
  for (const auto & hostname : sources) {
    ...
    if (!do_not_reinitialize) {
      ...
      if (!no_auto_udp_dest) {
        if (!orig_config.udp_dest || !in_multicast(orig_config.udp_dest.value())) {
          try {
            ...
            if (
              !http_client->is_vlp() && orig_config.udp_dest_zm.has_value() &&
      ...
      if (orig_config.operating_mode != config.operating_mode) {
        printf(
          "Will change sensor's operating mode from %s to %s\n",
      ...
      if (config.udp_port_imu && orig_config.udp_port_imu != config.udp_port_imu) {
        ...
        printf(
          "Will change sensor's IMU port from %i to %s\n", orig_config.udp_port_imu.value(),
          port_name.c_str());
      }
      ...
      if (config.udp_port_lidar && orig_config.udp_port_lidar != config.udp_port_lidar) {
        ...
        printf(
          "Will change sensor's lidar port from %i to %s\n", orig_config.udp_port_lidar.value(),
          port_name.c_str());
      }
    } else {
      try {
      ...
      if (desired_lidar && orig_config.udp_port_lidar != desired_lidar.value()) {
        throw std::runtime_error(
          "Sensor's lidar port " + std::to_string(orig_config.udp_port_lidar.value()) +
          " does "
      ...
      if (desired_imu && orig_config.udp_port_imu != desired_imu.value()) {
        throw std::runtime_error(
          "Sensor's IMU port " + std::to_string(orig_config.udp_port_imu.value()) +
          " does "
...
SensorPacketSource::SensorPacketSource(
  const std::string & source, SensorPacketSourceOptions options)
    ...
    options.reuse_ports.retrieve())
{
  ...
  populate_extrinsics(
    options.extrinsics_file.retrieve(), options.extrinsics.retrieve(), sensor_info_);
}

SensorPacketSource::SensorPacketSource(
  const std::vector<Sensor> & sensors, double timeout, double buffer_time)
: SensorPacketSource(sensors, {}, timeout, buffer_time)
{
}

SensorPacketSource::SensorPacketSource(
  const std::vector<Sensor> & sensors, const std::vector<SensorInfo> & infos, double config_timeout,
  double buffer_time, bool reuse_ports)
{
  ...
  if (!infos.empty()) {
    ...
    for (size_t i = 0; i < sensors.size(); i++) {
      ...
      if (
        ...
        (config.udp_port_imu && config.udp_port_imu != sensor_info_[i]->config.udp_port_imu)) {
        throw std::invalid_argument(
    ...
    for (size_t i = 0; i < sensors.size(); i++) {
      ...
      set_config(*sensor.http_client(), desired_config, 0 /*flags*/, config_timeout);
    }

    // fetch metadata
    // do this last so we dont wait N*reinit time to reconfigure lidars
    for (size_t i = 0; i < sensors.size(); i++) {
      sensor_info_.emplace_back(new SensorInfo(sensors[i].fetch_metadata(config_timeout)));
    }
  }
  ...
  for (const auto & sensor : sensors) {
    ...
    if (getaddrinfo(sensor.hostname().c_str(), nullptr, &hints, &result) != 0) {
      throw std::runtime_error("Could not resolve address '" + sensor.hostname() + "' for sensor.");
    }
  ...
  for (auto port : ports) {
    ...
    SOCKET sock = mtp_data_socket(port.first, multicast_addrs, "", reuse_ports);
    if (sock == SOCKET_ERROR) {
      close();
      throw std::runtime_error(
        "failed to obtain a UDP socket on port " + std::to_string(port.first));
    }
    sockets_.push_back(sock);
...
static PacketType get_packet_type(const PacketFormat & format, uint8_t * buf, size_t size)
{
  ...
  std::vector<uint8_t> & data, uint64_t & timestamp, double timeout_sec)
{
  ...
  for (auto sock : sockets_) {
    ...
    auto size = recvfrom(
      sock, reinterpret_cast<char *>(data.data()), 65535, 0,
        ...
        if (event.type == ClientEvent::POLL_TIMEOUT) {
          ...
          if (client_->iterator_timeout_ > 0) {
            ...
            if (time_elapsed_ns > timeout_ns) {
              throw ouster::sdk::sensor::ClientTimeout("No packets received in timeout.");
            }
            // update timeout for timeout remaining
            current_timeout_s = current_timeout_s - time_elapsed_ns / 1e9;
          }
        } else {
