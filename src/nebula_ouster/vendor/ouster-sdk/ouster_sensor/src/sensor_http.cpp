...
std::string SensorHttp::firmware_version_string(const std::string & hostname, int timeout_sec)
{
...
Version SensorHttp::firmware_version(const std::string & hostname, int timeout_sec)
{
...
std::unique_ptr<SensorHttp> SensorHttp::create(const std::string & hostname, int timeout_sec)
{
  ...
  if (fw == INVALID_VERSION || fw.major < 2 || (fw.major == 2 && fw.minor <= 3)) {
