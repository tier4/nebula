...
public:
   ...
   */
  OUSTER_API_FUNCTION inline ouster::sdk::core::Packet & packet() { return *packet_; }
...
public:
  ...
  OUSTER_API_FUNCTION
  Sensor(
    const std::string & hostname,                   ///< [in] sensor hostname
    const ouster::sdk::core::SensorConfig & config  ///< [in] desired sensor configuration
  );
  ...
  OUSTER_API_FUNCTION
  inline const ouster::sdk::core::SensorConfig & desired_config() const { return config_; }
...
: public ouster::sdk::core::PacketSource,
  ouster::sdk::impl::PacketSourceBuilderMulti<ouster::sdk::core::IoType::SENSOR, SensorPacketSource>
{
public:
  ...
  SensorPacketSource(
    ...
    const std::function<void(SensorPacketSourceOptions &)> & options = {}  ///< [in] source options
  );
...
private:
  ...
  InternalEvent get_packet_internal(std::vector<uint8_t> & data, uint64_t & ts, double timeout_sec);

  /// Start a background thread to do buffering if requested
  void start_buffer_thread(
    double buffer_time  ///< [in] time in seconds
  );
};
}  // namespace sensor
}  // namespace sdk
