...
template <typename T>
void cartesianT(
  PointCloudXYZ<T> & points, const Eigen::Ref<const img_t<uint32_t>> & range,
 ...
 */
template <typename T>
PointCloudXYZ<T> cartesianT(
  const Eigen::Ref<const img_t<uint32_t>> & range, const ArrayX3R<T> & direction,
...
template <typename T>
PointCloudXYZ<T> cartesianT(
  const LidarScan & scan, const ArrayX3R<T> & direction, const ArrayX3R<T> & offset)
{
