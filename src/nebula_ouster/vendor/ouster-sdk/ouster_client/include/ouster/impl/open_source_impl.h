  ...
  void check(const char * name, const char * source_name) const
  {
    if (set_) {
      throw std::runtime_error(
        "Parameter '" + std::string(name) + "' not supported by " + std::string(source_name) + ".");
    }
  }
...
using ScanBuilderT = std::function<std::unique_ptr<core::ScanSource>(
  const std::vector<std::string> &, const ouster::sdk::ScanSourceOptions &, bool /*collate*/,
  int /*sensor_idx*/)>;
...
public:
  ...
  static bool register_type()
  {
    get_builders()[type] =
      [](
        const std::vector<std::string> & sources, const ouster::sdk::ScanSourceOptions & options,
        bool collate, int sensor_idx) { return T::create(sources, options, collate, sensor_idx); };
    return true;
  }
...
const bool ScanSourceBuilder<type, T>::REGISTERED = ScanSourceBuilder<type, T>::register_type();
