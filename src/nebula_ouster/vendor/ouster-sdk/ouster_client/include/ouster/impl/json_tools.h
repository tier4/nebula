...
protected:
   ...
   */
  static void skipped_due_to_item(
    ValidatorIssues::EntryList & severity, const std::string & item_skipped,
    const std::string & cause_item, const std::string & explanation = "");
  ...
  static bool verify_all_not_zero(
   ...
   */
  static bool verify_string_not_empty(
    ValidatorIssues::EntryList & severity, const std::string & path, const std::string & data)
  {
    if (!data.empty()) {
      return true;
    } else {
  ...
  {
    return [lower, upper](ValidatorIssues::EntryList & severity, const std::string & path, T data) {
      bool result = true;
      if (data < lower) {
        std::stringstream error_message;
        error_message << "Item value " << data << " is lower than the lower bound " << lower;
      ...
      if (data > upper) {
        std::stringstream error_message;
        error_message << "Item value " << data << " is greater than the upper bound " << upper;
  ...
  bool parse_and_validate_item(
    ValidatorIssues::EntryList & severity, const std::string & path, T & output,
    F verification_callback, bool relaxed_number_verification = false)
  {
    ...
    if (value_array.size() == 1) {
      ...
      if (
        value.is<T>() ||
        (relaxed_number_verification && value.is_number() && std::is_arithmetic<T>::value)) {
      ...
      error_message << "Expected One Item In Data, "
                    << "Number Of Items: " << value_array.size() << " Values: \"" << value_array
                    << "\"";
      auto entry = ValidatorIssues::ValidatorEntry(path, error_message.str());
      severity.push_back(entry);
    }
  ...
  template <typename T>
  bool parse_and_validate_item(
    ValidatorIssues::EntryList & severity, const std::string & path, nonstd::optional<T> & output,
    bool relaxed_number_verification = false)
  {
    T data;
  ...
  template <typename T>
  bool parse_and_validate_item(
    ValidatorIssues::EntryList & severity, const std::string & path, std::vector<T> & output,
    size_t verify_count, bool relaxed_number_verification = false)
  {
    size_t index = 0;
    size_t matches = 0;
  ...
  bool parse_and_validate_item(
    ValidatorIssues::EntryList & severity, const std::string & path, std::vector<T> & output,
    size_t verify_count, F verification_callback, bool relaxed_number_verification = false)
  {
    size_t index = 0;
    size_t matches = 0;
    std::vector<T> shadow_output;
   ...
   */
  template <
    typename T, typename U, typename F, typename = std::enable_if<std::is_function<F>::value, F>>
  bool parse_and_validate_enum(
    ValidatorIssues::EntryList & severity, const std::string & path, nonstd::optional<U> & output,
    F func)
  {
    T data;
    if (parse_and_validate_item<T>(severity, path, data)) {
      try {
        ...
        std::stringstream error_message;
        error_message << "Failed To Parse Enum: " << data << " Error Message: \"" << e.what()
                      << "\"";
        severity.emplace_back(path, error_message.str());
        return false;
      }
  ...
  template <
    typename T, typename U, typename F, typename = std::enable_if<std::is_function<F>::value, F>>
  bool parse_and_validate_enum(
    ValidatorIssues::EntryList & severity, const std::string & path, U & output, F func)
  {
   ...
   */
  template <typename T>
  bool parse_and_validate_datetime(
    ValidatorIssues::EntryList & severity, const std::string & path,
