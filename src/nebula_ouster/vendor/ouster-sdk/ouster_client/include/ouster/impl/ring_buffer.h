...
public:
  RingBuffer(size_t size, T value = {}) : r_idx_(0), w_idx_(0), bufs_(size + 1, value) {}
   ...
   */
  size_t size() const { return (_capacity() + w_idx_ - r_idx_) % _capacity(); }
  ...
  void pop()
  {
    ...
    while (!r_idx_.compare_exchange_strong(read_idx, (read_idx + 1) % _capacity())) {
    }
  }
  ...
  void push()
  {
    ...
    while (!w_idx_.compare_exchange_strong(write_idx, (write_idx + 1) % _capacity())) {
    }
  }
};
...
public:
  ...
  {
    return std::any_of(
      rb_map_.begin(), rb_map_.end(), [](const auto & kv) { return kv.second.full(); });
  }
  ...
  {
    return std::any_of(
      rb_map_.begin(), rb_map_.end(), [](const auto & kv) { return kv.second.empty(); });
  }
  ...
  size_t size() const
  {
    return std::accumulate(
      rb_map_.begin(), rb_map_.end(), size_t{0},
      [](size_t total, const auto & kv) { return total + kv.second.size(); });
  }
  ...
  size_t capacity() const
  {
    return std::accumulate(
      rb_map_.begin(), rb_map_.end(), size_t{0},
      [](size_t total, const auto & kv) { return total + kv.second.capacity(); });
  }
};
