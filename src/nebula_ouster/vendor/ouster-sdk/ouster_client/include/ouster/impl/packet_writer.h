...
public:
  ...
  OUSTER_API_FUNCTION
  void set_shot_limiting_countdown(uint8_t * lidar_buf, uint8_t shot_limiting_countdown) const;
  OUSTER_API_FUNCTION
  void set_shutdown(uint8_t * lidar_buf, uint8_t status) const;
  OUSTER_API_FUNCTION
  void set_shutdown_countdown(uint8_t * lidar_buf, uint8_t shutdown_countdown) const;
   ...
   */
  template <typename T>
  OUSTER_API_FUNCTION void set_block(
  ...
  OUSTER_API_FUNCTION
  void set_imu_nmea_sentence(uint8_t * imu_buf, const std::string & sentence) const;
  ...
  OUSTER_API_FUNCTION
  void set_zone_state(uint8_t * zone_measurement_ptr, const ZoneState & zone_state) const;
};

/**
 * @deprecated Use `PacketWriter` instead.
 */
OUSTER_DEPRECATED_TYPE(packet_writer, PacketWriter, OUSTER_DEPRECATED_LAST_SUPPORTED_0_16);
...
template <typename T>
inline void PacketWriter::unpack_raw_headers(
  Eigen::Ref<const img_t<T>> field, uint8_t * lidar_buf) const
{
