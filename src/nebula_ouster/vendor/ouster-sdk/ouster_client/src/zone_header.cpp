...
SerialNumber serial_number_from_int(uint64_t serial_number_int)
{
  ...
  std::copy_n(sn_str.begin(), std::min(sn_str.size(), serial_number.size()), serial_number.begin());
...
CacheRenderMetadata::CacheRenderMetadata(
  const Zrb & zrb, const std::bitset<2048> & valid_col_mask_init, SerialNumber serial_number_init)
...
CacheHeader::CacheHeader(
  const Zrb & zrb, const std::bitset<2048> & valid_col_mask_init, SerialNumber serial_number_init)
: info(CacheHeaderInfo()), meta(CacheRenderMetadata(zrb, valid_col_mask_init, serial_number_init))
{
}
