...
BeamConfig::BeamConfig(
  uint32_t n_cols_init, const std::vector<double> & px_altitudes_init,
  const std::vector<double> & px_azimuths_init, mat4d beam_to_lidar_transform_init,
  mat4d lidar_to_sensor_transform_init, mat4d sensor_to_body_transform_init, float m_per_zmbin_init,
...
: n_cols(n_cols_init),
  ...
  lut(
    std::make_shared<XYZLut>(make_xyz_lut(
      n_cols, n_rows, 0.001, beam_to_lidar_transform,
      scale_translation(sensor_to_body_transform) * lidar_to_sensor_transform, px_azimuths,
      px_altitudes))),
  lut_no_sensor_to_body_transform(
    std::make_shared<XYZLut>(make_xyz_lut(
      n_cols, n_rows, 0.001, beam_to_lidar_transform, lidar_to_sensor_transform, px_azimuths,
      px_altitudes)))
{
