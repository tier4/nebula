  ...
  const Poses & poses_known)
{
  ...
  return interp_pose<double, double>(x_interp_eigen, x_known_eigen, poses_known);
}
  ...
  const std::vector<Matrix4dR> & poses_known)
{
  ...
  for (size_t i = 0; i < poses_known.size(); ++i) {
    poses_known_eigen.row(i) =
      Eigen::Map<const Eigen::Matrix<double, 1, 16>>(poses_known[i].data(), 16);
  }

  // Call the templated interp_pose function
  return interp_pose<double, double>(x_interp_eigen, x_known_eigen, poses_known_eigen);
}
