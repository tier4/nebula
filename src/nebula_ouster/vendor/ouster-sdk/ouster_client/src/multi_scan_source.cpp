...
{
public:
  explicit MultiScanIteratorImpl(const MultiScanSource * scan_source, int sensor_idx = -1)
  {
    source_ = scan_source;
    ...
    for (size_t i = 0; i < scan_source->sources_.size(); i++) {
      ...
      if (iters_[i] != scan_source->sources_[i]->end()) {
        nexts_[i] = std::make_unique<LidarScanSet>(std::move(*iters_[i]));
      } else {
        iters_.erase(i);
      }
    }
  ...
  bool advance(size_t offset) override
  {
    ...
    for (size_t i = 0; i < offset; i++) {
      ...
      if (offset > 1 && source_->is_indexed() && sensor_idx_ < 0) {
        ...
        for (const auto & amt : increment_amounts) {
          ...
          if (iters_[amt.first] == source_->sources_[amt.first]->end()) {
            ...
            nexts_[amt.first] = std::make_unique<LidarScanSet>(std::move(*iters_[amt.first]));
          }
        }
      ...
      for (auto & item : nexts_) {
        ...
        nexts_[item.first] = std::make_unique<LidarScanSet>(std::move(*iter));
      }
...
MultiScanSource::MultiScanSource(std::vector<std::shared_ptr<ScanSource>> sources)
: sources_(std::move(sources))
{
  ...
  for (size_t src_index = 0; src_index < sources_.size(); src_index++) {
    ...
    for (size_t i = 0; i < f_idx.size(); i++) {
      ...
      full_index_.push_back({sample.first, src_idx_to_dest_index[sample.second]});
...
ScanIterator MultiScanSource::end() const
{
  return ScanIterator(this);
}

const std::vector<std::shared_ptr<SensorInfo>> & MultiScanSource::sensor_info() const
{
  return sensor_info_;
}

bool MultiScanSource::is_live() const
{
  return false;
}

bool MultiScanSource::is_indexed() const
{
  return indexed_;
}

size_t MultiScanSource::size() const
{
  return size_;
}

const std::vector<std::vector<std::pair<uint64_t, uint64_t>>> & MultiScanSource::individual_index()
  const
{
  if (!is_indexed()) {
...
const std::vector<std::pair<uint64_t, uint64_t>> & MultiScanSource::full_index() const
{
  if (!is_indexed()) {
...
size_t MultiScanSource::size_hint() const
{
  return size_hint_;
}
