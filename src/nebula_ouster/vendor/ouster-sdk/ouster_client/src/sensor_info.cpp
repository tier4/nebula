...
bool operator==(const DataFormat & lhs, const DataFormat & rhs)
{
  return (
    ...
    lhs.pixel_shift_by_row == rhs.pixel_shift_by_row && lhs.column_window == rhs.column_window &&
...
{
  return (
    this->sn == other.sn && this->fw_rev == other.fw_rev && this->prod_line == other.prod_line &&
    this->format == other.format && this->beam_azimuth_angles == other.beam_azimuth_angles &&
    ...
    this->prod_pn == other.prod_pn && this->status == other.status && this->cal == other.cal &&
...
std::string SensorInfo::to_json_string() const
{
  ...
  if (beam_azimuth_angles.size() == format.pixels_per_column) {
    ...
    for (size_t i = 0; i < beam_azimuth_angles.size(); i++) {
      ...
      result["beam_intrinsics"]["beam_azimuth_angles"][j - 1].emplace_back(beam_azimuth_angles[i]);
    }
...
{
  if (
    format.udp_profile_lidar == UDPProfileLidar::RNG19_RFL8_SIG16_NIR16_DUAL ||
...
{
  if (metadata.empty()) {
    throw std::invalid_argument("firmware_version_from_metadata metadata empty!");
  }

  jsoncons::json value_array =
    jsoncons::jsonpath::json_query(jsoncons::json::parse(metadata), "$.sensor_info.image_rev");
