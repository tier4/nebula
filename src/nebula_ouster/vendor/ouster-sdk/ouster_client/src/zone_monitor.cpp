...
bool parse_and_validate_zone_set_config_zip(
  const std::vector<uint8_t> & zip_bytes, ZoneSet & zone_set_config, ValidatorIssues & issues)
{
  Zip zip(zip_bytes);
  jsoncons::json metadata = jsoncons::json::parse(zip.get_file_as_string("metadata.json"));
...
void ZoneSet::check_invariants() const
{
  if (sensor_to_body_transform == mat4d::Zero()) {
    throw std::logic_error("ZoneSet: sensor_to_body_transform must be set.");
  }
  ...
  for (const auto & pair : zones) {
    ...
    if (zone.zrb) {
      if (first_zrb_n_rows == -1 && first_zrb_n_cols == -1) {
        ...
        if (
          zone.zrb->near_range_mm.rows() != first_zrb_n_rows ||
...
ZoneSet::ZoneSet(const std::vector<uint8_t> & zip_bytes)
{  // NOLINT(cppcoreguidelines-pro-type-member-init)
  ValidatorIssues issues;
  if (!parse_and_validate_zone_set_config_zip(zip_bytes, *this, issues)) {
    std::stringstream msg;
...
void ZoneSet::render(const SensorInfo & sensor_info)
{
  ...
  BeamConfig beam_config(
    sensor_info.w(), sensor_info.beam_altitude_angles, sensor_info.beam_azimuth_angles,
    sensor_info.beam_to_lidar_transform, sensor_info.lidar_to_sensor_transform,
    sensor_to_body_transform, m_per_zmbin, sensor_info.sn);
    ...
    items[future_idx++] = pool.enqueue([&]() {
      ...
      if (zone.zrb && !zone.stl) {
        ...
        if (zone.render(beam_config)) {
          ...
          throw std::runtime_error(
            "ZoneSet::render: zone " + std::to_string(iter.first) + " was out of sensor FOV.");
        }
      }
