  ...
  const std::vector<double> & altitude_angles_deg)
{
  ...
  if (
    (azimuth_angles_deg.size() != h || altitude_angles_deg.size() != h) &&
  ...
  if (beam_to_lidar_transform(2, 3) != 0) {
    beam_to_lidar_euclidean_distance_mm = std::sqrt(
      std::pow(beam_to_lidar_transform(0, 3), 2) + std::pow(beam_to_lidar_transform(2, 3), 2));
  }
  ...
  lut.offset.col(0) = encoder.cos() * beam_to_lidar_transform(0, 3) -
                      lut.direction.col(0) * beam_to_lidar_euclidean_distance_mm;
  lut.offset.col(1) = encoder.sin() * beam_to_lidar_transform(0, 3) -
                      lut.direction.col(1) * beam_to_lidar_euclidean_distance_mm;
  lut.offset.col(2) =
    -lut.direction.col(2) * beam_to_lidar_euclidean_distance_mm + beam_to_lidar_transform(2, 3);
...
XYZLut make_xyz_lut(const SensorInfo & sensor, bool use_extrinsics)
{
  ...
  return make_xyz_lut(
    sensor.format.columns_per_frame, sensor.format.pixels_per_column, RANGE_UNIT,
    sensor.beam_to_lidar_transform, transform, sensor.beam_azimuth_angles,
