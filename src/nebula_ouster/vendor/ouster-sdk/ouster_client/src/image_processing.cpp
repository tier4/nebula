...
: lo_percentile_(lo_percentile), hi_percentile_(hi_percentile), ae_update_every_(update_every)
{
}
...
void AutoExposure::apply(Eigen::Ref<img_t<T>> image, bool update_state)
{
  ...
  if (counter_ == 0 && update_state) {
    ...
    const size_t lo_kth_extreme = static_cast<size_t>(indices.size() * lo_percentile_);
    std::nth_element(indices.begin(), indices.begin() + lo_kth_extreme, indices.end(), cmp);
    lo_ = key_eigen[*(indices.begin() + lo_kth_extreme)];

    const size_t hi_kth_extreme = static_cast<size_t>(indices.size() * hi_percentile_);
    std::nth_element(
      indices.begin() + lo_kth_extreme, indices.end() - hi_kth_extreme - 1, indices.end(), cmp);
...
void AutoExposure::apply(Eigen::TensorMap<rgb_img_t<T>> image, bool update_state)
{
  ...
  if (counter_ == 0 && update_state) {
    ...
    auto cmp = [&](const size_t a, const size_t b) { return lum_flat[a] < lum_flat[b]; };

    const size_t lo_k = static_cast<size_t>(static_cast<double>(indices.size()) * lo_percentile_);
    ...
    const size_t hi_k = static_cast<size_t>(static_cast<double>(indices.size()) * hi_percentile_);
...
template <typename T>
void AutoExposure::apply(
  Eigen::Ref<img_t<T>> r, Eigen::Ref<img_t<T>> g, Eigen::Ref<img_t<T>> b, bool update_state)
{
  ...
  if (counter_ == 0 && update_state) {
    img_t<T> lum =
      r * static_cast<T>(0.299) + g * static_cast<T>(0.587) + b * static_cast<T>(0.114);
...
void AutoExposure::update(
  Eigen::Ref<img_t<float>> r, Eigen::Ref<img_t<float>> g, Eigen::Ref<img_t<float>> b,
  bool update_state)
{
...
void AutoExposure::update(
  Eigen::Ref<img_t<double>> r, Eigen::Ref<img_t<double>> g, Eigen::Ref<img_t<double>> b,
  bool update_state)
{
  apply(r, g, b, update_state);
}
...
void AutoExposure::update(
  Eigen::TensorMap<const rgb_img_t<float16_t>> input, Eigen::TensorMap<rgb_img_t<float>> out,
  bool update_state)
{
 ...
 */
template <typename T>
static Eigen::Array<T, -1, 1> compute_dark_count(const Eigen::Ref<img_t<T>> & image)
{
