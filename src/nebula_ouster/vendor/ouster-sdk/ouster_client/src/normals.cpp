...
double compute_vertical_subtent(
  size_t width, size_t height, const RangeValidFn & has_range, const BeamFn & normalized_beam)
{
...
void compute_unit_normals(
  const double * xyz, const uint32_t * range, const double * xyz2, const uint32_t * range2,
  size_t height, size_t width, const Eigen::Ref<const MatrixX3dR> sensor_origins,
  double * normals_data, size_t pixel_search_range, double min_angle_of_incidence_rad,
  double target_distance_m, double vertical_pixel_subtent_override = 0.0)
{
  ...
  const double vertical_pixel_subtent_rad =
    vertical_pixel_subtent_override > 0.0
      ...
      : compute_vertical_subtent(
          width, height, [&](size_t row, size_t col) { return range[flat_index(row, col)] != 0; },
          normalized_beam);
  ...
  auto calc_max_distance_threshold = [safe_incidence_rad](
                                       uint32_t range_mm, double pixel_subtent_rad) {
  ...
  auto find_best_neighbor = [&](
    ...
    // state.
    auto consider_neighbor = [&](
                               size_t row, size_t col, const double * xyz_base,
                               const uint32_t * rng_base, bool flip_flag, size_t current_radius) {
      ...
      if (
        static_cast<int64_t>(neighbor_range) - static_cast<int64_t>(center_range_mm) <
    ...
    for (size_t radius = 1; radius <= pixel_search_range; ++radius) {
      if (axis == NeighborAxis::VERTICAL && radius > max_up_radius && radius > max_down_radius) {
        break;
      }
      ...
      if (axis == NeighborAxis::VERTICAL) {
        ...
        size_t col_right = static_cast<size_t>(col_right_unwrapped % static_cast<int>(width));
      ...
      if (
        desired_neighbor_distance_sq <= min_distance_sq &&
        min_distance_sq < (static_cast<double>(best_radius) * static_cast<double>(best_radius) *
    ...
    for (size_t block_v = 0; block_v < width; block_v += block_size) {
      ...
      for (size_t u = block_u; u < end_u; ++u) {
        ...
        for (size_t v = block_v; v < end_v; ++v) {
          ...
          bool vertical_requires_flip = false;    // true if picked vertical neighbor lies above
          bool horizontal_requires_flip = false;  // true if picked horizontal neighbor lies to
                                                  // the left
          ...
          const bool vertical_found = find_best_neighbor(
            NeighborAxis::VERTICAL, u, v, neighbor_distance_vertical_sq, center_point, center_range,
            vertical_diff, vertical_requires_flip, vertical_thin_foreground_object, max_up_radius,
            max_down_radius);
          const bool horizontal_found = find_best_neighbor(
            NeighborAxis::HORIZONTAL, u, v, neighbor_distance_horizontal_sq, center_point,
          ...
          if (vertical_found && (!horizontal_found || horizontal_thin_foreground_object)) {
            ...
            continue;
          } else if (horizontal_found && (!vertical_found || vertical_thin_foreground_object)) {
  ...
  double min_angle_of_incidence_rad, double target_distance_m)
{
  ...
  compute_unit_normals(
    xyz.data(), range.data(), nullptr, nullptr, h, w, sensor_origins_xyz, normals.data(),
    pixel_search_range, min_angle_of_incidence_rad, target_distance_m);
  ...
  double min_angle_of_incidence_rad, double target_distance_m)
{
  ...
  if (
    static_cast<size_t>(xyz.rows()) != h * w || xyz.cols() != 3 ||
  ...
  compute_unit_normals(
    xyz.data(), range.data(), xyz2.data(), range2.data(), h, w, sensor_origins_xyz, first.data(),
    pixel_search_range, min_angle_of_incidence_rad, target_distance_m, vertical_pixel_subtent_rad);

  MatrixX3dR second = MatrixX3dR::Zero(num_pixels, 3);
  compute_unit_normals(
    xyz2.data(), range2.data(), xyz.data(), range.data(), h, w, sensor_origins_xyz, second.data(),
    pixel_search_range, min_angle_of_incidence_rad, target_distance_m, vertical_pixel_subtent_rad);
