...
std::string to_string(FieldClass flag)
{
  ...
  auto res = std::find_if(impl::FIELD_CLASS_STRINGS.begin(), end, [flag](const auto & pair) {
    return pair.first == flag;
  });
...
FieldView::FieldView(void * ptr, const FieldDescriptor & desc) : ptr_(ptr), desc_(desc)
{
}
  ...
  switch (other.desc().element_size) {
    ...
    default:
      ...
      throw std::invalid_argument(
        "uint_view: got wrong element size " + std::to_string(other.desc().element_size) +
