...
RotH RotV::exp(double & angle_out, double & sin_angle_out, double & cos_angle_out) const
{
  ...
  return RotH(
    Eigen::Matrix3d::Identity() + sin_angle_out * axis_hat +
    (1 - cos_angle_out) * axis_hat * axis_hat);
}

Eigen::Matrix3d RotV::vee(const double angle, const double sin_angle, const double cos_angle) const
{
  if (angle < EPS) {
...
RotQ RotV::q(double & angle, double & sin_angle, double & cos_angle) const
{
  ...
  if (std::abs(angle) > EPS) {
    quat = RotQ(
      cos_angle_2, sin_angle_2 * operator()(0) / angle, sin_angle_2 * operator()(1) / angle,
