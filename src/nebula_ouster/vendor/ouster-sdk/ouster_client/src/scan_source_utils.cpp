...
public:
  CollatorManager(int len, int64_t delta_t) : delta_t_(delta_t) { list_.resize(len); }
  ...
  bool collate(int index, T & item, int64_t timestamp, bool & consumed)
  {
    ...
    if (
      min_ts_ < 0 || max_ts_ < 0 ||
      (timestamp >= min_ts_ + delta_t_ || timestamp < max_ts_ - delta_t_)) {
...
public:
    ...
    collator_(source->sensor_info().size(), delta_t)
  {
    ...
    if (source_->is_indexed()) {
      ...
      for (size_t i = 0; i < source_->full_index().size();) {
        ...
        bool consumed = false;
        if (collator.collate(item.second, value, item.first, consumed)) {
          length_++;
  ...
  {
    if (!source_->is_indexed()) {
      throw std::runtime_error("Difference is not supported on non-indexed scan sources.");
    }
    return length_;
  }
...
Collator::Collator(const ScanSource & source, uint64_t dt_ns) : source_(&source), dt_(dt_ns)
{
}
...
Collator::Collator(std::unique_ptr<ScanSource> source, uint64_t dt_ns) : parent_(std::move(source))
{
  dt_ = dt_ns;
  source_ = parent_.get();
}
...
{
  return ScanIterator(this, new DefaultCollatedScanIteratorImpl(source_, dt_));
}
...
bool Collator::is_live() const
{
  return source_->is_live();
}

bool Collator::is_indexed() const
{
  return source_->is_indexed();
}

const std::vector<std::vector<std::pair<uint64_t, uint64_t>>> & Collator::individual_index() const
{
...
Singler::Singler(const ScanSource & source, size_t idx) : source_(&source), idx_(idx)
{
...
Singler::Singler(std::unique_ptr<ScanSource> source, size_t idx) : parent_(std::move(source))
{
  idx_ = idx;
  source_ = parent_.get();
  if (idx >= source_->sensor_info().size()) {
...
{
  if (!is_indexed()) {
    throw std::runtime_error("'individual_index' not supported on unindexed scan sources.");
  }
...
{
  if (!is_indexed()) {
    throw std::runtime_error("'full_index' not supported on unindexed scan sources.");
  }
  return full_index_;
}
...
ScanIterator Singler::begin() const
{
  return source_->begin(idx_);
}
...
ScanIterator Singler::end() const
{
  return source_->end();
}
...
bool Singler::is_live() const
{
  return source_->is_live();
}

bool Singler::is_indexed() const
{
  return source_->is_indexed();
}
...
const std::vector<size_t> & Singler::scans_num() const
{
  ...
  throw std::runtime_error("'scans_num' not supported on unindexed scan sources.");
}
...
static void normalize_slice(const ScanSource * source, int & start, int & end, int & step)
{
  if (step <= 0) {
...
{
  return ScanIterator(
    this, new SlicerScanIteratorImpl<ScanSource, ScanIterator, ScanIteratorImpl, LidarScanSet>(
...
bool Slicer::is_live() const
{
  return source_->is_live();
}

bool Slicer::is_indexed() const
{
  return source_->is_indexed();
}
...
AnyScanSource::AnyScanSource(std::unique_ptr<ScanSource> source) : source_(std::move(source))
{
}

ScanIterator AnyScanSource::begin() const
{
  return source_->begin();
}

ScanIterator AnyScanSource::begin(int idx) const
{
  return source_->begin(idx);
}

ScanIterator AnyScanSource::end() const
{
  return source_->end();
}

const std::vector<std::shared_ptr<SensorInfo>> & AnyScanSource::sensor_info() const
{
  return source_->sensor_info();
}

bool AnyScanSource::is_live() const
{
  return source_->is_live();
}

bool AnyScanSource::is_indexed() const
{
  return source_->is_indexed();
}

size_t AnyScanSource::size() const
{
  return source_->size();
}

size_t AnyScanSource::size_hint() const
{
  return source_->size_hint();
}

const std::vector<std::vector<std::pair<uint64_t, uint64_t>>> & AnyScanSource::individual_index()
  const
{
  return source_->individual_index();
}

const std::vector<std::pair<uint64_t, uint64_t>> & AnyScanSource::full_index() const
{
  return source_->full_index();
}
...
std::shared_ptr<ScanSource> AnyScanSource::child() const
{
  return source_;
}

void AnyScanSource::close()
{
  source_.reset();
}

AnyPacketSource::AnyPacketSource(std::unique_ptr<PacketSource> source) : source_(std::move(source))
{
}
