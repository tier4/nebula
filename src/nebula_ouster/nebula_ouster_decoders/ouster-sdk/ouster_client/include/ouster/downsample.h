 ...
 */
template <
  typename DerivedPts, typename DerivedAttrib, typename DerivedOutPts, typename DerivedOutAttrib>
void voxel_downsample(
  const Eigen::Matrix<typename DerivedPts::Scalar, 3, 1> & voxel_size3, const DerivedPts & pts,
  const DerivedAttrib & attribs, DerivedOutPts & out_pts, DerivedOutAttrib & out_attribs,
  int min_pts_per_voxel = 1)
{
  ...
  if (has_attribute && attribs.rows() != pts.rows()) {
    throw std::invalid_argument(
      "Invalid number of attributes (" + std::to_string(attribs.rows()) +
      "). Must match number of input points (" + std::to_string(pts.rows()) + ") or be 0.");
  }
  ...
  std::unordered_map<
    Eigen::Vector3i, AccPointT, ouster::sdk::core::impl::hash_eigen<Eigen::Vector3i>>
    voxel_grid;
...
template <
  typename DerivedPts, typename DerivedAttrib, typename DerivedOutPts, typename DerivedOutAttrib>
void voxel_downsample(
  typename DerivedPts::Scalar voxel_size, const DerivedPts & pts, const DerivedAttrib & attribs,
  DerivedOutPts & out_pts, DerivedOutAttrib & out_attribs, int min_pts_per_voxel = 1)
{
  ...
  voxel_downsample(voxel_size3, pts, attribs, out_pts, out_attribs, min_pts_per_voxel);
}
}  // namespace core
}  // namespace sdk
}  // namespace ouster
