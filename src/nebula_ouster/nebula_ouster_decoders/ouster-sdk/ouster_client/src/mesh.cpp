...
std::pair<Coord, float> compute_bounding_sphere(const std::vector<Triangle> & triangles)
{
  ...
  for (auto & tri : triangles) {
    radius_squared = std::max(
      {(tri.coords[0] - centroid).squaredNorm(), (tri.coords[1] - centroid).squaredNorm(),
...
bool load_from_stl_ascii_facet(std::istream & stl_file, Triangle & out_triangle)
{
  ...
  if (
    !read_stl_ascii_line(stl_file, line) ||
  ...
  for (int i = 0; i < 3; ++i) {
    if (
      !read_stl_ascii_line(stl_file, line) || !std::regex_search(line, match, vertex_re) ||
      match.size() != 4) {
...
bool Mesh::load_from_stl_binary(std::istream & input_stream)
{
  ...
  for (uint32_t i = 0; i < n_tris; i++) {
    ...
    tris.emplace_back(Coord(in_tri.v0.data()), Coord(in_tri.v1.data()), Coord(in_tri.v2.data()));
...
bool Mesh::load_from_stl_stream(std::istream & input_stream)
{
  ...
  std::transform(zs.begin(), zs.end(), zs.begin(), [](unsigned char character) {
    return std::tolower(character);
  });
