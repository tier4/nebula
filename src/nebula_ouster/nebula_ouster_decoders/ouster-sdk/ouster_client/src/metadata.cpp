...
protected:
  ...
  void parse_and_validate_sensor_info(SensorInfo & sensor_info)
  {
    parse_and_validate_datetime(
      issues_.information, "$.sensor_info.build_date", "%Y-%m-%dT%TZ", sensor_info.build_date);

    parse_and_validate_item(
      issues_.information, "$.sensor_info.build_rev", sensor_info.fw_rev, verify_string_not_empty);

    parse_and_validate_item(
      issues_.information, "$.sensor_info.image_rev", sensor_info.image_rev,
      verify_string_not_empty);

    parse_and_validate_item(
      issues_.information, "$.sensor_info.initialization_id", sensor_info.init_id);

    if (parse_and_validate_item(issues_.information, prod_line_string_, sensor_info.prod_line)) {
      try {
    ...
    parse_and_validate_item(
      issues_.information, "$.sensor_info.prod_pn", sensor_info.prod_pn, verify_string_not_empty);

    std::string sn_string;
    parse_and_validate_item(
      issues_.information, "$.sensor_info.prod_sn", sn_string, verify_string_not_empty);
    char * end = nullptr;
    ...
    if (end != expected_end) {
      ...
      auto entry = ValidatorIssues::ValidatorEntry("$.sensor_info.prod_sn", error_message.str());
      issues_.information.push_back(entry);
    }

    parse_and_validate_item(
      issues_.information, "$.sensor_info.status", sensor_info.status, verify_string_not_empty);
  }
  ...
  void parse_and_validate_config_params(SensorConfig & config)
  {
    ...
    if (
      parse_and_validate_item(
        issues_.information, "$.config_params.azimuth_window.*", azimuth_window_data, 2,
        make_verify_in_bounds<uint64_t>(0, 360000))) {
      config.azimuth_window = {azimuth_window_data[0], azimuth_window_data[1]};
    }

    parse_and_validate_item(
      issues_.information, "$.config_params.lidar_frame_azimuth_offset",
      config.lidar_frame_azimuth_offset, make_verify_in_bounds<uint64_t>(0, 360000));

    parse_and_validate_item(
      issues_.information, "$.config_params.columns_per_packet", config.columns_per_packet);

    if (
      parse_and_validate_enum<std::string>(
        issues_.information, lidar_mode_string_, config.lidar_mode, lidar_mode_of_string)) {
      have_lidar_mode_ = true;
    }

    parse_and_validate_enum<std::string>(
      issues_.information, "$.config_params.multipurpose_io_mode", config.multipurpose_io_mode,
      multipurpose_io_mode_of_string);

    parse_and_validate_enum<std::string>(
      issues_.information, "$.config_params.nmea_baud_rate", config.nmea_baud_rate,
      nmea_baud_rate_of_string);

    uint64_t nmea_ignore_valid_char = 0;
    if (
      parse_and_validate_item(
        issues_.information, "$.config_params.nmea_ignore_valid_char", nmea_ignore_valid_char)) {
      config.nmea_ignore_valid_char = (nmea_ignore_valid_char != 0);
    }

    parse_and_validate_enum<std::string>(
      issues_.information, "$.config_params.nmea_in_polarity", config.nmea_in_polarity,
      polarity_of_string);

    parse_and_validate_item(
      issues_.information, "$.config_params.nmea_leap_seconds", config.nmea_leap_seconds, true);

    const std::string operating_mode_string = "$.config_params.operating_mode";
    if (!parse_and_validate_enum<std::string>(
          issues_.information, operating_mode_string, config.operating_mode,
      ...
      std::string auto_start_flag_deprecation =
        ...
        "appropriately...";
      if (
        parse_and_validate_item<bool>(
          issues_.information, auto_start_flag_string, auto_start_flag)) {
        auto entry =
          ValidatorIssues::ValidatorEntry(auto_start_flag_string, auto_start_flag_deprecation);
        issues_.information.push_back(entry);
        config.operating_mode = auto_start_flag ? OperatingMode::NORMAL : OperatingMode::STANDBY;
      } else if (
        parse_and_validate_item<int>(
          issues_.information, auto_start_flag_string, auto_start_int, true)) {
        auto entry =
          ValidatorIssues::ValidatorEntry(auto_start_flag_string, auto_start_flag_deprecation);
        issues_.information.push_back(entry);
    ...
    parse_and_validate_item(
      issues_.information, "$.config_params.phase_lock_enable", config.phase_lock_enable);

    parse_and_validate_item(
      issues_.information, "$.config_params.phase_lock_offset", config.phase_lock_offset, true);

    const std::string signal_multiplier_string = "$.config_params.signal_multiplier";
    if (
      parse_and_validate_item(
        issues_.information, signal_multiplier_string, config.signal_multiplier, true)) {
      try {
    ...
    parse_and_validate_enum<std::string>(
      issues_.information, "$.config_params.sync_pulse_in_polarity", config.sync_pulse_in_polarity,
      polarity_of_string);

    parse_and_validate_item(
      issues_.information, "$.config_params.sync_pulse_out_angle", config.sync_pulse_out_angle,
      make_verify_in_bounds<int>(0, 360), true);

    parse_and_validate_item(
      issues_.information, "$.config_params.sync_pulse_out_frequency",
    ...
    parse_and_validate_item(
      issues_.information, "$.config_params.sync_pulse_out_pulse_width",
      config.sync_pulse_out_pulse_width, true);

    parse_and_validate_enum<std::string>(
      issues_.information, "$.config_params.timestamp_mode", config.timestamp_mode,
      timestamp_mode_of_string);

    if (!parse_and_validate_item(
          issues_.information, "$.config_params.udp_dest", config.udp_dest)) {
      parse_and_validate_item(issues_.information, "$.config_params.udp_ip", config.udp_dest);
    }

    parse_and_validate_item(issues_.information, "$.config_params.udp_dest_zm", config.udp_dest_zm);

    parse_and_validate_item(
      issues_.information, "$.config_params.udp_port_imu", config.udp_port_imu,
      make_verify_in_bounds<uint16_t>(0, 65535));

    parse_and_validate_item(
      issues_.information, "$.config_params.udp_port_lidar", config.udp_port_lidar,
      make_verify_in_bounds<uint16_t>(0, 65535));

    parse_and_validate_item(
      issues_.information, "$.config_params.udp_port_zm", config.udp_port_zm,
      make_verify_in_bounds<uint16_t>(0, 65535));

    parse_and_validate_item(
      issues_.information, "$.config_params.udp_multicast_ttl", config.udp_multicast_ttl);

    parse_and_validate_item(
      issues_.information, "$.config_params.udp_multicast_ttl_zm", config.udp_multicast_ttl_zm);

    parse_and_validate_enum<std::string>(
      issues_.information, "$.config_params.udp_profile_imu", config.udp_profile_imu,
      udp_profile_imu_of_string);

    parse_and_validate_enum<std::string>(
      issues_.information, "$.config_params.udp_profile_lidar", config.udp_profile_lidar,
      udp_profile_lidar_of_string);

    parse_and_validate_enum<std::string>(
      issues_.information, "$.config_params.header_type", config.header_type,
      udp_profile_type_of_string);
    ...
    parse_and_validate_enum<std::string>(
      issues_.information, "$.config_params.gyro_fsr", config.gyro_fsr, full_scale_range_of_string);
    ...
    parse_and_validate_enum<std::string>(
      issues_.information, "$.config_params.return_order", config.return_order,
      return_order_of_string);

    parse_and_validate_item(
      issues_.information, "$.config_params.min_range_threshold_cm", config.min_range_threshold_cm);

    parse_and_validate_item(
      issues_.information, "$.config_params.imu_packets_per_frame", config.imu_packets_per_frame);
  ...
  bool internal_parse_and_validate_columns_per_frame(SensorInfo & sensor_info)
  {
    ...
    if (
      parse_and_validate_item<uint32_t>(
        issues_.information, columns_per_frame_string_, sensor_info.format.columns_per_frame)) {
      if (have_lidar_mode_) {
        // lidar mode is present, check that number of columns actually
        // matches config value
        auto temp_cols = n_cols_of_lidar_mode(*sensor_info.config.lidar_mode);
        if (sensor_info.format.columns_per_frame != temp_cols) {
          // found a misconfiguration
          std::stringstream error_message;
          error_message << columns_per_frame_string_ << "(" << sensor_info.format.columns_per_frame
                        << ") does not match " << lidar_mode_string_ << "(" << temp_cols << ")";

          auto entry =
            ValidatorIssues::ValidatorEntry(columns_per_frame_string_, error_message.str());
          issues_.warning.push_back(entry);
        } else {
        ...
        // nothing to match
        skipped_due_to_item(
          issues_.information, columns_per_frame_string_, lidar_mode_string_,
          "Lidar mode not found, can't verify columns per frame");
      }
    } else {
      // need either lidar mode or columns_per_frame
      if (!have_lidar_mode_) {
        auto entry = ValidatorIssues::ValidatorEntry(columns_per_frame_string_, "Missing field");
        issues_.critical.push_back(entry);
      }
  ...
  void internal_parse_and_validate_pixels_per_column(SensorInfo & sensor_info)
  {
    if (
      parse_and_validate_item(
        issues_.information, pixels_per_column_string_, sensor_info.format.pixels_per_column)) {
      if (have_prod_line_) {
        ...
        if (
          ...
          // found a misconfiguration
          std::stringstream error_message;
          error_message << pixels_per_column_string_ << "(" << sensor_info.format.columns_per_frame
                        << ") does not match " << prod_line_string_ << "("
                        << temp_prod_line.beam_count << ") details";
        ...
        // nothing to match
        skipped_due_to_item(issues_.information, pixels_per_column_string_, prod_line_string_);
      }
    } else {
      if (have_prod_line_) {
  ...
  void parse_and_validate_data_format(SensorInfo & sensor_info)
  {
    ...
    if (!columns_per_frame_success) {
      skipped_due_to_item(
        issues_.information, column_window_string, columns_per_frame_string_,
        "Columns per frame not found, can't verify "
        "numeric bounds on column window");
    }
    auto column_window_callback =
      [&](ValidatorIssues::EntryList & severity, const std::string & path, uint16_t data) {
        if (columns_per_frame_success) {
    ...
    if (
      parse_and_validate_item(
        issues_.information, column_window_string, column_window_data, 2, column_window_callback)) {
      sensor_info.format.column_window =
        std::make_pair(column_window_data[0], column_window_data[1]);
    }

    parse_and_validate_item(
      issues_.information, "$.lidar_data_format.columns_per_packet",
    ...
    if (have_prod_line_) {
      ...
      skipped_due_to_item(
        issues_.information, pixel_shift_string + ".length()", prod_line_string_,
        "Product line not found, can't verify size of "
        "pixel shift array");
    }
    parse_and_validate_item<int>(
      issues_.information, pixel_shift_string, sensor_info.format.pixel_shift_by_row, verify_count);
    ...
    if (!parse_and_validate_enum<std::string>(
          issues_.information, "$.lidar_data_format.header_type", sensor_info.format.header_type,
          udp_profile_type_of_string)) {
      // default_data_format defaults to HeaderType::STANDARD,
      // check if we need to switch it to FUSA
      if (sensor_info.format.udp_profile_lidar == UDPProfileLidar::FUSA_RNG15_RFL8_NIR8_DUAL) {
        sensor_info.format.header_type = HeaderType::FUSA;
      }
    }

    parse_and_validate_item(
      issues_.information, "$.imu_data_format.imu_packets_per_frame",
      sensor_info.format.imu_packets_per_frame);

    parse_and_validate_item(
      issues_.information, "$.imu_data_format.imu_measurements_per_packet",
    ...
    if (!parse_and_validate_item(issues_.information, fps_string, sensor_info.format.fps)) {
      if (have_lidar_mode_) {
        sensor_info.format.fps = frequency_of_lidar_mode(*sensor_info.config.lidar_mode);
    ...
    if (
      sensor_info.config.udp_port_lidar.value_or(7504) == 0 ||
  ...
  {
    parse_and_validate_datetime(
      issues_.information, "$.calibration_status.reflectivity.timestamp", "%Y-%m-%dT%T",
      sensor_info.cal.reflectivity_timestamp);

    if (!parse_and_validate_item(
          issues_.information, "$.calibration_status.reflectivity.valid",
   ...
   */
  void parse_and_validate_angles(
    const std::string & path, std::vector<double> & output, size_t width, size_t height)
  {
    jsoncons::json value_array = jsoncons::jsonpath::json_query(root_, path);
    ...
    for (const auto & val : angles.array_range()) {
      if (val.is_array()) {
        ...
        if (count != height) {
          auto entry = ValidatorIssues::ValidatorEntry(
            path, "Each beam angle sub-array must have " + std::to_string(height) + " elements.");
          issues_.critical.push_back(entry);
          return;
        }
  ...
  void parse_and_validate_intrinsics(SensorInfo & sensor_info)
  {
    ...
    if (
      parse_and_validate_item<double>(
        issues_.information, imu_intrinsics_string, imu_intrinsics_data, 16, true)) {
    ...
    const std::string lidar_intrinsics_string = "$.lidar_intrinsics.lidar_to_sensor_transform.*";

    if (
      parse_and_validate_item<double>(
        issues_.information, lidar_intrinsics_string, lidar_intrinsics_data, 16, true)) {
    ...
    if (!parse_and_validate_item(
          issues_.information, lidar_origin_string, sensor_info.lidar_origin_to_beam_origin_mm)) {
    ...
    if (
      parse_and_validate_item<double>(
        issues_.information, beam_to_lidar_string, beam_to_lidar_data, 16, true)) {
  ...
  void parse_and_validate_misc(SensorInfo & sensor_info)
  {
    ...
    if (
      parse_and_validate_item<double>(
        issues_.information, extrinsic_string, extrinsic_data, 16, true)) {
...
bool is_new_format(const jsoncons::json & root)
{
  ...
  if (nonlegacy_fields_present > 0 && nonlegacy_fields_present < NONLEGACY_METADATA_FIELDS.size()) {
...
jsoncons::json convert_legacy_to_nonlegacy(const jsoncons::json & root)
{
  ...
  if (root.contains("lidar_to_sensor_transform")) {
    result["lidar_intrinsics"]["lidar_to_sensor_transform"] = root.at("lidar_to_sensor_transform");
...
bool parse_and_validate_metadata(
  const jsoncons::json & root, SensorInfo & sensor_info, ValidatorIssues & issues)
{
  size_t nonlegacy_fields_present = 0;
  ...
  for (const auto & field_pair : NONLEGACY_METADATA_FIELDS) {
    if (root.contains(field_pair.first)) {
      ...
      auto entry = ValidatorIssues::ValidatorEntry(
        "$." + field_pair.first, "Non-legacy metadata must include field");
...
bool parse_and_validate_metadata(
  const std::string & json_data, SensorInfo & sensor_info, ValidatorIssues & issues)
{
...
bool parse_and_validate_config(
  const std::string & json_data, SensorConfig & config_out, ValidatorIssues & issues)
{
  jsoncons::json root;
...
bool parse_and_validate_config(const std::string & json_data, SensorConfig & sensor_config)
{
  ValidatorIssues issues;
