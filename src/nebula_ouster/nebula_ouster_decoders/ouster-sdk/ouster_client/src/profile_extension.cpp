...
// lidar_scan.cpp
extern Table<UDPProfileLidar, DefaultFieldsEntry, MAX_NUM_PROFILES> default_scan_fields;

static void extend_default_scan_fields(
  UDPProfileLidar profile, const std::vector<std::pair<std::string, ChanFieldType>> & scan_fields)
{
  auto end = impl::default_scan_fields.end();
  auto it = std::find_if(impl::default_scan_fields.begin(), end, [](const auto & key_value) {
...
static void extend_profile_entries(
  UDPProfileLidar profile, const std::vector<std::pair<std::string, FieldInfo>> & fields,
  size_t chan_data_size)
{
...
void add_custom_profile(
  ...
  const std::string & name, const std::vector<std::pair<std::string, impl::FieldInfo>> & fields,
  size_t chan_data_size)
{
  if (profile_nr == 0) {
