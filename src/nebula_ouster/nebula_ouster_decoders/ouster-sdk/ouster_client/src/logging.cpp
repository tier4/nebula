...
#define LOGGER_PROCESS_ARG(SINGLE_LOGGER_PROCESS_ARG_type) \
  template void Logger::process_arg(                       \
    std::shared_ptr<Logger::format_builder> builder, SINGLE_LOGGER_PROCESS_ARG_type data);
...
template <typename T>
void Logger::process_arg(std::shared_ptr<Logger::format_builder> builder, T data)
{
