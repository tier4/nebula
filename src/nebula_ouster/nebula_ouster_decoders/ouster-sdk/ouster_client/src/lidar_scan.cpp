...
struct ZeroFieldCols
{
  template <typename T>
  void operator()(
    Eigen::Ref<img_t<T>> field, const std::string & /*unused*/, std::ptrdiff_t start,
...
struct ParseFieldCol
{
  template <typename T>
  void operator()(
    Eigen::Ref<img_t<T>> field, const std::string & field_name, uint16_t m_id,
    const PacketFormat & packet_format, const uint8_t * col_buf) const
  {
    ...
    packet_format.col_field(col_buf, field_name, field.col(m_id).data(), field.cols());
  }
};
  ...
  for (int icol = 0; icol < pf.columns_per_packet; icol++) {
    ...
    if (raw_headers) {
      ...
      if (m_id >= next_headers_m_id_) {
        impl::visit_field(
          lidar_scan, ChanField::RAW_HEADERS, ZeroFieldCols{}, "", next_headers_m_id_, m_id);
    ...
    if (m_id >= next_valid_m_id_) {
      impl::foreach_channel_field(lidar_scan, pf, ZeroFieldCols{}, next_valid_m_id_, m_id);
    ...
    impl::foreach_channel_field(lidar_scan, pf, ParseFieldCol{}, m_id, pf, col_buf);
  }
}
...
struct ParseFieldBlock
{
  template <typename T>
  void operator()(
    Eigen::Ref<img_t<T>> field, const std::string & field_name, const PacketFormat & packet_format,
...
void ScanBatcher::parse_by_block(const uint8_t * packet_buf, LidarScan & lidar_scan)
{
  ...
  if (first_m_id >= next_valid_m_id_) {
    impl::foreach_channel_field(lidar_scan, pf, ZeroFieldCols{}, next_valid_m_id_, first_m_id);
  ...
  switch (pf.block_parsable()) {
    case 16:
      impl::foreach_channel_field(lidar_scan, pf, ParseFieldBlock<16>{}, pf, packet_buf);
      break;
    case 8:
      impl::foreach_channel_field(lidar_scan, pf, ParseFieldBlock<8>{}, pf, packet_buf);
      break;
    case 4:
      impl::foreach_channel_field(lidar_scan, pf, ParseFieldBlock<4>{}, pf, packet_buf);
      break;
    default:
...
void ScanBatcher::finalize_scan(LidarScan & lidar_scan)
{
  if (next_valid_m_id_ < w_) {
    impl::foreach_channel_field(lidar_scan, pf, ZeroFieldCols{}, next_valid_m_id_, w_);
  }

  if (impl::raw_headers_enabled(pf, lidar_scan)) {
    impl::visit_field(
      lidar_scan, ChanField::RAW_HEADERS, ZeroFieldCols{}, "", next_headers_m_id_, w_);
  }
