...
bool Zone::render(const BeamConfig & config)
{
  ...
  zrb = Zrb(
    config.n_rows, config.n_cols, config.m_per_zmbin, config.serial_number, stl->hash(),
    config.beam_to_lidar_transform, config.lidar_to_sensor_transform,
    config.sensor_to_body_transform);
  ...
  if (pixels_with_intersections && pixels_with_intersections < point_count) {
    throw std::logic_error(
      "Zone: area of rendered zone (" + std::to_string(pixels_with_intersections) +
      ") is smaller than "
      "point_count (" +
