...
class So2:
    ...
    def calc_Dx_this_mul_exp_x_at_0(self, x):
        return sympy.Matrix(2, 1, lambda r, c: sympy.diff((self * So2.exp(x))[r], x)).limit(x, 0)
    ...
    def calc_Dx_exp_x_matrix_at_0(x):
        return sympy.Matrix(2, 2, lambda r, c: sympy.diff(So2.exp(x).matrix()[r, c], x)).limit(x, 0)
...
class TestSo2(unittest.TestCase):
    ...
    def test_derivatives(self):
        ...
        for i in [0, 1]:
            self.assertEqual(
                sympy.simplify(So2.calc_Dxi_x_matrix(self.a, i) - So2.Dxi_x_matrix(self.a, i)),
                sympy.Matrix.zeros(2, 2),
            )

        self.assertEqual(
            sympy.simplify(So2.Dx_exp_x_matrix(self.theta) - So2.calc_Dx_exp_x_matrix(self.theta)),
            sympy.Matrix.zeros(2, 2),
        )
        self.assertEqual(
            sympy.simplify(So2.Dx_exp_x_matrix_at_0() - So2.calc_Dx_exp_x_matrix_at_0(self.theta)),
            sympy.Matrix.zeros(2, 2),
        )
