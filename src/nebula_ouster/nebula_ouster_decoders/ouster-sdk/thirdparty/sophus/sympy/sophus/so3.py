...
class So3:
    ...
    def exp(v):
        ...
        return So3(Quaternion(sympy.cos(0.5 * theta), sympy.sin(0.5 * theta) / theta * v))
    ...
    def calc_Dx_log_exp_x_times_this_at_0(self, x):
        return (
            sympy.Matrix(3, 3, lambda r, c: sympy.diff((So3.exp(x) * self).log()[r], x[c]))
            .subs(x[0], 0)
            .subs(x[1], 0)
            .limit(x[2], 0)
        )
    ...
    @staticmethod
    def Dx_exp_x_at_0():
        return sympy.Matrix([[0.5, 0.0, 0.0], [0.0, 0.5, 0.0], [0.0, 0.0, 0.5], [0.0, 0.0, 0.0]])
    ...
    def Dxi_x_matrix(x, i):
        ...
        if i == 3:
            return sympy.Matrix(
                [[0, -2 * x[2], 2 * x[1]], [2 * x[2], 0, -2 * x[0]], [-2 * x[1], 2 * x[0], 0]]
            )
    ...
    def calc_Dxi_exp_x_matrix(x, i):
        return sympy.Matrix(3, 3, lambda r, c: sympy.diff(So3.exp(x).matrix()[r, c], x[i]))
...
class TestSo3(unittest.TestCase):
    def setUp(self):
        omega0, omega1, omega2 = sympy.symbols("omega[0], omega[1], omega[2]", real=True)
    ...
    def test_exp_log(self):
        for o in [Vector3(0.0, 1, 0.5), Vector3(0.1, 0.1, 0.1), Vector3(0.01, 0.2, 0.03)]:
            w = So3.exp(o).log()
            for i in range(0, 3):
    ...
    def test_derivatives(self):
        ...
        for i in [0, 1, 2, 3]:
            self.assertEqual(
                sympy.simplify(So3.calc_Dxi_x_matrix(self.a, i) - So3.Dxi_x_matrix(self.a, i)),
                sympy.Matrix.zeros(3, 3),
            )
        for i in [0, 1, 2]:
            self.assertEqual(
                sympy.simplify(
                    So3.Dxi_exp_x_matrix(self.omega, i) - So3.calc_Dxi_exp_x_matrix(self.omega, i)
                ),
                sympy.Matrix.zeros(3, 3),
            )
            self.assertEqual(
                sympy.simplify(
                    So3.Dxi_exp_x_matrix_at_0(i) - So3.calc_Dxi_exp_x_matrix_at_0(self.omega, i)
                ),
                sympy.Matrix.zeros(3, 3),
            )
