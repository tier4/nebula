...
class Se2:
    ...
    def calc_Dxi_exp_x_matrix(x, i):
        return sympy.Matrix(3, 3, lambda r, c: sympy.diff(Se2.exp(x).matrix()[r, c], x[i]))
...
class TestSe2(unittest.TestCase):
    def setUp(self):
        upsilon0, upsilon1, theta = sympy.symbols("upsilon[0], upsilon[1], theta", real=True)
    ...
    def test_exp_log(self):
        for v in [Vector3(0.0, 1, 0.5), Vector3(0.1, 0.1, 0.1), Vector3(0.01, 0.2, 0.03)]:
            w = Se2.exp(v).log()
            for i in range(0, 3):
    ...
    def test_derivatives(self):
        self.assertEqual(
            sympy.simplify(Se2.calc_Dx_exp_x_at_0(self.upsilon_theta) - Se2.Dx_exp_x_at_0()),
            sympy.Matrix.zeros(4, 3),
        )
        for i in range(0, 4):
            self.assertEqual(
                sympy.simplify(Se2.calc_Dxi_x_matrix(self.a, i) - Se2.Dxi_x_matrix(self.a, i)),
