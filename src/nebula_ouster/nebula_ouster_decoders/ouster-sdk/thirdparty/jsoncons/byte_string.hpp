...
typename std::enable_if<
  std::is_same<typename std::iterator_traits<InputIt>::value_type, uint8_t>::value, size_t>::type
...
decode_base64_generic(
  InputIt first, InputIt last, const uint8_t reverse_alphabet[256], F f, Container & result)
{
  uint8_t a4[4], a3[3];
  uint8_t i = 0;
  uint8_t j = 0;
...
decode_base64url(InputIt first, InputIt last, Container & result)
{
  ...
  auto retval = jsoncons::detail::decode_base64_generic(
    first, last, reverse_alphabet, is_base64url<typename std::iterator_traits<InputIt>::value_type>,
    result);
...
decode_base64(InputIt first, InputIt last, Container & result)
{
  static constexpr uint8_t reverse_alphabet[256] = {
    ...
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
  auto retval = jsoncons::detail::decode_base64_generic(
    first, last, reverse_alphabet, is_base64<typename std::iterator_traits<InputIt>::value_type>,
    result);
  return retval.ec == conv_errc::success ? retval
...
public:
  ...
  friend std::basic_ostream<CharT> & operator<<(
    std::basic_ostream<CharT> & os, const byte_string_view & bstr)
  {
...
public:
  ...
  basic_byte_string(std::initializer_list<uint8_t> init) : data_(std::move(init)) {}
  ...
  explicit basic_byte_string(const byte_string_view & v) : data_(v.begin(), v.end()) {}

  basic_byte_string(const basic_byte_string<Allocator> & v) : data_(v.data_) {}

  basic_byte_string(basic_byte_string<Allocator> && v) noexcept : data_(std::move(v.data_)) {}
  ...
  void append(const uint8_t * s, std::size_t count) { data_.insert(data_.end(), s, s + count); }
  ...
  template <class CharT>
  friend std::basic_ostream<CharT> & operator<<(
    std::basic_ostream<CharT> & os, const basic_byte_string & o)
  {
...
// ==
inline bool operator==(const byte_string_view & lhs, const byte_string_view & rhs) noexcept
{
...
template <class Allocator>
bool operator==(
  const basic_byte_string<Allocator> & lhs, const basic_byte_string<Allocator> & rhs) noexcept
{
  return rhs.compare(lhs) == 0;
}
