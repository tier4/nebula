...
public:
  ...
  basic_staj_event(
    staj_event_type event_type, std::size_t length, semantic_tag tag = semantic_tag::none)
  ...
  basic_staj_event(
    const string_view_type & s, staj_event_type event_type, semantic_tag tag = semantic_tag::none)
  ...
  basic_staj_event(
    const byte_string_view & s, staj_event_type event_type, semantic_tag tag = semantic_tag::none)
  : event_type_(event_type), tag_(tag), ext_tag_(0), length_(s.size())
  {
    value_.byte_string_data_ = s.data();
  }

  basic_staj_event(const byte_string_view & s, staj_event_type event_type, uint64_t ext_tag)
  ...
  typename std::enable_if<std::is_same<T, byte_string_view>::value, T>::type get_(
  ...
  typename std::enable_if<extension_traits::is_integer<IntegerType>::value, IntegerType>::type get_(
    Allocator, std::error_code & ec) const
  {
    switch (event_type_) {
      case staj_event_type::string_value: {
...
private:
  ...
  friend bool send_json_event(
    const basic_staj_event<CharT> & ev, basic_json_visitor<CharT> & visitor,
...
public:
  basic_staj_visitor()
  : pred_(accept), event_(staj_event_type::null_value), state_(), data_(), shape_(), index_(0)
  {
  }

  basic_staj_visitor(std::function<bool(const basic_staj_event<CharT> &, const ser_context &)> pred)
  : pred_(pred), event_(staj_event_type::null_value), state_(), data_(), shape_(), index_(0)
  {
  }
...
private:
  ...
  bool visit_key(
    const string_view_type & name, const ser_context & context, std::error_code &) override
  {
...
public:
  ...
  virtual void read_to(basic_json_visitor<CharT> & visitor, std::error_code & ec) = 0;
...
public:
  ...
  const basic_staj_event<CharT> & current() const override { return cursor_->current(); }

  void read_to(basic_json_visitor<CharT> & visitor) override { cursor_->read_to(visitor); }
