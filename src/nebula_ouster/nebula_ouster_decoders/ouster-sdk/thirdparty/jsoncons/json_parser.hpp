...
private:
  ...
  using char_allocator_type =
    typename std::allocator_traits<temp_allocator_type>::template rebind_alloc<CharT>;
  using parse_state_allocator_type =
    typename std::allocator_traits<temp_allocator_type>::template rebind_alloc<json_parse_state>;
...
public:
    ...
    state_stack_(temp_alloc)
  {
    ...
    if (options_.enable_str_to_inf()) {
      string_double_map_.emplace_back(
        options_.inf_to_str(), std::numeric_limits<double>::infinity());
    }
    if (options_.enable_str_to_neginf()) {
      string_double_map_.emplace_back(
        options_.neginf_to_str(), -std::numeric_limits<double>::infinity());
    }
  }
  ...
  void parse_false(basic_json_visitor<char_type> & visitor, std::error_code & ec)
  {
    ...
    if (JSONCONS_LIKELY(end_input_ - input_ptr_ >= 5)) {
      if (
        *(input_ptr_ + 1) == 'a' && *(input_ptr_ + 2) == 'l' && *(input_ptr_ + 3) == 's' &&
        *(input_ptr_ + 4) == 'e') {
        input_ptr_ += 5;
        position_ += 5;
        more_ = visitor.bool_value(false, semantic_tag::none, *this, ec);
  ...
  JSONCONS_DEPRECATED_MSG(
    "Instead, use finish_parse(basic_json_visitor<char_type>&, std::error_code&)")
  ...
  std::size_t column() const override { return (position_ - mark_position_) + 1; }
...
private:
  ...
  void end_negative_value(basic_json_visitor<char_type> & visitor, std::error_code & ec)
  {
    int64_t val;
    auto result =
      jsoncons::detail::to_integer_unchecked(string_buffer_.data(), string_buffer_.length(), val);
    if (result) {
  ...
  void end_string_value(
    const char_type * s, std::size_t length, basic_json_visitor<char_type> & visitor,
    std::error_code & ec)
  {
    ...
    switch (parent()) {
      ...
      case json_parse_state::array: {
        auto it = std::find_if(
          string_double_map_.begin(), string_double_map_.end(), string_maps_to_double{sv});
