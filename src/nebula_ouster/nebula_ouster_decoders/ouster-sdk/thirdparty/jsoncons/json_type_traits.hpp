...
struct json_type_traits
{
  ...
  static T as(const Json &) { static_assert(unimplemented<T>::value, "as not implemented"); }
...
using traits_can_convert_t = decltype(json_type_traits<Json, T>::can_convert(Json()));

template <class Json, class T>
using has_can_convert = extension_traits::is_detected<traits_can_convert_t, Json, T>;

template <class T>
struct invoke_can_convert
{
  template <class Json>
  static typename std::enable_if<has_can_convert<Json, T>::value, bool>::type can_convert(
    const Json & j) noexcept
  {
...
struct is_json_type_traits_unspecialized<
  ...
  typename std::enable_if<
    !std::integral_constant<bool, json_type_traits<Json, T>::is_compatible>::value>::type>
: std::true_type
{
};
...
struct is_compatible_array_type<
  Json, T,
  typename std::enable_if<
    !std::is_same<T, typename Json::array>::value && extension_traits::is_list_like<T>::value &&
    !is_json_type_traits_unspecialized<
      Json, typename std::iterator_traits<typename T::iterator>::value_type>::value>::type>
: std::true_type
{
};
...
struct is_json_type_traits_specialized<
  Json, T,
  typename std::enable_if<
    !jsoncons::detail::is_json_type_traits_unspecialized<Json, T>::value>::type> : std::true_type
{
};
...
struct json_type_traits<
  Json, T,
  typename std::enable_if<
    (extension_traits::is_signed_integer<T>::value && sizeof(T) <= sizeof(int64_t)) ||
    (extension_traits::is_unsigned_integer<T>::value && sizeof(T) <= sizeof(uint64_t))>::type>
{
  using allocator_type = typename Json::allocator_type;
  ...
  static Json to_json(T val, const allocator_type &) { return Json(val, semantic_tag::none); }
};
...
template <class Json, class T>
struct json_type_traits<Json, T, typename std::enable_if<std::is_floating_point<T>::value>::type>
{
  using allocator_type = typename Json::allocator_type;
...
struct json_type_traits<Json, typename Json::object>
{
  ...
  static Json to_json(const json_object & o) { return Json(o, semantic_tag::none); }
...
struct json_type_traits<Json, typename Json::array>
{
  ...
  static Json to_json(const json_array & a) { return Json(a, semantic_tag::none); }
...
struct json_type_traits<Json, bool>
{
  ...
  static Json to_json(bool val, const allocator_type &) { return Json(val, semantic_tag::none); }
};
...
struct json_type_traits<
  Json, T,
  typename std::enable_if<
    !is_json_type_traits_declared<T>::value && extension_traits::is_string<T>::value &&
...
struct json_type_traits<
  Json, T,
  typename std::enable_if<
    !is_json_type_traits_declared<T>::value && extension_traits::is_string_view<T>::value &&
    std::is_same<typename Json::char_type, typename T::value_type>::value>::type>
{
  using allocator_type = typename Json::allocator_type;
  ...
  static T as(const Json & j) { return T(j.as_string_view().data(), j.as_string_view().size()); }
    ...
    extension_traits::is_back_insertable<T>::value>::type>
{
  ...
  static typename std::enable_if<
    !extension_traits::is_byte<typename Container::value_type>::value, Container>::type
  as(const Json & j)
  {
    if (j.is_array()) {
      T result;
      visit_reserve_(
        typename std::integral_constant<bool, extension_traits::has_reserve<T>::value>::type(),
        result, j.size());
  ...
  template <class Container = T>
  static typename std::enable_if<
    !extension_traits::is_std_byte<typename Container::value_type>::value, Json>::type
  to_json(const T & val)
  {
  ...
  static void visit_reserve_(std::true_type, T & v, std::size_t size) { v.reserve(size); }
...
struct json_type_traits<
  ...
  typename std::enable_if<
    !is_json_type_traits_declared<T>::value &&
    jsoncons::detail::is_compatible_array_type<Json, T>::value &&
    !extension_traits::is_back_insertable<T>::value && !extension_traits::is_insertable<T>::value &&
...
struct json_type_traits<Json, std::array<E, N>>
{
  ...
  static Json to_json(const std::array<E, N> & val, const allocator_type & alloc)
  {
    Json j(json_array_arg, alloc);
    j.reserve(N);
    for (auto it = val.begin(); it != val.end(); ++it) {
...
struct json_type_traits<
  Json, T,
  typename std::enable_if<
    !is_json_type_traits_declared<T>::value && extension_traits::is_map_like<T>::value &&
    ...
    is_json_type_traits_specialized<Json, typename T::mapped_type>::value>::type>
{
  ...
  static T as(const Json & j)
  {
    ...
    for (const auto & item : j.object_range()) {
      result.emplace(
        key_type(item.key().data(), item.key().size()), item.value().template as<mapped_type>());
    }
...
private:
  using helper =
    tuple_detail::json_tuple_helper<sizeof...(E), sizeof...(E), Json, std::tuple<E...>>;
...
public:
  ...
  static Json to_json(const std::tuple<E...> & val, const allocator_type & alloc)
  {
    Json j(json_array_arg, alloc);
    j.reserve(sizeof...(E));
    helper::to_json(val, j);
...
public:
  ...
  static bool is(const Json & j) noexcept { return j.is_array() && j.size() == 2; }
...
template <class Json, class T>
struct json_type_traits<
  Json, T, typename std::enable_if<extension_traits::is_basic_byte_string<T>::value>::type>
{
public:
  using allocator_type = typename Json::allocator_type;
  ...
  static T as(const Json & j) { return j.template as_byte_string<typename T::allocator_type>(); }
...
{
  static bool is(const Json & j) noexcept { return j.is_null() || j.template is<ValueType>(); }

  static std::shared_ptr<ValueType> as(const Json & j)
  {
    return j.is_null() ? std::shared_ptr<ValueType>(nullptr)
    ...
    !std::is_polymorphic<ValueType>::value>::type>
{
  ...
  {
    return j.is_null() ? std::unique_ptr<ValueType>(nullptr)
...
template <class Json, class T>
struct json_type_traits<
  Json, jsoncons::optional<T>,
  typename std::enable_if<!is_json_type_traits_declared<jsoncons::optional<T>>::value>::type>
{
public:
  static bool is(const Json & j) noexcept { return j.is_null() || j.template is<T>(); }
...
typename std::enable_if<N == std::variant_size_v<Variant>, bool>::type is_variant(
  const Json & /*j*/)
{
  return false;
}
...
template <std::size_t N, class Json, class Variant, class T, class... U>
  typename std::enable_if < N<std::variant_size_v<Variant>, bool>::type is_variant(const Json & j)
{
  if (j.template is<T>()) {
    return true;
  } else {
...
template <int N, class Json, class Variant, class... Args>
typename std::enable_if<N == std::variant_size_v<Variant>, Variant>::type as_variant(
  const Json & /*j*/)
{
...
struct json_type_traits<Json, std::chrono::duration<Rep, Period>>
{
  ...
  {
    return (
      j.tag() == semantic_tag::epoch_second || j.tag() == semantic_tag::epoch_milli ||
  ...
  static typename std::enable_if<std::is_same<PeriodT, std::ratio<1>>::value, duration_type>::type
  ...
  template <class PeriodT = Period>
  static typename std::enable_if<std::is_same<PeriodT, std::milli>::value, duration_type>::type
  from_json_(const Json & j)
  {
  ...
  template <class PeriodT = Period>
  static typename std::enable_if<std::is_same<PeriodT, std::nano>::value, duration_type>::type
  from_json_(const Json & j)
  {
    if (j.is_int64() || j.is_uint64() || j.is_double()) {
      auto count = j.template as<Rep>();
  ...
  template <class PeriodT = Period>
  static typename std::enable_if<std::is_same<PeriodT, std::ratio<1>>::value, Json>::type to_json_(
    const duration_type & val)
  {
    return Json(val.count(), semantic_tag::epoch_second);
  }

  template <class PeriodT = Period>
  static typename std::enable_if<std::is_same<PeriodT, std::milli>::value, Json>::type to_json_(
    const duration_type & val)
  {
  ...
  template <class PeriodT = Period>
  static typename std::enable_if<std::is_same<PeriodT, std::nano>::value, Json>::type to_json_(
    const duration_type & val)
  {
...
struct json_type_traits<Json, std::bitset<N>>
{
  ...
  static Json to_json(const std::bitset<N> & val, const allocator_type & alloc = allocator_type())
  {
    std::vector<uint8_t> bits;
