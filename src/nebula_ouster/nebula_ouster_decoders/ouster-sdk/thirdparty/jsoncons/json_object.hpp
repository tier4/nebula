...
public:
  ...
  key_value(const key_value & member) : key_(member.key_), value_(member.value_) {}
...
// Sort keys
template <
  class KeyT, class Json, template <typename, typename> class SequenceContainer = std::vector>
...
public:
  ...
  : allocator_holder<allocator_type>(alloc), members_(key_value_allocator_type(alloc))
  {
  }

  sorted_json_object(const sorted_json_object & other)
  : allocator_holder<allocator_type>(other.get_allocator()), members_(other.members_)
  {
  }
  ...
  : allocator_holder<allocator_type>(other.get_allocator()), members_(std::move(other.members_))
  {
  }
  ...
  sorted_json_object(
    const std::initializer_list<std::pair<std::basic_string<char_type>, Json>> & init,
  ...
  void swap(sorted_json_object & other) noexcept { members_.swap(other.members_); }
  ...
  typename std::enable_if<
    !extension_traits::is_stateless<A>::value, std::pair<iterator, bool>>::type
  insert_or_assign(const string_view_type & name, T && value)
  {
    bool inserted;
    auto it = std::lower_bound(members_.begin(), members_.end(), name, Comp());
    if (it == members_.end()) {
      members_.emplace_back(
        key_type(name.begin(), name.end(), get_allocator()), std::forward<T>(value));
      inserted = true;
      it = members_.begin() + members_.size() - 1;
    } else if (it->key() == name) {
      ...
      it = members_.emplace(
        it, key_type(name.begin(), name.end(), get_allocator()), std::forward<T>(value));
      inserted = true;
    }
    return std::make_pair(it, inserted);
  }
  ...
  try_emplace(const string_view_type & name, Args &&... args)
  {
    ...
    if (it == members_.end()) {
      ...
      it = members_.emplace(it, key_type(name.begin(), name.end()), std::forward<Args>(args)...);
      inserted = true;
    }
    return std::make_pair(it, inserted);
  }
  ...
  try_emplace(const string_view_type & name, Args &&... args)
  {
    ...
    if (it == members_.end()) {
      members_.emplace_back(
        key_type(name.begin(), name.end(), get_allocator()), std::forward<Args>(args)...);
      it = members_.begin() + members_.size() - 1;
      inserted = true;
    } else if (it->key() == name) {
      inserted = false;
      ...
      it = members_.emplace(
        it, key_type(name.begin(), name.end(), get_allocator()), std::forward<Args>(args)...);
      inserted = true;
    }
    return std::make_pair(it, inserted);
  }
  ...
  template <class A = allocator_type, class... Args>
  typename std::enable_if<extension_traits::is_stateless<A>::value, iterator>::type try_emplace(
    iterator hint, const string_view_type & name, Args &&... args)
  {
    iterator it = hint;
  ...
  void merge(sorted_json_object && source)
  {
    ...
    for (; it != end; ++it) {
      auto pos = std::lower_bound(members_.begin(), members_.end(), (*it).key(), Comp());
      if (pos == members_.end()) {
  ...
  void merge(iterator hint, sorted_json_object && source)
  {
    ...
    for (; it != end; ++it) {
      ...
      if (hint != members_.end() && hint->key() <= (*it).key()) {
        ...
        pos = std::lower_bound(members_.begin(), members_.end(), (*it).key(), Comp());
      }
      if (pos == members_.end()) {
        members_.emplace_back(*it);
  ...
  bool operator==(const sorted_json_object & rhs) const { return members_ == rhs.members_; }
...
public:
  ...
  : allocator_holder<allocator_type>(val.get_allocator()), members_(val.members_)
  {
  }
  ...
  : allocator_holder<allocator_type>(val.get_allocator()), members_(std::move(val.members_))
  {
  }

  order_preserving_json_object(
    const order_preserving_json_object & val, const allocator_type & alloc)
  : allocator_holder<allocator_type>(alloc), members_(val.members_, key_value_allocator_type(alloc))
  {
  }
  ...
  void swap(order_preserving_json_object & other) noexcept { members_.swap(other.members_); }
  ...
  try_emplace(const string_view_type & key, Args &&... args)
  {
    ...
    if (it == members_.end()) {
      members_.emplace_back(
        key_type(key.begin(), key.end(), get_allocator()), std::forward<Args>(args)...);
  ...
  template <class A = allocator_type, class... Args>
  typename std::enable_if<extension_traits::is_stateless<A>::value, iterator>::type try_emplace(
    iterator hint, const string_view_type & key, Args &&... args)
  {
    if (hint == members_.end()) {
  ...
  bool operator<(const order_preserving_json_object & rhs) const { return members_ < rhs.members_; }
