...
class basic_null_istream : public std::basic_istream<CharT>
{
  ...
  public:
    ...
    int_type overflow(
      int_type ch = typename std::basic_streambuf<CharT>::traits_type::eof()) override
    {
      return ch;
    }
  } nb_;
...
public:
  ...
  basic_null_istream(basic_null_istream &&) noexcept : std::basic_istream<CharT>(&nb_) {}
  ...
  void fill_buffer()
  {
    ...
    JSONCONS_TRY
    {
      std::streamsize count =
        sbuf_->sgetn(reinterpret_cast<char_type *>(buffer_.data()), buffer_.size());
...
public:
  ...
  string_source(
    ...
    typename std::enable_if<
      extension_traits::is_sequence_of<Sourceable, value_type>::value>::type * = 0)
  ...
  std::size_t position() const { return (current_ - data_) / sizeof(value_type); }
...
public:
  iterator_source(
    const IteratorT & first, const IteratorT & last, std::size_t buf_size = default_max_buffer_size)
  ...
  typename std::enable_if<
    std::is_same<Category, std::random_access_iterator_tag>::value, std::size_t>::type
