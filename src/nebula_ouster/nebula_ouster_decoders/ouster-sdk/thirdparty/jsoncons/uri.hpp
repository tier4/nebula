...
public:
  ...
  uri(
    jsoncons::string_view scheme, jsoncons::string_view userinfo, jsoncons::string_view host,
    jsoncons::string_view port, jsoncons::string_view path, jsoncons::string_view query,
  ...
  const std::string & string() const { return uri_; }

  bool is_absolute() const noexcept { return scheme_.second > scheme_.first; }

  bool is_opaque() const noexcept { return is_absolute() && !authority().empty(); }
  ...
  friend bool operator==(const uri & lhs, const uri & rhs) { return lhs.compare(rhs) == 0; }
...
private:
  ...
  uri(
    const std::string & uri, part_type scheme, part_type userinfo, part_type host, part_type port,
    part_type path, part_type query, part_type fragment)
  : uri_(uri),
    scheme_(scheme),
    userinfo_(userinfo),
    host_(host),
