...
public:
  ...
  staj_array_iterator(staj_array_view<T, Json> & view) : view_(std::addressof(view))
  {
  ...
  {
    return (!a.view_ && !b.view_) || (!a.view_ && b.done()) || (!b.view_ && a.done());
  }
...
private:
  ...
  void next()
  {
    ...
    if (ec) {
      JSONCONS_THROW(
        ser_error(ec, view_->cursor_->context().line(), view_->cursor_->context().column()));
    }
  }
...
public:
  ...
  staj_object_iterator(staj_object_view<Key, T, Json> & view) : view_(std::addressof(view))
  {
  ...
  staj_object_iterator(staj_object_view<Key, T, Json> & view, std::error_code & ec)
  : view_(std::addressof(view))
  {
    if (view_->cursor_->current().event_type() == staj_event_type::begin_object) {
      next(ec);
...
private:
  ...
  {
    return view_->cursor_->done() ||
           view_->cursor_->current().event_type() == staj_event_type::end_object;
  }
      ...
      if (!done()) {
        ...
        JSONCONS_TRY
        {
          view_->key_value_ = value_type(
            std::move(key),
            decode_traits<T, char_type>::decode(*view_->cursor_, view_->decoder_, ec));
        }
        JSONCONS_CATCH(const conv_error &)
        {
          eptr_ = std::current_exception();
...
public:
  staj_array_view(basic_staj_cursor<char_type> & cursor) : cursor_(std::addressof(cursor)) {}
...
public:
  staj_object_view(basic_staj_cursor<char_type> & cursor) : cursor_(std::addressof(cursor)) {}
