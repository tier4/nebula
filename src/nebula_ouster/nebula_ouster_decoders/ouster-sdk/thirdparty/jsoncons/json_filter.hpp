...
public:
  basic_json_filter(basic_json_visitor<char_type> & visitor) : destination_(std::addressof(visitor))
  {
  }
...
private:
  ...
  bool visit_byte_string(
    const byte_string_view & b, uint64_t ext_tag, const ser_context & context,
    std::error_code & ec) override
  {
...
public:
  basic_rename_object_key_filter(
    const std::basic_string<CharT> & name, const std::basic_string<CharT> & new_name,
...
public:
  json_visitor_adaptor_base(To & visitor) : destination_(std::addressof(visitor)) {}
...
class json_visitor_adaptor<
  ...
  typename std::enable_if<
    extension_traits::is_narrow_character<typename From::char_type>::value &&
    extension_traits::is_narrow_character<typename To::char_type>::value>::type>
...
private:
  bool visit_key(
    const string_view_type & key, const ser_context & context, std::error_code & ec) override
  {
    return destination().key(
      string_view_type(reinterpret_cast<const to_char_type *>(key.data()), key.size()), context,
      ec);
  }
    ...
    std::error_code & ec) override
  {
    return destination().string_value(
      string_view_type(reinterpret_cast<const to_char_type *>(value.data()), value.size()), tag,
      context, ec);
  }
};
...
private:
    ...
    const string_view_type & name, const ser_context & context, std::error_code & ec) override
  {
    ...
    auto result =
      unicode_traits::convert(name.data(), name.size(), target, unicode_traits::conv_flags::strict);
    ...
    std::error_code & ec) override
  {
    ...
    auto result = unicode_traits::convert(
      value.data(), value.size(), target, unicode_traits::conv_flags::strict);
    if (result.ec != unicode_traits::conv_errc()) {
