...
public:
  ...
  using basic_type_allocator_type =
...
private:
  ...
  struct short_storage
  {
    ...
    short_storage() : is_dynamic_(false), is_negative_(false), length_(0), values_{0, 0} {}
    ...
    short_storage(
      T n, typename std::enable_if<
             std::is_integral<T>::value && sizeof(int64_t) < sizeof(T) &&
             std::is_signed<T>::value>::type * = 0)
    : is_dynamic_(false), is_negative_(n < 0), length_(n == 0 ? 0 : max_short_storage_size)
    {
      using unsigned_type = typename std::make_unsigned<T>::type;

      auto u =
        n < 0 ? (unsigned_type(0) - static_cast<unsigned_type>(n)) : static_cast<unsigned_type>(n);
    ...
    short_storage(
      T n, typename std::enable_if<
             std::is_integral<T>::value && sizeof(int64_t) < sizeof(T) &&
             !std::is_signed<T>::value>::type * = 0)
    : is_dynamic_(false), is_negative_(false), length_(n == 0 ? 0 : max_short_storage_size)
    {
    ...
    : is_dynamic_(false), is_negative_(stor.is_negative_), length_(stor.length_)
    {
  ...
  struct dynamic_storage
  {
    ...
    dynamic_storage()
    : is_dynamic_(true), is_negative_(false), length_(0), capacity_(0), data_(nullptr)
    {
    }
    ...
    constexpr size_type round_up(size_type i) const { return (i / word_length + 1) * word_length; }
  };
...
public:
  ...
  basic_bigint(basic_bigint<Allocator> && other) noexcept : base_t(other.get_allocator())
  {
    if (!other.is_dynamic()) {
  ...
  basic_bigint(Integer n, typename std::enable_if<std::is_integral<Integer>::value>::type * = 0)
  {
  ...
  template <class CharT>
  static basic_bigint<Allocator> from_string_radix(
    const CharT * data, size_type length, uint8_t radix)
  {
    if (!(radix >= 2 && radix <= 16)) {
    ...
    for (size_type i = 0; i < length; i++) {
      ...
      switch (c) {
        ...
        default:
          JSONCONS_THROW(
            std::runtime_error(
              std::string("Invalid digit in radix ") + std::to_string(radix) + ": \'" + (char)c +
              "\'"));
      }
      if (d >= radix) {
        JSONCONS_THROW(
  ...
  void resize(size_type new_length)
  {
    ...
    if (old_length < new_length) {
      if (is_dynamic()) {
        std::memset(
          dynamic_stor_.data_ + old_length, 0,
  ...
  {
    if (is_negative() != y.is_negative()) return *this += -y;
    if ((!is_negative() && y > *this) || (is_negative() && y < *this)) return *this = -(y - *this);
    uint64_t borrow = 0;
    uint64_t d;
    for (size_type i = 0; i < length(); i++) {
  ...
  basic_bigint & operator<<=(uint64_t k)
  {
    ...
    {
      resize(length() + q);
      for (size_type i = length(); i-- > 0;) data()[i] = (i < q ? 0 : data()[i - q]);
      k %= basic_type_bits;
    }
  ...
  basic_bigint & operator&=(const basic_bigint<Allocator> & a)
  {
    ...
    if (old_length > new_length) {
      if (is_dynamic()) {
        std::memset(
          dynamic_stor_.data_ + new_length, 0,
  ...
  friend bool operator==(
    const basic_bigint<Allocator> & x, const basic_bigint<Allocator> & y) noexcept
  {
  ...
  friend basic_bigint<Allocator> operator+(
    basic_bigint<Allocator> x, const basic_bigint<Allocator> & y)
  {
    return x += y;
  }

  friend basic_bigint<Allocator> operator+(basic_bigint<Allocator> x, int64_t y) { return x += y; }
  ...
  friend basic_bigint<Allocator> operator/(basic_bigint<Allocator> x, int y) { return x /= y; }
  ...
  friend basic_bigint<Allocator> operator<<(basic_bigint<Allocator> u, int k) { return u <<= k; }
  ...
  friend basic_bigint<Allocator> operator|(basic_bigint<Allocator> x, unsigned y) { return x |= y; }
  ...
  {
    if (is_negative() != y.is_negative()) return y.is_negative() - is_negative();
    int code = 0;
  ...
  void divide(
    basic_bigint<Allocator> denom, basic_bigint<Allocator> & quot, basic_bigint<Allocator> & rem,
    bool remDesired) const
  {
    if (denom.length() == 0) {
...
private:
  ...
  {
    uint64_t hiA = A >> basic_type_halfBits, loA = A & r_mask, hiB = B >> basic_type_halfBits,
             loB = B & r_mask, mid1, mid2, old;
  ...
  {
    uint64_t left, middle, right, qHi, qLo, x, dLo1, dHi = d >> basic_type_halfBits,
                                                     dLo = d & r_mask;
    qHi = A / (dHi + 1);
