...
struct encode_traits
{
  template <class Json>
  static void encode(
    const T & val, basic_json_visitor<CharT> & encoder, const Json & proto, std::error_code & ec)
  {
    encode(
      std::integral_constant<
        bool, extension_traits::is_stateless<typename Json::allocator_type>::value>(),
...
private:
  template <class Json>
  static void encode(
    std::true_type, const T & val, basic_json_visitor<CharT> & encoder, const Json & /*proto*/,
    std::error_code & ec)
  {
  ...
  template <class Json>
  static void encode(
    std::false_type, const T & val, basic_json_visitor<CharT> & encoder, const Json & proto,
    std::error_code & ec)
  {
...
struct encode_traits<T, CharT, typename std::enable_if<extension_traits::is_bool<T>::value>::type>
{
  template <class Json>
  static void encode(
    const T & val, basic_json_visitor<CharT> & encoder, const Json &, std::error_code & ec)
  {
...
template <class T, class CharT>
struct encode_traits<
  T, CharT, typename std::enable_if<extension_traits::is_u8_u16_u32_or_u64<T>::value>::type>
{
  template <class Json>
  static void encode(
    const T & val, basic_json_visitor<CharT> & encoder, const Json &, std::error_code & ec)
...
template <class T, class CharT>
struct encode_traits<
  T, CharT, typename std::enable_if<extension_traits::is_i8_i16_i32_or_i64<T>::value>::type>
{
  template <class Json>
  static void encode(
    const T & val, basic_json_visitor<CharT> & encoder, const Json &, std::error_code & ec)
...
template <class T, class CharT>
struct encode_traits<
  T, CharT, typename std::enable_if<extension_traits::is_float_or_double<T>::value>::type>
{
  template <class Json>
  static void encode(
    const T & val, basic_json_visitor<CharT> & encoder, const Json &, std::error_code & ec)
...
struct encode_traits<std::pair<T1, T2>, CharT>
{
  ...
  template <class Json>
  static void encode(
    const value_type & val, basic_json_visitor<CharT> & encoder, const Json & proto,
    std::error_code & ec)
  {
...
struct json_serialize_tuple_helper
{
  ...
  static void encode(
    const Tuple & tuple, basic_json_visitor<char_type> & encoder, const Json & proto,
    std::error_code & ec)
  {
...
struct json_serialize_tuple_helper<0, Size, Json, Tuple>
{
  using char_type = typename Json::char_type;
  static void encode(
    const Tuple &, basic_json_visitor<char_type> &, const Json &, std::error_code &)
  {
  }
};
}  // namespace detail
...
struct encode_traits<std::tuple<E...>, CharT>
{
  ...
  {
    using helper =
      jsoncons::detail::json_serialize_tuple_helper<size, size, Json, std::tuple<E...>>;
    ...
    extension_traits::is_typed_array<T>::value>::type>
{
  ...
  {
    encoder.typed_array(
      jsoncons::span<const value_type>(val), semantic_tag::none, ser_context(), ec);
  }
};
...
struct encode_traits<std::array<T, N>, CharT>
{
  ...
  template <class Json>
  static void encode(
    const std::array<T, N> & val, basic_json_visitor<CharT> & encoder, const Json & proto,
    std::error_code & ec)
  {
    encoder.begin_array(val.size(), semantic_tag::none, ser_context(), ec);
    if (ec) return;
