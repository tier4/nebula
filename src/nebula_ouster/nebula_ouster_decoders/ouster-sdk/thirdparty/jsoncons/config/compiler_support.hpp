...
#define JSONCONS_COPY(first, last, d_first) \
  std::copy(                                \
    first, last,                            \
    stdext::make_checked_array_iterator(    \
      d_first, static_cast<std::size_t>(std::distance(first, last))))
#else
...
{
public:
  assertion_error(const std::string & s) noexcept : std::runtime_error(s) {}
...
#ifdef _DEBUG
#define JSONCONS_ASSERT(x)                                                     \
  if (!(x)) {                                                                  \
    JSONCONS_THROW(                                                            \
      jsoncons::assertion_error(                                               \
        "assertion '" #x "' failed at " __FILE__ ":" JSONCONS_STR(__LINE__))); \
  }
#else
#define JSONCONS_ASSERT(x)                                                              \
  if (!(x)) {                                                                           \
    JSONCONS_THROW(                                                                     \
      jsoncons::assertion_error("assertion '" #x "' failed at  <> :" JSONCONS_STR(0))); \
  }
#endif  // _DEBUG
