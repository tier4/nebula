...
inline bool is_control_character(uint32_t c)
{
  return c <= 0x1F || c == 0x7f;
}

inline bool is_non_ascii_codepoint(uint32_t cp)
{
  return cp >= 0x80;
}

template <class CharT, class Sink>
std::size_t escape_string(
  const CharT * s, std::size_t length, bool escape_all_non_ascii, bool escape_solidus, Sink & sink)
{
  std::size_t count = 0;
  const CharT * begin = s;
...
template <
  class CharT, class Sink = jsoncons::stream_sink<CharT>, class Allocator = std::allocator<char>>
...
private:
  ...
  public:
    encoding_context(
      container_type type, line_split_kind split_lines, bool indent_once, std::size_t begin_pos,
    ...
    bool is_same_line() const { return line_splits_ == line_split_kind::same_line; }

    bool is_new_line() const { return line_splits_ == line_split_kind::new_line; }

    bool is_multi_line() const { return line_splits_ == line_split_kind::multi_line; }

    bool is_indent_once() const { return count_ == 0 ? indent_before_ : false; }
  };
  using encoding_context_allocator_type =
    typename std::allocator_traits<allocator_type>::template rebind_alloc<encoding_context>;
...
public:
  ...
  basic_json_encoder(
    Sink && sink, const basic_json_encode_options<CharT> & options,
...
private:
  ...
  bool visit_begin_object(semantic_tag, const ser_context &, std::error_code & ec) override
  {
    ...
    if (!stack_.empty())  // object or array
    {
      if (stack_.back().is_object()) {
        ...
        stack_.emplace_back(
          container_type::object, options_.object_object_line_splits(), false, column_,
      ...
      } else  // array
      {
        ...
        stack_.emplace_back(
          container_type::object, options_.array_object_line_splits(), false, column_,
          column_ + open_object_brace_str_.length());
      }
    } else {
      stack_.emplace_back(
        container_type::object, line_split_kind::multi_line, false, column_,
        column_ + open_object_brace_str_.length());
    }
    indent();
    ...
    if (!stack_.empty()) {
      if (stack_.back().is_object()) {
        switch (options_.object_array_line_splits()) {
          case line_split_kind::same_line:
            stack_.emplace_back(
              container_type::array, options_.object_array_line_splits(), false, column_,
              column_ + open_array_bracket_str_.length());
            break;
          case line_split_kind::new_line: {
            stack_.emplace_back(
              container_type::array, options_.object_array_line_splits(), true, column_,
              column_ + open_array_bracket_str_.length());
            break;
          }
          default:  // multi_line
      ...
      } else  // array
      {
        switch (options_.array_array_line_splits()) {
          case line_split_kind::same_line:
            ...
            stack_.emplace_back(
              container_type::array, options_.array_array_line_splits(), false, column_,
              column_ + open_array_bracket_str_.length());
            break;
          case line_split_kind::new_line:
            stack_.back().new_line_after(true);
      ...
      stack_.emplace_back(
        container_type::array, line_split_kind::multi_line, false, column_,
        column_ + open_array_bracket_str_.length());
    }
    indent();
  ...
  bool visit_key(const string_view_type & name, const ser_context &, std::error_code &) override
  {
    ...
    std::size_t length = jsoncons::detail::escape_string(
      name.data(), name.length(), options_.escape_all_non_ascii(), options_.escape_solidus(),
      sink_);
    sink_.push_back('\"');
  ...
  bool visit_string(
    const string_view_type & sv, semantic_tag tag, const ser_context &, std::error_code &) override
  {
    if (!stack_.empty()) {
      if (stack_.back().is_array()) {
        begin_scalar_value();
    ...
    switch (tag) {
      ...
      default: {
        sink_.push_back('\"');
        std::size_t length = jsoncons::detail::escape_string(
          sv.data(), sv.length(), options_.escape_all_non_ascii(), options_.escape_solidus(),
          sink_);
        sink_.push_back('\"');
        column_ += (length + 2);
        break;
  ...
  bool visit_byte_string(
    const byte_string_view & b, semantic_tag tag, const ser_context &, std::error_code &) override
  {
    if (!stack_.empty()) {
      if (stack_.back().is_array()) {
        begin_scalar_value();
    ...
    byte_string_chars_format format = jsoncons::detail::resolve_byte_string_chars_format(
      options_.byte_string_format(), encoding_hint, byte_string_chars_format::base64url);
    switch (format) {
      case byte_string_chars_format::base16: {
        sink_.push_back('\"');
  ...
  bool visit_double(
    double value, semantic_tag, const ser_context & context, std::error_code & ec) override
  {
    if (!stack_.empty()) {
      if (stack_.back().is_array()) {
        begin_scalar_value();
  ...
  bool visit_int64(int64_t value, semantic_tag, const ser_context &, std::error_code &) override
  {
    if (!stack_.empty()) {
      if (stack_.back().is_array()) {
        begin_scalar_value();
  ...
  bool visit_uint64(uint64_t value, semantic_tag, const ser_context &, std::error_code &) override
  {
    if (!stack_.empty()) {
      if (stack_.back().is_array()) {
        begin_scalar_value();
  ...
  void unindent() { indent_amount_ -= static_cast<int>(options_.indent_size()); }
...
typedef basic_compact_json_encoder<char, jsoncons::stream_sink<char>>
  json_compressed_stream_encoder;
JSONCONS_DEPRECATED_MSG("Instead, use compact_wjson_stream_encoder")
typedef basic_compact_json_encoder<wchar_t, jsoncons::stream_sink<wchar_t>>
  wjson_compressed_stream_encoder;
JSONCONS_DEPRECATED_MSG("Instead, use compact_json_string_encoder")
typedef basic_compact_json_encoder<char, jsoncons::string_sink<char>>
  json_compressed_string_encoder;
JSONCONS_DEPRECATED_MSG("Instead, use compact_wjson_string_encoder")
typedef basic_compact_json_encoder<wchar_t, jsoncons::string_sink<wchar_t>>
  wjson_compressed_string_encoder;
...
JSONCONS_DEPRECATED_MSG("Instead, use wjson_stream_encoder")
typedef basic_json_encoder<wchar_t, jsoncons::stream_sink<wchar_t>> wjson_serializer;

JSONCONS_DEPRECATED_MSG("Instead, use compact_json_stream_encoder")
typedef basic_compact_json_encoder<char, jsoncons::stream_sink<char>> json_compressed_serializer;
JSONCONS_DEPRECATED_MSG("Instead, use compact_wjson_stream_encoder")
typedef basic_compact_json_encoder<wchar_t, jsoncons::stream_sink<wchar_t>>
  wjson_compressed_serializer;

JSONCONS_DEPRECATED_MSG("Instead, use json_string_encoder")
typedef basic_json_encoder<char, jsoncons::string_sink<std::string>> json_string_serializer;
JSONCONS_DEPRECATED_MSG("Instead, use wjson_string_encoder")
typedef basic_json_encoder<wchar_t, jsoncons::string_sink<std::wstring>> wjson_string_serializer;

JSONCONS_DEPRECATED_MSG("Instead, use compact_json_string_encoder")
typedef basic_compact_json_encoder<char, jsoncons::string_sink<std::string>>
  json_compressed_string_serializer;
JSONCONS_DEPRECATED_MSG("Instead, use wcompact_json_string_encoder")
typedef basic_compact_json_encoder<wchar_t, jsoncons::string_sink<std::wstring>>
  wjson_compressed_string_serializer;
#endif
