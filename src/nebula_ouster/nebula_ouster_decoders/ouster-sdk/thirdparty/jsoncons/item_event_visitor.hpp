...
private:
  using char_allocator_type =
    typename std::allocator_traits<Allocator>::template rebind_alloc<char_type>;
  ...
  basic_item_event_visitor_to_json_visitor(const basic_item_event_visitor_to_json_visitor &) =
    delete;
...
public:
  ...
  explicit basic_item_event_visitor_to_json_visitor(
    basic_json_visitor<char_type> & visitor, const Allocator & alloc = Allocator())
  : destination_(std::addressof(visitor)), key_(alloc), key_buffer_(alloc), level_stack_(alloc)
  {
    level_stack_.emplace_back(target_t::destination, container_t::root);  // root
  }
  ...
  void destination(basic_json_visitor<char_type> & dest) { destination_ = std::addressof(dest); }
...
public:
  ...
  basic_default_item_event_visitor(bool accept_more = true, std::error_code ec = std::error_code())
...
private:
  ...
  bool visit_begin_object(
    std::size_t, semantic_tag, const ser_context &, std::error_code & ec) override
  {
    if (ec_) {
      ec = ec_;
    }
...
private:
  ...
  // noncopyable and nonmoveable
  basic_json_visitor_to_item_event_visitor(const basic_json_visitor_to_item_event_visitor &) =
    delete;
...
class diagnostics_visitor2 : public basic_default_item_event_visitor<char>
{
  ...
  bool visit_typed_array(
    const jsoncons::span<const uint16_t> & s, semantic_tag tag, const ser_context &,
  ...
  bool visit_typed_array(
    half_arg_t, const jsoncons::span<const uint16_t> & s, semantic_tag tag, const ser_context &,
    std::error_code &) override
  {
