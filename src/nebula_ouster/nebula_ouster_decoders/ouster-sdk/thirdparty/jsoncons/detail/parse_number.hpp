...
{
public:
  const char * name() const noexcept override { return "jsoncons/to_integer_unchecked"; }
...
struct to_integer_result
{
  const CharT * ptr;
  to_integer_errc ec;
  constexpr to_integer_result(const CharT * ptr_) : ptr(ptr_), ec(to_integer_errc()) {}
  constexpr to_integer_result(const CharT * ptr_, to_integer_errc ec_) : ptr(ptr_), ec(ec_) {}

  to_integer_result(const to_integer_result &) = default;

  to_integer_result & operator=(const to_integer_result &) = default;

  constexpr explicit operator bool() const noexcept { return ec == to_integer_errc(); }
...
typename std::enable_if<
  extension_traits::integer_limits<T>::is_specialized &&
    ...
    switch (state) {
      case integer_chars_state::initial: {
        switch (*s) {
          case '0':
            if (++s == end) {
              return (++s == end) ? to_integer_result<T, CharT>(s)
...
template <class T, class CharT>
typename std::enable_if<
  extension_traits::integer_limits<T>::is_specialized, to_integer_result<T, CharT>>::type
...
{
  static_assert(
    extension_traits::integer_limits<T>::is_specialized, "Integer type not specialized");
...
public:
  ...
  template <class CharT>
  typename std::enable_if<std::is_same<CharT, char>::value, double>::type operator()(
    const CharT * s, std::size_t len) const
  {
    double val = 0;
  ...
  template <class CharT>
  typename std::enable_if<std::is_same<CharT, wchar_t>::value, double>::type operator()(
    const CharT * s, std::size_t len) const
  {
...
public:
  ...
  template <class CharT>
  typename std::enable_if<std::is_same<CharT, char>::value, double>::type operator()(
    const CharT * s, std::size_t) const
  {
    CharT * end = nullptr;
...
public:
  ...
  chars_to(const chars_to &) { locale_ = newlocale(LC_ALL_MASK, "C", (locale_t)0); }
...
public:
  ...
  template <class CharT>
  typename std::enable_if<std::is_same<CharT, char>::value, double>::type operator()(
    const CharT * s, std::size_t /*length*/) const
  {
    CharT * end = nullptr;
