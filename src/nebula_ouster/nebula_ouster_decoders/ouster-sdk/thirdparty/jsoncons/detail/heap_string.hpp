...
struct heap_string_base
{
  ...
  heap_string_base(const Extra & extra, const Allocator & alloc) : extra_(extra), alloc_(alloc) {}
...
private:
  ...
  using heap_string_allocator_type =
    typename std::allocator_traits<Allocator>::template rebind_alloc<heap_string_type>;
...
public:
  ...
  typedef
    typename jsoncons_aligned_storage<sizeof(storage_t), alignof(storage_t)>::type storage_type;

  static size_t aligned_size(std::size_t n) { return sizeof(storage_type) + n; }

public:
  static pointer create(
    const char_type * s, std::size_t length, Extra extra, const Allocator & alloc)
  {
  ...
  static void destroy(pointer ptr)
  {
    if (ptr != nullptr) {
      ...
      std::size_t mem_size =
        (alignof(storage_type) - 1) + aligned_size(ptr->length_ * sizeof(char_type));
