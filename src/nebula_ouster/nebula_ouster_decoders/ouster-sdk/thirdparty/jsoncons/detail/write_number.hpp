...
template <class Result>
void prettify_string(
  const char * buffer, std::size_t length, int k, int min_exp, int max_exp, Result & result)
{
...
bool dtoa_general(double v, char decimal_point, Result & result, std::true_type)
{
  ...
  if (jsoncons::detail::grisu3(u, buffer, &length, &k)) {
    ...
    jsoncons::detail::prettify_string(
      buffer, length, k, -4, std::numeric_limits<double>::max_digits10, result);
    return true;
  } else {
...
bool dtoa_fixed(double v, char decimal_point, Result & result, std::true_type)
{
  ...
  if (jsoncons::detail::grisu3(u, buffer, &length, &k)) {
    ...
    jsoncons::detail::prettify_string(
      buffer, length, k, std::numeric_limits<int>::lowest(), (std::numeric_limits<int>::max)(),
      result);
    return true;
  } else {
