...
public:
  ...
  constexpr basic_string_view(const CharT * data, std::size_t length) : data_(data), length_(length)
  {
  }

  basic_string_view(const CharT * data) : data_(data), length_(Traits::length(data)) {}
  ...
  size_type find(basic_string_view s, size_type pos = 0) const noexcept
  {
    ...
    const_iterator it = std::search(cbegin() + pos, cend(), s.cbegin(), s.cend(), Traits::eq);
  ...
  size_type find_first_of(basic_string_view s, size_type pos = 0) const noexcept
  {
    ...
    const_iterator it =
      std::find_first_of(cbegin() + pos, cend(), s.cbegin(), s.cend(), Traits::eq);
    return it == cend() ? npos : std::distance(cbegin(), it);
  }
  ...
  size_type find_last_of(basic_string_view s, size_type pos = npos) const noexcept
  {
    ...
    const_reverse_iterator it =
      std::find_first_of(crbegin() + pos, crend(), s.cbegin(), s.cend(), Traits::eq);
  ...
  friend std::basic_ostream<CharT> & operator<<(
    std::basic_ostream<CharT> & os, const basic_string_view & sv)
  {
...
template <class CharT, class Traits, class Allocator>
bool operator==(
  const std::basic_string<CharT, Traits, Allocator> & lhs,
  const basic_string_view<CharT, Traits> & rhs) noexcept
{
  return rhs.compare(lhs) == 0;
}
template <class CharT, class Traits>
bool operator==(const basic_string_view<CharT, Traits> & lhs, const CharT * rhs) noexcept
{
  return lhs.compare(rhs) == 0;
}
template <class CharT, class Traits>
bool operator==(const CharT * lhs, const basic_string_view<CharT, Traits> & rhs) noexcept
{
  return rhs.compare(lhs) == 0;
}
