...
public:
  ...
  optional(
    ...
    typename std::enable_if<
      std::is_move_constructible<typename std::decay<T2>::type>::value>::type * = 0)
  ...
  template <class U>
  optional(
    optional<U> && value, typename std::enable_if<
  ...
  optional(
    T2 && value, typename std::enable_if<
                   !std::is_same<optional<T>, typename std::decay<T2>::type>::value &&
                     std::is_constructible<T, T2>::value && std::is_convertible<T2, T>::value,
                   int>::type = 0)  // (8)
  ...
  template <typename U>
  typename std::enable_if<
    !std::is_same<optional<T>, U>::value && std::is_constructible<T, const U &>::value &&
  ...
  template <typename U>
  typename std::enable_if<
    !std::is_same<optional<T>, U>::value && std::is_constructible<T, U>::value &&
      !is_constructible_convertible_or_assignable_from_optional<T, U>::value &&
  ...
  typename std::enable_if<
    !std::is_same<optional<T>, typename std::decay<T2>::type>::value &&
      std::is_constructible<T, T2>::value && std::is_assignable<T &, T2>::value &&
...
#pragma warning(disable : 4702)
  ...
  {
    static_assert(
      std::is_move_constructible<T>::value, "get_value_or: T must be move constructible");
    static_assert(std::is_convertible<U &&, T>::value, "get_value_or: U must be convertible to T");
...
template <typename T>
typename std::enable_if<std::is_nothrow_move_constructible<T>::value, void>::type swap(
  optional<T> & lhs, optional<T> & rhs) noexcept
{
  lhs.swap(rhs);
}
