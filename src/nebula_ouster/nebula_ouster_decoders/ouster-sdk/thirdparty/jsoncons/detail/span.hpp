...
public:
  ...
  span(element_type (&arr)[N]) noexcept : data_(std::addressof(arr[0])), size_(N)
  {
  }
  ...
  template <typename C>
  constexpr span(
    const C & c, typename std::enable_if<
                   !is_span<C>::value && !extension_traits::is_std_array<C>::value &&
                   extension_traits::is_compatible_element<C, element_type>::value &&
                   extension_traits::has_data_and_size<C>::value>::type * = 0)
  : data_(c.data()), size_(c.size())
  ...
  constexpr span(
    ...
    typename std::enable_if<
      ...
      std::is_convertible<U (*)[], element_type (*)[]>::value>::type * = 0) noexcept
  ...
  {
    // JSONCONS_ASSERT((offset <= size() && (count == dynamic_extent || (offset + count <=
    // size()))));
