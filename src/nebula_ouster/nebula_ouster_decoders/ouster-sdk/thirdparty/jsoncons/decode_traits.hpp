...
struct decode_traits
{
  ...
  static T decode(
    basic_staj_cursor<CharT> & cursor, json_decoder<Json, TempAllocator> & decoder,
    std::error_code & ec)
  {
    decoder.reset();
    ...
    if (ec) {
      ...
      JSONCONS_THROW(ser_error(
        conv_errc::conversion_failed, cursor.context().line(), cursor.context().column()));
    }
...
struct decode_traits<
  T, CharT, typename std::enable_if<extension_traits::is_primitive<T>::value>::type>
{
  template <class Json, class TempAllocator>
  static T decode(
    basic_staj_cursor<CharT> & cursor, json_decoder<Json, TempAllocator> &, std::error_code & ec)
  {
...
struct decode_traits<
  T, CharT,
  typename std::enable_if<
    !is_json_type_traits_declared<T>::value && extension_traits::is_list_like<T>::value &&
...
public:
  ...
  typed_array_visitor(T & v) : default_json_visitor(false, conv_errc::not_vector), v_(v), level_(0)
  {
  }
...
private:
  ...
  bool visit_begin_array(
    std::size_t size, semantic_tag, const ser_context &, std::error_code & ec) override
  {
    if (++level_ != 1) {
      ec = conv_errc::not_vector;
      return false;
    ...
    if (size > 0) {
      reserve_storage(
        typename std::integral_constant<bool, extension_traits::has_reserve<T>::value>::type(), v_,
        size);
    }
    return true;
  }
  ...
  bool visit_half(uint16_t value, semantic_tag, const ser_context &, std::error_code &) override
  {
    return visit_half_(
      typename std::integral_constant<bool, std::is_integral<value_type>::value>::type(), value);
  }
  ...
  bool visit_double(double value, semantic_tag, const ser_context &, std::error_code &) override
  {
    v_.push_back(static_cast<value_type>(value));
    return true;
  }
  ...
  bool visit_typed_array(
    const jsoncons::span<const value_type> & data, semantic_tag, const ser_context &,
  ...
  static void reserve_storage(std::true_type, T & v, std::size_t new_cap) { v.reserve(new_cap); }
    ...
    extension_traits::is_insertable<T>::value>::type>
{
    ...
    std::error_code & ec)
  {
    ...
    if (cursor.current().size() > 0) {
      reserve_storage(
        typename std::integral_constant<bool, extension_traits::has_reserve<T>::value>::type(), v,
        cursor.current().size());
    }
    cursor.next(ec);
    while (cursor.current().event_type() != staj_event_type::end_array && !ec) {
...
struct decode_traits<std::array<T, N>, CharT>
{
    ...
    std::error_code & ec)
  {
    ...
    cursor.next(ec);
    for (std::size_t i = 0;
         i < N && cursor.current().event_type() != staj_event_type::end_array && !ec; ++i) {
    ...
    type>
{
    ...
    std::error_code & ec)
  {
    ...
    if (cursor.current().size() > 0) {
      reserve_storage(
        typename std::integral_constant<bool, extension_traits::has_reserve<T>::value>::type(), val,
        cursor.current().size());
    }
    cursor.next(ec);
    ...
    std::is_integral<typename T::key_type>::value>::type>
{
    ...
    std::error_code & ec)
  {
    ...
    while (cursor.current().event_type() != staj_event_type::end_object && !ec) {
      ...
      auto s =
        cursor.current().template get<jsoncons::basic_string_view<typename Json::char_type>>(ec);
      if (ec) return val;
      key_type n{0};
