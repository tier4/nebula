...
public:
  basic_path_element(
    const char_type * name, std::size_t length, const Allocator & alloc = Allocator())
  : has_name_(true), name_(name, length, alloc), index_(0)
  {
  }

  explicit basic_path_element(const string_type & name) : has_name_(true), name_(name), index_(0) {}
  ...
  basic_path_element(std::size_t index, const Allocator & alloc = Allocator())
  ...
  bool has_index() const { return !has_name_; }
...
public:
  ...
  using path_element_allocator_type =
    typename std::allocator_traits<allocator_type>::template rebind_alloc<path_element_type>;
...
public:
  ...
  : alloc_(alloc), line_(1), column_(1), end_input_(nullptr), p_(nullptr)
  {
  }

  json_location_parser(
    std::size_t line, std::size_t column, const allocator_type & alloc = allocator_type())
  : alloc_(alloc), line_(line), column_(column), end_input_(nullptr), p_(nullptr)
  {
  }
...
public:
  basic_json_location(const allocator_type & alloc = Allocator()) : elements_(alloc) {}

  explicit basic_json_location(
    const basic_path_node<char_type> & path, const allocator_type & alloc = Allocator())
  : elements_(alloc)
  {
  ...
  int compare(const basic_json_location & other) const
  {
    ...
    return (elements_.size() < other.elements_.size())    ? -1
           : (elements_.size() == other.elements_.size()) ? 0
                                                          : 1;
  }
  ...
  typename std::enable_if<
    extension_traits::is_integer<IntegerType>::value, basic_json_location &>::type
  ...
  static basic_json_location parse(
    const jsoncons::basic_string_view<char_type> & normalized_path, std::error_code ec)
  {
    jsonpath::detail::json_location_parser<char, std::allocator<char>> parser;
...
template <class Json>
std::size_t remove(
  Json & root_value, const basic_json_location<typename Json::char_type> & location)
{
  std::size_t count = 0;
...
template <class CharT, class Allocator = std::allocator<CharT>>
std::basic_string<CharT, std::char_traits<CharT>, Allocator> to_basic_string(
  const basic_json_location<CharT, Allocator> & location, const Allocator & alloc = Allocator())
{
...
inline std::string to_string(const json_location & location)
{
  return to_basic_string(location);
}

inline std::wstring to_wstring(const wjson_location & location)
{
  return to_basic_string(location);
}
