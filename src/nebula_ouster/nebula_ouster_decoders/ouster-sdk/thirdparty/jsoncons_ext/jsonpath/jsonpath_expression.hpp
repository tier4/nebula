...
struct jsonpath_traits
{
  ...
  using evaluator_type =
    typename jsoncons::jsonpath::detail::jsonpath_evaluator<value_type, reference>;
...
private:
  ...
  std::unique_ptr<jsoncons::jsonpath::detail::static_resources<value_type, reference>>
    static_resources_;
...
public:
  ...
  typename std::enable_if<
    extension_traits::is_binary_function_object<
      BinaryCallback, const string_type &, const_reference>::value,
    void>::type
  evaluate(
    const_reference root_value, BinaryCallback callback,
    result_options options = result_options()) const
  {
    ...
    expr_.evaluate(
      resources, const_cast<reference>(root_value), path_node_type{},
      const_cast<reference>(root_value), f, options | result_options::path);
  }
  ...
  value_type evaluate(const_reference root_value, result_options options = result_options()) const
  {
    if ((options & result_options::path) == result_options::path) {
      ...
      expr_.evaluate(
        resources, const_cast<reference>(root_value), path_node_type{},
        const_cast<reference>(root_value), callback, options);
      return result;
    } else {
      jsoncons::jsonpath::detail::dynamic_resources<value_type, reference> resources{alloc_};
      return expr_.evaluate(
        resources, const_cast<reference>(root_value), path_node_type{},
        const_cast<reference>(root_value), options);
    }
  }
  ...
  typename std::enable_if<
    extension_traits::is_binary_function_object<
      BinaryCallback, const path_node_type &, const_reference>::value,
    void>::type
  select(
    const_reference root_value, BinaryCallback callback,
    result_options options = result_options()) const
  {
    ...
    expr_.evaluate(
      resources, const_cast<reference>(root_value), path_node_type{},
      const_cast<reference>(root_value), callback, options | result_options::path);
  }

  template <class BinaryCallback>
  typename std::enable_if<
    extension_traits::is_binary_function_object<
      BinaryCallback, const path_node_type &, value_type &>::value,
    void>::type
  update(reference root_value, BinaryCallback callback) const
  {
    ...
    result_options options =
      result_options::nodups | result_options::path | result_options::sort_descending;
    expr_.evaluate(resources, root_value, path_node_type{}, root_value, callback, options);
  }

  std::vector<basic_json_location<char_type>> select_paths(
    const_reference root_value,
...
template <class Json>
jsonpath_expression<Json> make_expression(
  const typename Json::string_view_type & path,
  const jsoncons::jsonpath::custom_functions<typename jsonpath_traits<Json>::value_type> & funcs =
    jsoncons::jsonpath::custom_functions<typename jsonpath_traits<Json>::value_type>())
{
  using jsonpath_traits_type = jsoncons::jsonpath::jsonpath_traits<Json>;
  ...
  auto static_resources =
    jsoncons::make_unique<jsoncons::jsonpath::detail::static_resources<value_type, reference>>(
      funcs);
  evaluator_type evaluator;
  auto expr = evaluator.compile(*static_resources, path);

  return jsonpath_expression<Json>(
    jsoncons::combine_allocators(), std::move(static_resources), std::move(expr));
}
...
jsonpath_expression<Json> make_expression(
  const allocator_set<typename Json::allocator_type, TempAllocator> & alloc_set,
  const typename Json::string_view_type & expr, std::error_code & ec)
{
  ...
  const custom_functions<Json> & functions = custom_functions<Json>())
{
  ...
  auto resources =
    jsoncons::make_unique<jsoncons::jsonpath::detail::static_resources<value_type, reference>>(
      functions, alloc_set.get_allocator());
...
template <class Json>
std::size_t remove(
  Json & root_value, const jsoncons::basic_string_view<typename Json::char_type> & path_string)
{
  std::size_t count = 0;

  auto expr = jsonpath::make_expression<json>(path_string);
  std::vector<jsonpath::json_location> locations = expr.select_paths(
    root_value, jsonpath::result_options::nodups | jsonpath::result_options::sort_descending);
