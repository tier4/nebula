...
public:
  ...
  : alloc_(alloc), line_(1), column_(1), begin_input_(nullptr), end_input_(nullptr), p_(nullptr)
  {
  }

  jsonpath_evaluator(
    std::size_t line, std::size_t column, const allocator_type & alloc = allocator_type())
  : alloc_(alloc),
    line_(line),
    column_(column),
  ...
  path_expression_type compile(
    static_resources<value_type, reference> & resources, const string_view_type & path)
  {
    std::error_code ec;
    ...
    while (p_ < end_input_ && !state_stack_.empty()) {
      switch (state_stack_.back()) {
        case path_state::start: {
          switch (*p_) {
            ...
            case '@': {
              push_token(
                resources,
                token_type(resources.new_selector(current_node_selector<Json, JsonReference>())),
                ec);
              if (ec) {
          ...
          switch (*p_) {
            ...
            case '$':
              ...
              push_token(
                resources,
                token_type(
                  resources.new_selector(root_selector<Json, JsonReference>(selector_id++))),
                ec);
              if (ec) {
                return path_expression_type(alloc_);
              }
        ...
        case path_state::recursive_descent_or_expression_lhs:
          switch (*p_) {
            case '.':
              push_token(
                resources,
                token_type(resources.new_selector(recursive_selector<Json, JsonReference>())), ec);
              if (ec) {
                return path_expression_type(alloc_);
              }
              ++p_;
        ...
        case path_state::json_text: {
          ...
          push_token(
            resources, token_type(literal_arg, Json(buffer, semantic_tag::none, alloc_)), ec);
          if (ec) {
            return path_expression_type(alloc_);
          }
          buffer.clear();
        ...
        case path_state::relative_path:
          switch (*p_) {
            ...
            case '*':
              push_token(
                resources,
                token_type(resources.new_selector(wildcard_selector<Json, JsonReference>())), ec);
              if (ec) {
                return path_expression_type(alloc_);
              }
              state_stack_.pop_back();
          ...
          switch (*p_) {
            ...
            default: {
              push_token(
                resources,
                token_type(
                  resources.new_selector(identifier_selector<Json, JsonReference>(buffer))),
                ec);
              if (ec) {
                return path_expression_type(alloc_);
              }
        ...
        case path_state::parent_operator: {
          push_token(
            resources,
            token_type(
              resources.new_selector(parent_node_selector<Json, JsonReference>(ancestor_depth))),
            ec);
          paths_required = true;
          ancestor_depth = 0;
          ++p_;
        ...
        case path_state::identifier:
          push_token(
            resources,
            token_type(resources.new_selector(identifier_selector<Json, JsonReference>(buffer))),
            ec);
          if (ec) {
            return path_expression_type(alloc_);
          }
        ...
        case path_state::index_or_slice_or_union:
          switch (*p_) {
            ...
            case ']': {
              ...
              push_token(
                resources,
                token_type(resources.new_selector(index_selector<Json, JsonReference>(n))), ec);
              if (ec) {
                return path_expression_type(alloc_);
              }
              buffer.clear();
        ...
        case path_state::slice_expression_stop: {
          ...
          switch (*p_) {
            ...
            case ',':
              push_token(
                resources,
                token_type(resources.new_selector(slice_selector<Json, JsonReference>(slic))), ec);
              if (ec) {
                return path_expression_type(alloc_);
              }
              slic = slice{};
  ...
  void push_token(
    ...
    token_type && tok, std::error_code & ec)
  {
    // std::cout << tok.to_string() << "\n";
    switch (tok.token_kind()) {
      ...
      case jsonpath_token_kind::end_filter: {
        ...
        while (it != output_stack_.rend() &&
               it->token_kind() != jsonpath_token_kind::begin_filter) {
        ...
        if (!output_stack_.empty() && output_stack_.back().is_path()) {
          output_stack_.back().selector_->append_selector(resources.new_selector(
            filter_selector<Json, JsonReference>(expression_type(std::move(toks)))));
        } else {
          output_stack_.emplace_back(token_type(resources.new_selector(
            filter_selector<Json, JsonReference>(expression_type(std::move(toks))))));
        }
      ...
      case jsonpath_token_kind::end_index_expression: {
        ...
        auto it = output_stack_.rbegin();
        while (it != output_stack_.rend() &&
               it->token_kind() != jsonpath_token_kind::begin_expression) {
          toks.emplace_back(std::move(*it));
          ++it;
        }
        if (it == output_stack_.rend()) {
        ...
        if (!output_stack_.empty() && output_stack_.back().is_path()) {
          output_stack_.back().selector_->append_selector(resources.new_selector(
            index_expression_selector<Json, JsonReference>(expression_type(std::move(toks)))));
        } else {
          output_stack_.emplace_back(token_type(resources.new_selector(
            index_expression_selector<Json, JsonReference>(expression_type(std::move(toks))))));
        }
      ...
      case jsonpath_token_kind::end_argument_expression: {
        ...
        output_stack_.emplace_back(
          token_type(jsoncons::make_unique<expression_type>(std::move(toks))));
        break;
      }
      ...
      case jsonpath_token_kind::end_union: {
        ...
        while (it != output_stack_.rend() && it->token_kind() != jsonpath_token_kind::begin_union) {
          ...
          do {
            ++it;
                   ...
                   it->token_kind() != jsonpath_token_kind::begin_union &&
                   it->token_kind() != jsonpath_token_kind::separator);
        ...
        if (!output_stack_.empty() && output_stack_.back().is_path()) {
          output_stack_.back().selector_->append_selector(
            resources.new_selector(union_selector<Json, JsonReference>(std::move(expressions))));
        } else {
          output_stack_.emplace_back(token_type(
            resources.new_selector(union_selector<Json, JsonReference>(std::move(expressions)))));
        }
        break;
      }
      ...
      case jsonpath_token_kind::end_function: {
        ...
        if (!output_stack_.empty() && output_stack_.back().is_path()) {
          output_stack_.back().selector_->append_selector(resources.new_selector(
            function_selector<Json, JsonReference>(expression_type(std::move(toks)))));
        } else {
          output_stack_.emplace_back(token_type(
            resources.new_selector(function_selector<Json, JsonReference>(std::move(toks)))));
        }
        break;
      }
      case jsonpath_token_kind::literal:
        if (
          !output_stack_.empty() &&
          (output_stack_.back().token_kind() == jsonpath_token_kind::current_node ||
           output_stack_.back().token_kind() == jsonpath_token_kind::root_node)) {
