...
public:
  ...
  json_array_receiver(Json * ptr) : val(ptr) {}

  void add(const path_node_type &, reference value) override { val->emplace_back(value); }
};
...
struct path_generator
{
  ...
  static const path_node_type & generate(
    dynamic_resources<Json, JsonReference> & resources, const path_node_type & last,
    std::size_t index, result_options options)
  {
    const result_options require_path =
      result_options::path | result_options::nodups | result_options::sort;
  ...
  static const path_node_type & generate(
    dynamic_resources<Json, JsonReference> & resources, const path_node_type & last,
    const string_view_type & identifier, result_options options)
  {
    const result_options require_path =
      result_options::path | result_options::nodups | result_options::sort;
    if ((options & require_path) != result_options()) {
...
public:
  ...
  base_selector() : supertype(true, 11), tail_(nullptr) {}
  ...
  void tail_select(
    dynamic_resources<Json, JsonReference> & resources, reference root, const path_node_type & last,
    reference current, node_receiver_type & receiver, result_options options) const
  {
    if (!tail_) {
  ...
  reference evaluate_tail(
    dynamic_resources<Json, JsonReference> & resources, reference root, const path_node_type & last,
    reference current, result_options options, std::error_code & ec) const
  {
    if (!tail_) {
      return current;
    } else {
...
public:
  ...
  void select(
    dynamic_resources<Json, JsonReference> & resources, reference root, const path_node_type & last,
    reference current, node_receiver_type & receiver, result_options options) const override
  {
    if (current.is_object()) {
      auto it = current.find(identifier_);
      if (it != current.object_range().end()) {
        this->tail_select(
          resources, root, path_generator_type::generate(resources, last, identifier_, options),
      ...
      if (r) {
        std::size_t index = (n >= 0)
                              ...
                              : static_cast<std::size_t>(static_cast<int64_t>(current.size()) + n);
        if (index < current.size()) {
          this->tail_select(
            resources, root, path_generator_type::generate(resources, last, index, options),
            current[index], receiver, options);
        }
      } else if (identifier_ == resources.length_label() && current.size() >= 0) {
        pointer ptr =
          resources.create_json(current.size(), semantic_tag::none, resources.get_allocator());
        this->tail_select(
          resources, root, path_generator_type::generate(resources, last, identifier_, options),
      ...
      this->tail_select(
        resources, root, path_generator_type::generate(resources, last, identifier_, options), *ptr,
        receiver, options);
    }
  ...
  reference evaluate(
    dynamic_resources<Json, JsonReference> & resources, reference root, const path_node_type & last,
    reference current, result_options options, std::error_code & ec) const override
  {
    if (current.is_object()) {
      auto it = current.find(identifier_);
      if (it != current.object_range().end()) {
        return this->evaluate_tail(
          resources, root, path_generator_type::generate(resources, last, identifier_, options),
          it->value(), options, ec);
      } else {
...
public:
  ...
  root_selector(std::size_t id) : base_selector<Json, JsonReference>(), id_(id) {}

  void select(
    dynamic_resources<Json, JsonReference> & resources, reference root, const path_node_type & last,
    reference, node_receiver_type & receiver, result_options options) const override
  {
  ...
  reference evaluate(
    dynamic_resources<Json, JsonReference> & resources, reference root, const path_node_type & last,
    reference, result_options options, std::error_code & ec) const override
  {
...
public:
    ...
    reference current, result_options options, std::error_code & ec) const override
  {
    ...
    return this->evaluate_tail(resources, root, last, current, options, ec);
  }
...
public:
  ...
  parent_node_selector(int ancestor_depth) { ancestor_depth_ = ancestor_depth; }
...
public:
  ...
  index_selector(int64_t index) : base_selector<Json, JsonReference>(), index_(index) {}
    ...
    reference current, node_receiver_type & receiver, result_options options) const override
  {
    if (current.is_array()) {
      ...
      if (index_ >= 0 && index_ < slen) {
        ...
        this->tail_select(
          resources, root, path_generator_type::generate(resources, last, i, options),
...
public:
  ...
  wildcard_selector() : base_selector<Json, JsonReference>() {}
    ...
    reference current, node_receiver_type & receiver, result_options options) const override
  {
    if (current.is_array()) {
      for (std::size_t i = 0; i < current.size(); ++i) {
        this->tail_select(
          resources, root, path_generator_type::generate(resources, last, i, options), current[i],
          receiver, options);
      }
    } else if (current.is_object()) {
      for (auto & member : current.object_range()) {
        this->tail_select(
          resources, root, path_generator_type::generate(resources, last, member.key(), options),
  ...
  reference evaluate(
    dynamic_resources<Json, JsonReference> & resources, reference root, const path_node_type & last,
    reference current, result_options options, std::error_code &) const override
  {
    auto jptr =
      resources.create_json(json_array_arg, semantic_tag::none, resources.get_allocator());
...
public:
  ...
  recursive_selector() : base_selector<Json, JsonReference>() {}
    ...
    reference current, node_receiver_type & receiver, result_options options) const override
  {
    if (current.is_array()) {
      ...
      for (auto & item : current.object_range()) {
        select(
          resources, root, path_generator_type::generate(resources, last, item.key(), options),
          item.value(), receiver, options);
      }
    }
    // std::cout << "end wildcard_selector\n";
  }
    ...
    reference current, node_receiver_type & receiver, result_options options) const override
  {
    ...
    if (!ec) {
      if (j.template is<std::size_t>() && current.is_array()) {
        ...
        this->tail_select(
          resources, root, path_generator_type::generate(resources, last, start, options),
          current.at(start), receiver, options);
      } else if (j.is_string() && current.is_object()) {
        auto sv = j.as_string_view();
        this->tail_select(
          resources, root, path_generator_type::generate(resources, last, sv, options),
    ...
    reference current, result_options options, std::error_code & ec) const override
  {
    ...
    if (!ec) {
      if (j.template is<std::size_t>() && current.is_array()) {
        ...
        return this->evaluate_tail(
          resources, root, last, current.at(j.as_string_view()), options, ec);
      } else {
        return resources.null_value();
      }
    } else {
...
public:
  ...
  slice_selector(const slice & slic) : base_selector<Json, JsonReference>(), slice_(slic) {}
    ...
    if (current.is_array()) {
      ...
      if (step > 0) {
        ...
        for (int64_t i = start; i < end; i += step) {
          ...
          this->tail_select(
            resources, root, path_generator_type::generate(resources, last, j, options), current[j],
            receiver, options);
        }
      } else if (step < 0) {
        ...
        for (int64_t i = start; i > end; i += step) {
          ...
          if (j < current.size()) {
            this->tail_select(
              resources, root, path_generator_type::generate(resources, last, j, options),
...
public:
    ...
    reference current, node_receiver_type & receiver, result_options options) const override
  {
    ...
    if (!ec) {
      this->tail_select(
        resources, root, last, *resources.create_json(std::move(ref)), receiver, options);
    }
  }
    ...
    reference current, result_options options, std::error_code & ec) const override
  {
    ...
    if (!ec) {
      return this->evaluate_tail(
        resources, root, last, *resources.create_json(std::move(ref)), options, ec);
    } else {
      return resources.null_value();
    }
  }
