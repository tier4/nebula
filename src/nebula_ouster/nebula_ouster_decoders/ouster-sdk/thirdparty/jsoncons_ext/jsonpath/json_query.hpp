...
typename std::enable_if<
  extension_traits::is_binary_function_object<
    Callback, const typename Json::string_type &, const Json &>::value,
  void>::type
json_query(
  const Json & root_value, const typename Json::string_view_type & path, Callback callback,
  result_options options = result_options(),
  const custom_functions<Json> & functions = custom_functions<Json>())
{
  auto expr = make_expression<Json>(path, functions);
...
typename std::enable_if<is_json_type_traits_specialized<Json, T>::value, void>::type json_replace(
...
typename std::enable_if<is_json_type_traits_specialized<Json, T>::value, void>::type json_replace(
  const allocator_set<typename Json::allocator_type, TempAllocator> & alloc_set, Json & root_value,
  const typename Json::string_view_type & path, T && new_value,
  const custom_functions<Json> & funcs = custom_functions<Json>())
{
  using jsonpath_traits_type = jsoncons::jsonpath::legacy_jsonpath_traits<Json, Json &>;
...
typename std::enable_if<
  extension_traits::is_binary_function_object<
    BinaryCallback, const typename Json::string_type &, Json &>::value,
  void>::type
json_replace(
...
json_replace(
  const allocator_set<typename Json::allocator_type, TempAllocator> & alloc_set, Json & root_value,
  const typename Json::string_view_type & path, BinaryCallback callback,
  const custom_functions<Json> & funcs = custom_functions<Json>())
{
  using jsonpath_traits_type = jsoncons::jsonpath::legacy_jsonpath_traits<Json, Json &>;
...
typename std::enable_if<
  extension_traits::is_unary_function_object<UnaryCallback, Json>::value, void>::type
json_replace(
  Json & root_value, const typename Json::string_view_type & path, UnaryCallback callback)
{
  using jsonpath_traits_type = jsoncons::jsonpath::legacy_jsonpath_traits<Json, Json &>;
  ...
  jsoncons::jsonpath::detail::dynamic_resources<Json, reference> resources;
  auto f = [callback](const path_node_type &, reference v) { v = callback(v); };
  result_options options =
    result_options::nodups | result_options::path | result_options::sort_descending;
  expr.evaluate(resources, root_value, path_node_type{}, root_value, f, options);
}
