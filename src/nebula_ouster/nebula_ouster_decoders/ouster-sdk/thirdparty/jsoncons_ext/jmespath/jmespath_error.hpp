...
public:
  jmespath_error(std::error_code ec) : std::system_error(ec), line_number_(0), column_number_(0) {}
...
class jmespath_error_category_impl : public std::error_category
{
public:
  const char * name() const noexcept override { return "jsoncons/jmespath"; }
...
inline const std::error_category & jmespath_error_category()
{
  static jmespath_error_category_impl instance;
  return instance;
}

inline std::error_code make_error_code(jmespath_errc result)
{
