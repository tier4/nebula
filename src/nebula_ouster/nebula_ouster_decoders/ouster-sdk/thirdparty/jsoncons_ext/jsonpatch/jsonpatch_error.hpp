...
class jsonpatch_error_category_impl : public std::error_category
{
public:
  const char * name() const noexcept override { return "jsoncons/jsonpatch"; }
...
inline const std::error_category & jsonpatch_error_category()
{
  static jsonpatch_error_category_impl instance;
  return instance;
}

inline std::error_code make_error_code(jsonpatch_errc result)
{
...
{
public:
  jsonpatch_error(const std::error_code & ec) : std::system_error(ec) {}
  ...
  const char * what() const noexcept override { return std::system_error::what(); }
};
}  // namespace jsonpatch
}  // namespace jsoncons
