...
template <class Json>
jsonpointer::basic_json_pointer<typename Json::char_type> definite_path(
  const Json & root, jsonpointer::basic_json_pointer<typename Json::char_type> & location)
{
...
struct operation_unwinder
{
  ...
  operation_unwinder(Json & j) : target(j), state(state_type::begin) {}
...
template <class Json>
Json from_diff(
  const Json & source, const Json & target, const typename Json::string_view_type & path)
{
  using char_type = typename Json::char_type;
  ...
  if (source.is_array() && target.is_array()) {
    ...
    for (std::size_t i = 0; i < common; ++i) {
      ...
      result.insert(
        result.array_range().end(), temp_diff.array_range().begin(), temp_diff.array_range().end());
    }
    ...
    for (std::size_t i = source.size(); i-- > target.size();) {
      ...
      val.insert_or_assign(
        jsonpatch_names<char_type>::op_name(), jsonpatch_names<char_type>::remove_name());
    ...
    for (std::size_t i = source.size(); i < target.size(); ++i) {
      ...
      Json val(json_object_arg);
      val.insert_or_assign(
        jsonpatch_names<char_type>::op_name(), jsonpatch_names<char_type>::add_name());
      val.insert_or_assign(jsonpatch_names<char_type>::path_name(), ss);
    ...
    for (const auto & a : source.object_range()) {
      ...
      if (it != target.object_range().end()) {
        ...
        result.insert(
          result.array_range().end(), temp_diff.array_range().begin(),
          temp_diff.array_range().end());
      } else {
        Json val(json_object_arg);
        val.insert_or_assign(
          jsonpatch_names<char_type>::op_name(), jsonpatch_names<char_type>::remove_name());
    ...
    Json val(json_object_arg);
    val.insert_or_assign(
      jsonpatch_names<char_type>::op_name(), jsonpatch_names<char_type>::replace_name());
