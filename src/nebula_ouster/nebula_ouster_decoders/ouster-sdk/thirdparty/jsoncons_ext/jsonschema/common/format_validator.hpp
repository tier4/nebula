      ...
      case state_t::expect_hexdig_or_unspecified: {
        switch (c) {
          ...
          case '9':
            dec_value =
              dec_value * 10 +
              static_cast<std::size_t>(c - '0');  // just in case this piece is followed by a dot
            state = state_t::hexdig;
            ++digit_count;
            break;
          case 'A':
...
{
  enum class state_t { expect_indicator_or_dotted_quad, decbyte, bindig, octdig, hexdig };
...
inline bool validate_date_time_rfc3339(const std::string & s, date_time_type type)
{
  enum class state_t {
    fullyear,
    month,
    ...
    switch (state) {
      ...
      case state_t::mday: {
        if (piece_length < 2 && (c >= '0' && c <= '9')) {
          ...
          (c == 'T' || c == 't') && piece_length == 2 && (mday <= days_in_month(year, month))) {
          piece_length = 0;
          state = state_t::hour;
        } else {
          return false;
      ...
      case state_t::second: {
        if (piece_length < 2 && (c >= '0' && c <= '9')) {
          piece_length++;
          value = value * 10 + static_cast<std::size_t>(c - '0');
        } else if (piece_length == 2 && (/*value >=0 && */ value <= 60))  // 00-58, 00-59, 00-60
...
using format_checker = std::function<void(
  const std::string & schema_path, const jsonpointer::json_pointer & instance_location,
  const std::string &, error_reporter & reporter)>;
  ...
  const std::string & value, error_reporter & reporter)
{
  if (!validate_date_time_rfc3339(value, date_time_type::date)) {
    reporter.error(validation_output(
      "date", schema_path, instance_location.to_uri_fragment(),
  ...
  const std::string & value, error_reporter & reporter)
{
  if (!validate_date_time_rfc3339(value, date_time_type::time)) {
    reporter.error(validation_output(
      "time", schema_path, instance_location.to_uri_fragment(),
  ...
  const std::string & value, error_reporter & reporter)
{
  ...
  try {
    ...
    reporter.error(validation_output(
      "pattern", schema_path, instance_location.to_uri_fragment(),
