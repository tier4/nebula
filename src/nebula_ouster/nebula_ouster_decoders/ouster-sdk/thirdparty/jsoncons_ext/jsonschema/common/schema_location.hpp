...
public:
  ...
  schema_location(const uri & uri) : uri_{uri} { uri_ = jsoncons::uri(uri); }

  jsoncons::uri uri() const { return uri_; }

  bool has_fragment() const { return !identifier_.empty(); }
  ...
  jsoncons::string_view path() const { return uri_.path(); }
  ...
  schema_location append(const std::string & field) const
  {
    ...
    jsoncons::uri new_uri(
      uri_.scheme(), uri_.userinfo(), uri_.host(), uri_.port(), uri_.path(), uri_.query(),
      pointer.to_string());
