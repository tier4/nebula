...
private:
  void do_error(const validation_output & o) override { errors.push_back(o); }
};
...
public:
  ...
  : keyword_validator<Json>(validator->schema_path()), validator_(validator)
  {
  }
...
public:
  ...
  : keyword_validator<Json>(schema_path), content_encoding_(content_encoding)
  {
  }
...
private:
  void do_validate(
    const Json & instance, const jsonpointer::json_pointer & instance_location,
    error_reporter & reporter, Json &) const override
  {
    if (content_encoding_ == "base64") {
      ...
      if (retval.ec != jsoncons::conv_errc::success) {
        reporter.error(validation_output(
          "contentEncoding", this->schema_path(), instance_location.to_uri_fragment(),
...
public:
  content_media_type_validator(
    const std::string & schema_path, const std::string & content_media_type)
  : keyword_validator<Json>(schema_path), content_media_type_(content_media_type)
  {
  }
...
private:
    ...
    error_reporter & reporter, Json &) const override
  {
    if (content_media_type_ == "application/Json") {
      ...
      if (ec) {
        reporter.error(validation_output(
          "contentMediaType", this->schema_path(), instance_location.to_uri_fragment(),
...
public:
  pattern_validator(
    const std::string & schema_path, const std::string & pattern_string, const std::regex & regex)
  : keyword_validator<Json>(schema_path), pattern_string_(pattern_string), regex_(regex)
  {
  }
...
private:
    ...
    error_reporter & reporter, Json &) const override
  {
    ...
    if (!std::regex_search(s, regex_)) {
      ...
      message.append("\"");
      reporter.error(validation_output(
        "pattern", this->schema_path(), instance_location.to_uri_fragment(), std::move(message)));
      if (reporter.fail_early()) {
        return;
      }
    }
...
{
public:
  pattern_validator(const std::string & schema_path) : keyword_validator<Json>(schema_path) {}

private:
  void do_validate(
    const Json &, const jsonpointer::json_pointer &, error_reporter &, Json &) const override
  {
  }
};
#endif
...
private:
    ...
    error_reporter & reporter, Json &) const override
  {
    ...
    if (length > max_length_) {
      reporter.error(validation_output(
        "maxLength", this->schema_path(), instance_location.to_uri_fragment(),
...
private:
    ...
    error_reporter & reporter, Json &) const override
  {
    if (instance.size() > max_items_) {
      ...
      reporter.error(validation_output(
        "maxItems", this->schema_path(), instance_location.to_uri_fragment(), std::move(message)));
      if (reporter.fail_early()) {
        return;
      }
    }
...
private:
    ...
    error_reporter & reporter, Json &) const override
  {
    if (instance.size() < min_items_) {
      ...
      message.append(", found: " + std::to_string(instance.size()));
      reporter.error(validation_output(
        "minItems", this->schema_path(), instance_location.to_uri_fragment(), std::move(message)));
      if (reporter.fail_early()) {
        return;
      }
    }
...
public:
  items_array_validator(
    const std::string & schema_path, std::vector<validator_type> && item_validators,
...
public:
  contains_validator(const std::string & schema_path, validator_type && validator)
  : keyword_validator<Json>(schema_path), validator_(std::move(validator))
  {
  }
...
private:
    ...
    error_reporter & reporter, Json & patch) const override
  {
    if (validator_) {
      ...
      if (!contained) {
        reporter.error(validation_output(
          "contains", this->schema_path(), instance_location.to_uri_fragment(),
          "Expected at least one array item to match \"contains\" schema", local_reporter.errors));
        if (reporter.fail_early()) {
          return;
        }
      }
...
public:
  items_object_validator(const std::string & schema_path, validator_type && items_validator)
  : keyword_validator<Json>(schema_path), items_validator_(std::move(items_validator))
  {
  }
...
private:
    ...
    error_reporter & reporter, Json &) const override
  {
    if (are_unique_ && !array_has_unique_items(instance)) {
      reporter.error(validation_output(
        "uniqueItems", this->schema_path(), instance_location.to_uri_fragment(),
...
private:
    ...
    error_reporter & reporter, Json &) const override
  {
    ...
    if (length < min_length_) {
      reporter.error(validation_output(
        "minLength", this->schema_path(), instance_location.to_uri_fragment(),
...
public:
  string_validator(const std::string & schema_path, std::vector<validator_type> && validators)
...
public:
  not_validator(const std::string & schema_path, validator_type && rule)
  : keyword_validator<Json>(schema_path), rule_(std::move(rule))
  {
  }
...
private:
    ...
    error_reporter & reporter, Json & patch) const final
  {
    ...
    if (local_reporter.errors.empty()) {
      reporter.error(validation_output(
        "not", this->schema_path(), instance_location.to_uri_fragment(),
...
struct all_of_criterion
{
  ...
  static bool is_complete(
    const Json &, const jsonpointer::json_pointer & instance_location, error_reporter & reporter,
    const collecting_error_reporter & local_reporter, std::size_t)
  {
    if (!local_reporter.errors.empty())
      reporter.error(validation_output(
        "allOf", "", instance_location.to_uri_fragment(),
...
struct any_of_criterion
{
  ...
  static bool is_complete(
    const Json &, const jsonpointer::json_pointer &, error_reporter &,
...
struct one_of_criterion
{
    ...
    const collecting_error_reporter &, std::size_t count)
  {
    if (count > 1) {
      ...
      reporter.error(
        validation_output("oneOf", "", instance_location.to_uri_fragment(), std::move(message)));
    }
    return count > 1;
  }
};
...
public:
  combining_validator(std::string && schema_path, std::vector<validator_type> && subschemas)
  : keyword_validator<Json>(std::move(schema_path)), subschemas_(std::move(subschemas))
  {
  }
...
private:
    ...
    error_reporter & reporter, Json & patch) const final
  {
    ...
    if (count == 0) {
      reporter.error(validation_output(
        "combined", this->schema_path(), instance_location.to_uri_fragment(),
        "No schema matched, but one of them is required to match", local_reporter.errors));
    }
  }
};
...
private:
  void do_validate(
    const Json & instance, const jsonpointer::json_pointer & instance_location,
    error_reporter & reporter, Json &) const
  {
    T value = instance.template as<T>();
    if (value > value_) {
      reporter.error(validation_output(
        "maximum", this->schema_path(), instance_location.to_uri_fragment(),
...
private:
    ...
    error_reporter & reporter, Json &) const
  {
    ...
    if (value >= value_) {
      reporter.error(validation_output(
        "exclusiveMaximum", this->schema_path(), instance_location.to_uri_fragment(),
        instance.template as<std::string>() + " exceeds exclusiveMaximum of " +
...
private:
    ...
    error_reporter & reporter, Json &) const
  {
    ...
    if (value < value_) {
      reporter.error(validation_output(
        "minimum", this->schema_path(), instance_location.to_uri_fragment(),
...
private:
    ...
    error_reporter & reporter, Json &) const
  {
    ...
    if (value <= value_) {
      reporter.error(validation_output(
        "exclusiveMinimum", this->schema_path(), instance_location.to_uri_fragment(),
        instance.template as<std::string>() + " exceeds exclusiveMinimum of " +
          std::to_string(value_)));
    }
  }
};
    ...
    error_reporter & reporter, Json &) const
  {
    ...
    if (value != 0)  // Exclude zero
    {
      if (!is_multiple_of(value, static_cast<double>(value_))) {
        reporter.error(validation_output(
          "multipleOf", this->schema_path(), instance_location.to_uri_fragment(),
...
public:
  integer_validator(const std::string & schema_path, std::vector<validator_type> && validators)
  : keyword_validator<Json>(schema_path), validators_(std::move(validators))
  {
  }
...
private:
    ...
    error_reporter & reporter, Json & patch) const
  {
    if (!(instance.template is_integer<int64_t>() ||
          (instance.is_double() && static_cast<double>(instance.template as<int64_t>()) ==
                                     instance.template as<double>()))) {
      reporter.error(validation_output(
        "integer", this->schema_path(), instance_location.to_uri_fragment(),
...
public:
  number_validator(const std::string & schema_path, std::vector<validator_type> && validators)
  : keyword_validator<Json>(schema_path), validators_(std::move(validators))
  {
  }
...
private:
    ...
    error_reporter & reporter, Json & patch) const
  {
    if (!(instance.template is_integer<int64_t>() || instance.is_double())) {
      reporter.error(validation_output(
        "number", this->schema_path(), instance_location.to_uri_fragment(),
...
{
public:
  null_validator(const std::string & schema_path) : keyword_validator<Json>(schema_path) {}
...
private:
    ...
    error_reporter & reporter, Json &) const override
  {
    if (!instance.is_null()) {
      reporter.error(validation_output(
        "null", this->schema_path(), instance_location.to_uri_fragment(), "Expected to be null"));
    }
  }
};
...
{
public:
  boolean_validator(const std::string & schema_path) : keyword_validator<Json>(schema_path) {}
...
{
public:
  true_validator(const std::string & schema_path) : keyword_validator<Json>(schema_path) {}
...
{
public:
  false_validator(const std::string & schema_path) : keyword_validator<Json>(schema_path) {}
...
private:
  ...
  {
    reporter.error(validation_output(
      "false", this->schema_path(), instance_location.to_uri_fragment(),
...
public:
  required_validator(const std::string & schema_path, const std::vector<std::string> & items)
...
private:
  void do_validate(
    const Json & instance, const jsonpointer::json_pointer & instance_location,
    error_reporter & reporter, Json &) const override final
  {
    for (const auto & key : items_) {
      if (instance.find(key) == instance.object_range().end()) {
        reporter.error(validation_output(
          "required", this->schema_path(), instance_location.to_uri_fragment(),
...
public:
  object_validator(
    std::string && schema_path, jsoncons::optional<std::size_t> && max_properties,
...
#endif
    validator_type && additional_properties, std::map<std::string, validator_type> && dependencies,
...
private:
    ...
    error_reporter & reporter, Json & patch) const override
  {
    if (max_properties_ && instance.size() > *max_properties_) {
      ...
      message.append(", found: " + std::to_string(instance.size()));
      reporter.error(validation_output(
        "maxProperties", absolute_max_properties_location_, instance_location.to_uri_fragment(),
        std::move(message)));
      if (reporter.fail_early()) {
        return;
      }
    ...
    if (min_properties_ && instance.size() < *min_properties_) {
      ...
      message.append(", found: " + std::to_string(instance.size()));
      reporter.error(validation_output(
        "minProperties", absolute_min_properties_location_, instance_location.to_uri_fragment(),
        std::move(message)));
      if (reporter.fail_early()) {
        return;
      }
    ...
    if (required_) required_->validate(instance, instance_location, reporter, patch);
    ...
    for (const auto & property : instance.object_range()) {
      ...
      if (!a_prop_or_pattern_matched && additional_properties_) {
        ...
        if (!local_reporter.errors.empty()) {
          reporter.error(validation_output(
            "additionalProperties", additional_properties_->schema_path(),
  ...
  void update_patch(
    Json & patch, const jsonpointer::json_pointer & instance_location, Json && default_value) const
  {
    Json j;
...
public:
  conditional_validator(
    const std::string && schema_path, validator_type && if_validator,
    validator_type && then_validator, validator_type && else_validator)
  : keyword_validator<Json>(std::move(schema_path)),
...
private:
  void do_validate(
    const Json & instance, const jsonpointer::json_pointer & instance_location,
    error_reporter & reporter, Json &) const final
  {
    bool in_range = false;
    ...
    if (!in_range) {
      reporter.error(validation_output(
        "enum", this->schema_path(), instance_location.to_uri_fragment(),
...
private:
    ...
    error_reporter & reporter, Json &) const final
  {
    if (const_validator_ != instance)
      reporter.error(validation_output(
        "const", this->schema_path(), instance_location.to_uri_fragment(),
...
public:
  ...
  type_validator(
    std::string && schema_path, std::vector<validator_type> && type_mapping, Json && default_value,
...
private:
  void do_validate(
    const Json & instance, const jsonpointer::json_pointer & instance_location,
    error_reporter & reporter, Json & patch) const override final
  {
    ...
    if (type)
      ...
      reporter.error(validation_output(
        "type", this->schema_path(), instance_location.to_uri_fragment(), ss.str()));
      if (reporter.fail_early()) {
        return;
      }
    }
  ...
  jsoncons::optional<Json> get_default_value(
    const jsonpointer::json_pointer &, const Json &, error_reporter &) const override
  {
