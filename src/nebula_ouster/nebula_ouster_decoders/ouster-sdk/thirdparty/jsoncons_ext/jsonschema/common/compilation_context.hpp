...
public:
  ...
  explicit compilation_context(const std::vector<schema_location> & uris) : uris_(uris)
  {
  ...
  explicit compilation_context(std::vector<schema_location> && uris) : uris_(std::move(uris))
  {
    for (auto it = uris_.rbegin(); it != uris_.rend(); ++it) {
      if (it->uri().is_absolute()) {
        absolute_uri_ = it->uri();
