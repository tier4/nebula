...
struct default_uri_resolver
{
  Json operator()(const jsoncons::uri & uri)
  {
    ...
    JSONCONS_THROW(
      jsonschema::schema_error("Don't know how to load JSON Schema " + uri.base().string()));
  }
};
...
public:
  ...
  validator_type make_subschema_validator(
    ...
    switch (schema.type()) {
      ...
      default:
        JSONCONS_THROW(schema_error(
          "invalid JSON-type for a schema for " + new_context.get_absolute_uri().string() +
        ...
        if (schema.type() == json_type::object_value) {
          ...
          for (const auto & item : schema.object_range())
            insert_unknown_keyword(
              uri, item.key(), item.value());  // save unknown keywords for later reference
        } else {
          insert_schema(uri, sch);
        }
      }
  ...
  void init_type_mapping(
    std::vector<validator_type> & type_mapping, const std::string & type, const Json & schema,
    const compilation_context & context, std::set<std::string> & keywords)
  {
    if (type == "null") {
      ...
      type_mapping[(uint8_t)json_type::int64_value] =
        make_integer_validator(schema, context, keywords);
      type_mapping[(uint8_t)json_type::uint64_value] =
        make_integer_validator(schema, context, keywords);
      type_mapping[(uint8_t)json_type::double_value] =
        make_integer_validator(schema, context, keywords);
    } else if (type == "number") {
      type_mapping[(uint8_t)json_type::double_value] =
        make_number_validator(schema, context, keywords);
      type_mapping[(uint8_t)json_type::int64_value] =
        make_number_validator(schema, context, keywords);
      type_mapping[(uint8_t)json_type::uint64_value] =
        make_number_validator(schema, context, keywords);
  ...
  std::unique_ptr<type_validator<Json>> make_type_validator(
    const Json & schema, const compilation_context & context)
  {
    ...
    return jsoncons::make_unique<type_validator<Json>>(
      std::move(schema_path), std::move(type_mapping), std::move(default_value),
      std::move(std::move(enumvalidator)), std::move(std::move(const_validator)),
      std::move(std::move(combined_validators)), std::move(std::move(conditionalvalidator)),
      std::move(std::move(expected_types)));
  }

  std::unique_ptr<content_encoding_validator<Json>> make_content_encoding_validator(
    const Json & schema, const compilation_context & context)
  {
  ...
  std::unique_ptr<string_validator<Json>> make_string_validator(
    const Json & schema, const compilation_context & context)
  {
  ...
  std::unique_ptr<content_media_type_validator<Json>> make_content_media_type_validator(
    const Json & schema, const compilation_context & context)
  {
  ...
  std::unique_ptr<format_validator<Json>> make_format_validator(
    const Json & schema, const compilation_context & context)
  {
    ...
    return jsoncons::make_unique<format_validator<Json>>(schema_path, format_check);
  }

  std::unique_ptr<pattern_validator<Json>> make_pattern_validator(
    const Json & schema, const compilation_context & context)
  {
    ...
    return jsoncons::make_unique<pattern_validator<Json>>(schema_path, pattern_string, regex);
  }

  std::unique_ptr<max_length_validator<Json>> make_max_length_validator(
    const Json & schema, const compilation_context & context)
  {
  ...
  std::unique_ptr<max_items_validator<Json>> make_max_items_validator(
    const Json & schema, const compilation_context & context)
  {
  ...
  std::unique_ptr<min_items_validator<Json>> make_min_items_validator(
    const Json & schema, const compilation_context & context)
  {
  ...
  std::unique_ptr<array_validator<Json>> make_array_validator(
    const Json & schema, const compilation_context & context)
  {
    ...
    if (it != schema.object_range().end()) {
      if (it->value().type() == json_type::array_value) {
        validators.emplace_back(make_items_array_validator(schema, it->value(), context));
      } else if (
        it->value().type() == json_type::object_value ||
        it->value().type() == json_type::bool_value) {
        validators.emplace_back(make_items_object_validator(schema, it->value(), context));
      }
    }

    it = schema.find("contains");
    if (it != schema.object_range().end()) {
      validators.emplace_back(make_contains_validator(schema, it->value(), context));
    }

    return jsoncons::make_unique<array_validator<Json>>(schema_path, std::move(validators));
  }

  std::unique_ptr<contains_validator<Json>> make_contains_validator(
    const Json & /*parent*/, const Json & schema, const compilation_context & context)
  {
    std::string schema_path = context.make_schema_path_with("contains");

    return jsoncons::make_unique<contains_validator<Json>>(
      schema_path, make_subschema_validator(schema, context, {"contains"}));
  }
    ...
    if (schema.type() == json_type::array_value) {
      ...
      for (const auto & subsch : schema.array_range())
        item_validators.emplace_back(
          make_subschema_validator(subsch, context, {"items", std::to_string(c++)}));

      auto it = parent.find("additionalItems");
      if (it != parent.object_range().end()) {
        additional_items_validator =
          make_subschema_validator(it->value(), context, {"additionalItems"});
      }
    }

    return jsoncons::make_unique<items_array_validator<Json>>(
      schema_path, std::move(item_validators), std::move(additional_items_validator));
  }
    ...
    const Json & /* parent */, const Json & schema, const compilation_context & context)
  {
    ...
    return jsoncons::make_unique<items_object_validator<Json>>(
      schema_path, make_subschema_validator(schema, context, {"items"}));
  }

  std::unique_ptr<unique_items_validator<Json>> make_unique_items_validator(
    const Json & schema, const compilation_context & context)
  {
  ...
  std::unique_ptr<min_length_validator<Json>> make_min_length_validator(
    const Json & schema, const compilation_context & context)
  {
  ...
  std::unique_ptr<not_validator<Json>> make_not_validator(
    const Json & schema, const compilation_context & context)
  {
    std::string schema_path = context.make_schema_path_with("not");
    return jsoncons::make_unique<not_validator<Json>>(
      schema_path, make_subschema_validator(schema, context, {"not"}));
  }

  std::unique_ptr<maximum_validator<Json, int64_t>> make_maximum_integer_validator(
    const Json & schema, const compilation_context & context)
  {
  ...
  std::unique_ptr<maximum_validator<Json, double>> make_maximum_number_validator(
    const Json & schema, const compilation_context & context)
  {
    std::string schema_path = context.make_schema_path_with("maximum");
    if (!schema.is_number()) {
  ...
  std::unique_ptr<exclusive_maximum_validator<Json, int64_t>>
  make_exclusive_maximum_integer_validator(const Json & schema, const compilation_context & context)
  {
  ...
  std::unique_ptr<exclusive_maximum_validator<Json, double>>
  make_exclusive_maximum_number_validator(const Json & schema, const compilation_context & context)
  {
    std::string schema_path = context.make_schema_path_with("exclusiveMaximum");
    if (!schema.is_number()) {
      std::string message("exclusiveMaximum must be a number value");
  ...
  std::unique_ptr<minimum_validator<Json, int64_t>> make_minimum_integer_validator(
    const Json & schema, const compilation_context & context)
  {
  ...
  std::unique_ptr<minimum_validator<Json, double>> make_minimum_number_validator(
    const Json & schema, const compilation_context & context)
  {
    std::string schema_path = context.make_schema_path_with("minimum");
    if (!schema.is_number()) {
  ...
  std::unique_ptr<exclusive_minimum_validator<Json, int64_t>>
  make_exclusive_minimum_integer_validator(const Json & schema, const compilation_context & context)
  {
  ...
  std::unique_ptr<exclusive_minimum_validator<Json, double>>
  make_exclusive_minimum_number_validator(const Json & schema, const compilation_context & context)
  {
    std::string schema_path = context.make_schema_path_with("exclusiveMinimum");
    if (!schema.is_number()) {
      std::string message("exclusiveMinimum must be a number value");
  ...
  std::unique_ptr<multiple_of_validator<Json>> make_multiple_of_validator(
    const Json & schema, const compilation_context & context)
  {
  ...
  std::unique_ptr<integer_validator<Json>> make_integer_validator(
    const Json & schema, const compilation_context & context, std::set<std::string> & keywords)
  {
  ...
  std::unique_ptr<number_validator<Json>> make_number_validator(
    const Json & schema, const compilation_context & context, std::set<std::string> & keywords)
  {
  ...
  std::unique_ptr<boolean_validator<Json>> make_boolean_validator(
  ...
  std::unique_ptr<required_validator<Json>> make_required_validator(
    const compilation_context & context, const std::vector<std::string> & items)
  {
  ...
  conditional_validator<Json> make_conditional_validator(
    const Json & sch_if, const Json & schema, const compilation_context & context)
  {
    std::string schema_path = context.get_absolute_uri().string();
    ...
    return conditional_validator<Json>(
      std::move(schema_path), std::move(if_validator), std::move(then_validator),
      std::move(else_validator));
  }

  std::unique_ptr<combining_validator<Json, all_of_criterion<Json>>> make_all_of_validator(
    const Json & schema, const compilation_context & context)
  {
    ...
    size_t c = 0;
    for (const auto & subsch : schema.array_range()) {
      subschemas.emplace_back(make_subschema_validator(
        subsch, context, {all_of_criterion<Json>::key(), std::to_string(c++)}));
    }
    return jsoncons::make_unique<combining_validator<Json, all_of_criterion<Json>>>(
      std::move(schema_path), std::move(subschemas));
  }

  std::unique_ptr<combining_validator<Json, any_of_criterion<Json>>> make_any_of_validator(
    const Json & schema, const compilation_context & context)
  {
    ...
    for (const auto & subsch : schema.array_range()) {
      subschemas.emplace_back(make_subschema_validator(
        subsch, context, {any_of_criterion<Json>::key(), std::to_string(c++)}));
    }
    return jsoncons::make_unique<combining_validator<Json, any_of_criterion<Json>>>(
      std::move(schema_path), std::move(subschemas));
  }

  std::unique_ptr<combining_validator<Json, one_of_criterion<Json>>> make_one_of_validator(
    const Json & schema, const compilation_context & context)
  {
    ...
    for (const auto & subsch : schema.array_range()) {
      subschemas.emplace_back(make_subschema_validator(
        subsch, context, {one_of_criterion<Json>::key(), std::to_string(c++)}));
    }
    return jsoncons::make_unique<combining_validator<Json, one_of_criterion<Json>>>(
      std::move(schema_path), std::move(subschemas));
  }

  std::unique_ptr<object_validator<Json>> make_object_validator(
    const Json & schema, const compilation_context & context)
  {
    ...
    if (it != schema.object_range().end()) {
      ...
      required =
        required_validator<Json>(location, it->value().template as<std::vector<std::string>>());
    }
    ...
    if (it != schema.object_range().end()) {
      additional_properties =
        make_subschema_validator(it->value(), context, {"additionalProperties"});
    }
        ...
        switch (dep.value().type()) {
          case json_type::array_value: {
            ...
            dependencies.emplace(
              dep.key(), make_required_validator(
                           compilation_context(std::vector<schema_location>{{location}}),
    ...
    if (property_names_it != schema.object_range().end()) {
      property_name_validator =
        make_subschema_validator(property_names_it->value(), context, {"propertyNames"});
    }

    return jsoncons::make_unique<object_validator<Json>>(
      std::move(schema_path), std::move(max_properties),
      std::move(absolute_max_properties_location), std::move(min_properties),
      std::move(absolute_min_properties_location), std::move(required), std::move(properties),
#if defined(JSONCONS_HAS_STD_REGEX)
      std::move(pattern_properties),
#endif
      std::move(additional_properties), std::move(dependencies),
      std::move(property_name_validator));
  }

  void parse(const Json & sch) override { parse(sch, "#"); }
  ...
  void load(const Json & sch, const compilation_context & context)
  {
    ...
    do {
      ...
      for (const auto & item : subschema_registries_) locations.push_back(item.first);
      ...
      for (const auto & loc : locations) {
        if (subschema_registries_[loc].schemas.empty())  // registry for this file is empty
        {
          if (resolver_) {
            ...
            make_subschema_validator(
              external_schema, compilation_context(schema_location(loc)), {});
            ++loaded_count;
          } else {
            JSONCONS_THROW(schema_error(
    ...
    for (const auto & file : subschema_registries_) {
      if (!file.second.unresolved.empty()) {
        JSONCONS_THROW(schema_error(
          "After all files have been parsed, '" + (file.first == "" ? "<root>" : file.first) +
...
private:
  ...
  void insert_unknown_keyword(
    const schema_location & uri, const std::string & key, const Json & value)
  {
    auto & file = get_or_create_file(uri.base().string());
    ...
    if (new_uri.has_fragment() && !new_uri.has_plain_name_fragment()) {
      ...
      // is there a reference looking for this unknown-keyword, which is thus no longer a unknown
      // keyword but a schema
  ...
  validator_type get_or_create_reference(const schema_location & uri)
  {
    ...
    if (uri.has_fragment() && !uri.has_plain_name_fragment()) {
      ...
      if (unprocessed_keywords_it != file.unprocessed_keywords.end()) {
        ...
        auto s = make_subschema_validator(
          subsch, compilation_context(uri), {});  //  A JSON Schema MUST be an object or a boolean.
    ...
    if (ref != file.unresolved.end()) {
      ...
      auto p = file.unresolved.insert(ref, {std::string(uri.fragment()), orig.get()})
