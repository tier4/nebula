  ...
  bool reuse_ports = true)
{
  ...
  for (auto preferred_af : {AF_INET6, AF_INET}) {
    ...
    if (
      setsockopt(
        sock_fd, SOL_SOCKET, SO_RCVBUF, reinterpret_cast<const char *>(&RCVBUF_SIZE),
        sizeof(RCVBUF_SIZE))) {
    ...
    if (
      getsockopt(
        sock_fd, SOL_SOCKET, SO_RCVBUF, reinterpret_cast<char *>(&actual_rcvbuf_value),
...
{
bool get_config(
  SensorHttp & sensor_http, ouster::sdk::core::SensorConfig & config, bool active = true,
...
bool get_config(const std::string & hostname, SensorConfig & config, bool active, int timeout_sec)
{
  ...
  SensorHttp & sensor_http, const SensorConfig & config, uint8_t config_flags, int timeout_sec)
{
  ...
  if (config_json.contains("signal_multiplier")) {
    ...
    if (
      config_params["signal_multiplier"].as<double>() != 0.25 &&
...
bool set_config(
  const std::string & hostname, const SensorConfig & config, uint8_t config_flags, int timeout_sec)
{
  auto sensor_http = SensorHttp::create(hostname, timeout_sec);
...
{
  logger().info(
    "initializing sensor client: {} expecting lidar port/imu port: {}/{}", hostname, lidar_port,
    imu_port);
...
std::shared_ptr<Client> init_client(
  const std::string & hostname, const std::string & udp_dest_host, LidarMode ld_mode,
  TimestampMode ts_mode, int lidar_port, int imu_port, int timeout_sec, bool persist_config,
...
std::shared_ptr<Client> mtp_init_client(
  const std::string & hostname, const SensorConfig & config, const std::string & mtp_dest_host,
  bool main, int timeout_sec, bool persist_config)
{
...
int poll(ClientPoller & poller, int timeout_sec)
{
  ...
  // NOLINTBEGIN (misc-include-cleaner)
  SOCKET retval = select((int)poller.max_fd + 1, &poller.rfds, NULL, NULL, &timeout_val);
  // NOLINTEND (misc-include-cleaner)
...
ClientState get_poll(const ClientPoller & poller, const Client & client)
{
  ...
  if (FD_ISSET(client.imu_fd, &poller.rfds)) state = ClientState(state | IMU_DATA);
  // NOLINTEND (misc-include-cleaner)
  return state;
}
...
bool read_lidar_packet(const Client & client, uint8_t * buf, const PacketFormat & packet_format)
{
...
bool read_imu_packet(const Client & cli, uint8_t * buf, const PacketFormat & packet_format)
{
...
{
  return get_sock_port(cli.lidar_fd);
}

int get_imu_port(const Client & cli)
{
  return get_sock_port(cli.imu_fd);
}
 ...
 */
extern SOCKET get_lidar_socket_fd(Client & cli)
{
  return cli.lidar_fd;
}
 ...
 * @return the socket file descriptor.
 */
extern SOCKET get_imu_socket_fd(Client & cli)
{
  return cli.imu_fd;
}
