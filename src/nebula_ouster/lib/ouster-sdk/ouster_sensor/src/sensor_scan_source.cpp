    ...
    for (size_t i = 0; i < offset; i++) {
      ...
      if (source_->timeout_ > 0) {
        ...
        if (scan.second) {
          ...
          now = std::chrono::duration_cast<std::chrono::nanoseconds>(now_sec.time_since_epoch())
                  .count();
        }
        ...
        for (const auto time : source_->last_receive_times_) {
          if (now - time > source_->timeout_ns_) {
            ...
            throw ouster::sdk::sensor::ClientTimeout(
              "No valid scans received within " + std::to_string(source_->timeout_) +
              " from sensor " + std::to_string(metadata->sn) + " using udp destination '" +
              metadata->config.udp_dest.value_or("") + "' on port " +
...
SensorScanSource::SensorScanSource(
  const std::string & source, const std::function<void(SensorScanSourceOptions &)> & options)
...
SensorScanSource::SensorScanSource(const std::string & source, SensorScanSourceOptions options)
: SensorScanSource(std::vector<std::string>({source}), options)
{
}

SensorScanSource::SensorScanSource(
  const std::vector<std::string> & source, SensorScanSourceOptions options)
: client_(source, SensorPacketSourceOptions(PacketSourceOptions((ScanSourceOptions &)options))),
  timeout_(options.timeout.retrieve()),
  timeout_ns_(options.timeout.retrieve() * 1e9)
{
  id_error_count_ = 0;

  fields_ = resolve_field_types(
    sensor_info(), options.raw_headers.retrieve(), options.raw_fields.retrieve(),
...
SensorScanSource::SensorScanSource(
  const std::vector<Sensor> & sensors, double config_timeout, unsigned int queue_size,
  bool soft_id_check)
: SensorScanSource(sensors, {}, {}, config_timeout, queue_size, soft_id_check)
{
}
...
: SensorScanSource(sensors, infos, {}, config_timeout, queue_size, soft_id_check)
{
}

SensorScanSource::SensorScanSource(
  const std::vector<Sensor> & sensors, const std::vector<SensorInfo> & infos,
  const std::vector<LidarScanFieldTypes> & fields, double config_timeout, unsigned int queue_size,
  bool soft_id_check)
: client_(sensors, infos, config_timeout)
{
  ...
  if ((!infos.empty()) && infos.size() != sensors.size()) {
    throw std::invalid_argument("If sensor_infos are provided, must provide one for each sensor.");
  }
...
void SensorScanSource::start_thread(unsigned int queue_size, bool soft_id_check)
{
  auto now_sec = std::chrono::system_clock::now();
  int64_t now =
  ...
  batcher_thread_ = std::thread([this, queue_size, soft_id_check]() {
    ...
    while (run_thread_) {
      ...
      if (packet_event.type == ClientEvent::PACKET) {
        ...
        if (batchers[packet_event.source](packet, *scans[packet_event.source])) {
          {
            std::unique_lock<std::mutex> lock(buffer_mutex_);
            buffer_.emplace_back(packet_event.source, std::move(scans[packet_event.source]));
          ...
          scans[packet_event.source] =
            std::make_unique<LidarScan>(info, fields_[packet_event.source]);
        }
      }
    }
  });
...
std::pair<int, std::unique_ptr<LidarScan>> SensorScanSource::get_scan(double timeout_sec)
{
  std::unique_lock<std::mutex> lock(buffer_mutex_);
...
size_t SensorScanSource::size_hint() const
{
  return 0;
}
...
{
  return core::ScanIterator(this, new SensorScanIteratorImpl(const_cast<SensorScanSource *>(this)));
}
