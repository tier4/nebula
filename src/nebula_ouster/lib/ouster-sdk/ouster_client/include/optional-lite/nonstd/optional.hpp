...
public:
  ...
  template <
    typename U = T optional_REQUIRES_T(
      std11::is_move_constructible<U>::value || std11::is_trivially_move_constructible<U>::value)>
  ...
  template <
    typename U, typename... Args optional_REQUIRES_T(
                  std::is_constructible<T, std::initializer_list<U> &, Args &&...>::value)>
  optional_constexpr explicit optional(
    nonstd_lite_in_place_t(T), std::initializer_list<U> il, Args &&... args)
...
template <typename T>
inline optional_constexpr bool operator==(optional<T> const & x, nullopt_t /*unused*/)
  optional_noexcept
{
  return (!x);
}
...
template <typename T>
inline optional_constexpr bool operator==(nullopt_t /*unused*/, optional<T> const & x)
  optional_noexcept
{
  return (!x);
}
...
template <typename T>
inline optional_constexpr bool operator<(optional<T> const & /*unused*/, nullopt_t /*unused*/)
  optional_noexcept
{
  return false;
}
...
template <typename T>
inline optional_constexpr bool operator<=(nullopt_t /*unused*/, optional<T> const & /*unused*/)
  optional_noexcept
{
  return true;
}
