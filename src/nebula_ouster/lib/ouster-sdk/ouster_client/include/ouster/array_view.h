...
public:
  ...
  : ArrayView(
      ptr, std::begin(shape), std::end(shape), impl::index_row<Dim>{}, impl::index_row<Dim - 1>{})
  {
  }
  ...
  : ArrayView(ptr, shape.begin(), shape.end(), impl::index_row<Dim>{}, impl::index_row<Dim - 1>{})
  {
  }
   ...
   */
  template <
    typename OtherT, typename = std::enable_if_t<std::is_same<T, const OtherT>::VALUE, void>>
  ...
  /// @copydoc begin() const
  T * begin() { return const_cast<T *>(const_cast<const ArrayView *>(this)->begin()); }

  /// @copydoc end() const
  T * end() { return const_cast<T *>(const_cast<const ArrayView *>(this)->end()); }
...
private:
  ...
  ArrayView(
    T * ptr, const Indexable1 & _shape, const Indexable2 & _strides,
  ...
  ArrayView(
    T * ptr, IterT first, IterT last, impl::row_pack<I...> /*unused*/,
  ...
  OSDK_FN ArrayView(
    T * ptr, IntT * first, IntT * last, impl::row_pack<I...> /*unused*/,
    impl::row_pack<J...> /*unused*/)
  : data_(ptr),
    strides{impl::product<int32_t>{}(last, impl::index_row<Dim - 1 - J>{})..., 1},
    shape{(void(I), static_cast<uint32_t>(*first++))...}
  ...
  template <int... I>
  OSDK_FN ArrayView(
    T * ptr, const int32_t (&shape)[Dim], const int32_t (&strides)[Dim],
    impl::row_pack<I...> /*unused*/)
