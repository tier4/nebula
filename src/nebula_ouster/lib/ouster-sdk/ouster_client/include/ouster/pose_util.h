...
template <typename T>
void dewarp(
  Eigen::Ref<PointCloudXYZ<T>> dewarped, const Eigen::Ref<const PointCloudXYZ<T>> points,
 ...
 */
template <typename T>
PointCloudXYZ<T> dewarp(
  const Eigen::Ref<const PointCloudXYZ<T>> & points, const Eigen::Ref<const PosesT<T>> poses)
{
...
template <typename T>
void transform(
  Eigen::Ref<PointCloudXYZ<T>> transformed, const Eigen::Ref<const PointCloudXYZ<T>> points,
 ...
 */
template <typename T>
PointCloudXYZ<T> transform(
  const Eigen::Ref<const PointCloudXYZ<T>> points, const Eigen::Ref<const Vector16<T>> pose)
{
 ...
 */
OUSTER_API_FUNCTION
inline PointCloudXYZd transform(const PointCloudXYZd & points, const Pose & pose)
{
  // forward to your templated two-arg version
 ...
 */
template <typename T>
std::vector<Matrix4dR> interp_pose(
  const std::vector<T> & x_interp, T t0, Eigen::Ref<const Matrix4dR> x0, T t1,
  ...
  const Eigen::Ref<const PosesT<Scalar>> poses_known)
{
  ...
  std::vector<ouster::sdk::core::impl::PoseV> pose_diff_intervals(x_known_n - 1);
  ...
  for (size_t i = 0; i < x_known_n - 1; i++) {
    ...
    // matrix
    Eigen::Map<const Matrix4R<Scalar>> prev_pose_matrix(poses_known.row(i).data());
    Eigen::Map<const Matrix4R<Scalar>> curr_pose_matrix(poses_known.row(i + 1).data());

    ouster::sdk::core::impl::PoseH prev_poseh(prev_pose_matrix.template cast<double>());
    ouster::sdk::core::impl::PoseH curr_poseh(curr_pose_matrix.template cast<double>());
    ouster::sdk::core::impl::PoseV pose_diff =
      (curr_poseh * ouster::sdk::core::impl::PoseH(prev_poseh.inverse())).log();
  ...
  for (size_t i = 0; i < x_interp_n; i++) {
    ...
    if (x <= x_known(0)) {
      ...
      Eigen::Map<const Matrix4R<Scalar>> first_pose_matrix(poses_known.row(0).data());
      ouster::sdk::core::impl::PoseH prev_poseh(first_pose_matrix.template cast<double>());
      ...
      Matrix4R<Scalar> result_matrix = interp_poseh.matrix().template cast<Scalar>();
    ...
    if (x >= x_known(x_known_n - 1)) {
      ...
      Eigen::Map<const Matrix4R<Scalar>> last_pose_matrix(poses_known.row(x_known_n - 1).data());
      ouster::sdk::core::impl::PoseH prev_poseh(last_pose_matrix.template cast<double>());
      ouster::sdk::core::impl::PoseV pose_diff = pose_diff_intervals.back();
      ouster::sdk::core::impl::PoseV delta = ratio * pose_diff;
      ouster::sdk::core::impl::PoseH interp_poseh = (delta.exp()) * prev_poseh;
      Matrix4R<Scalar> result_matrix = interp_poseh.matrix().template cast<Scalar>();
      Eigen::Map<const Vector16<Scalar>> result_row(result_matrix.data());
    ...
    while (current_interval < x_known_n - 1 && x_known(current_interval + 1) < x) {
      current_interval++;
    }
    ...
    Eigen::Map<const Eigen::Matrix<Scalar, 4, 4, Eigen::RowMajor>> interval_pose_matrix(
      poses_known.row(current_interval).data());
    ouster::sdk::core::impl::PoseH prev_poseh(interval_pose_matrix.template cast<double>());
    ouster::sdk::core::impl::PoseV pose_diff = pose_diff_intervals[current_interval];
    ouster::sdk::core::impl::PoseV delta = ratio * pose_diff;
    ouster::sdk::core::impl::PoseH interp_poseh = delta.exp() * prev_poseh;
    Eigen::Matrix<Scalar, 4, 4, Eigen::RowMajor> result_matrix =
      interp_poseh.matrix().template cast<Scalar>();
    Eigen::Map<const Eigen::Matrix<Scalar, 1, 16, Eigen::RowMajor>> result_row(
      result_matrix.data());
    results.row(i) = result_row;
  }
...
OUSTER_API_FUNCTION
Poses interp_pose(
  const std::vector<double> & x_interp, const std::vector<double> & x_known,
...
OUSTER_API_FUNCTION std::vector<Eigen::Vector3<T>> dewarp(
  const LidarScan & lidar_scan, const XYZLutT<T> & xyzlut, double min_range, double max_range)
{
  ...
  auto poses = Eigen::Map<const MatrixX16dR>(lidar_scan.pose().get<double>(), lidar_scan.w, 16);
...
template <typename T>
OUSTER_API_FUNCTION std::vector<Eigen::Vector3<T>> dewarp(
  const LidarScanSet & lidar_scan_set, const std::vector<XYZLutT<T>> & xyzluts, double min_range,
  double max_range)
{
  assert(
    lidar_scan_set.size() == xyzluts.size() &&
  ...
  for (size_t idx : lidar_scan_set.valid_indices()) {
    ...
    total_pts.insert(
      total_pts.end(), std::make_move_iterator(dewarped_pts.begin()),
