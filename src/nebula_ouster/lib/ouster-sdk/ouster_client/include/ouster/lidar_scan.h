...
struct OUSTER_API_CLASS FieldType
{
  ...
  OUSTER_API_FUNCTION
  FieldType(
    const std::string & name_, ChanFieldType element_type_,
    const std::vector<size_t> extra_dims_ = {}, FieldClass class_ = FieldClass::PIXEL_FIELD);
   ...
   */
  OUSTER_API_FUNCTION
  inline bool operator<(const FieldType & other) const { return name < other.name; }
};
...
private:
  ...
  LidarScan(size_t w, size_t h, LidarScanFieldTypes field_types, size_t columns_per_packet);
...
public:
  ...
  OUSTER_API_FUNCTION
  LidarScan(
    size_t w, size_t h, UDPProfileLidar profile,
  ...
  LidarScan(
    size_t w, size_t h, Iterator begin, Iterator end,
    size_t columns_per_packet = DEFAULT_COLUMNS_PER_PACKET)
  ...
  OUSTER_API_FUNCTION
  Field & add_field(
    const std::string & name, FieldDescriptor d, FieldClass field_class = FieldClass::PIXEL_FIELD);
 ...
 */
OUSTER_DEPRECATED_MSG("get_field_types(format, fw_version)", OUSTER_DEPRECATED_LAST_SUPPORTED_0_16)
OUSTER_API_FUNCTION
...
inline img_t<T> destagger(
  const Eigen::Ref<const img_t<T>> & img, const std::vector<int> & pixel_shift_by_row,
  bool inverse = false);
