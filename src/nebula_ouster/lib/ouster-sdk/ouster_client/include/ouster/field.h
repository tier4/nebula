...
public:
  ...
  const T * get() const
  {
    if (!desc_.eligible_type<T>()) {
      throw std::invalid_argument(
        ...
        "type " +
        ouster::sdk::core::to_string(desc_.tag()) + ". Dereference type must match or be void.");
    }
  ...
  operator Eigen::Ref<const Eigen::Array<T, Eigen::Dynamic, 3, Eigen::RowMajor>>() const
  {
    ...
    Eigen::Index w = shape()[0];
    return Eigen::Map<const Eigen::Array<T, Eigen::Dynamic, 3, Eigen::RowMajor>>{get<T>(), w, 3};
  }
  ...
  FieldView reshape(Args... dims) const
  {
    ...
    if (requested_size != size()) {
      throw std::invalid_argument(
        ...
        "dimensions had " +
        std::to_string(requested_size) + " but we have " + std::to_string(size()) + " elements.");
    }
