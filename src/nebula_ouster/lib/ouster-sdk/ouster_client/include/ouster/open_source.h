...
struct OUSTER_API_CLASS ScanSourceOptions
{
  ...
  OUSTER_API_FUNCTION
  void check(
    const char * source_type  ///<[in] name of source type
  ) const;
};
...
ouster::sdk::core::AnyScanSource open_source(
  ...
  const std::function<void(ScanSourceOptions &)> & options = {},  ///< [in] source options
...
ouster::sdk::core::AnyScanSource open_source(
  ...
  const std::function<void(ScanSourceOptions &)> & options = {}, bool collate = true,
  int sensor_idx = -1);
...
struct OUSTER_API_CLASS PacketSourceOptions
{
  ...
  OUSTER_API_FUNCTION
  explicit PacketSourceOptions(
    ScanSourceOptions & options  ///< [in] options
  );
