...
protected:
  ...
  Packet(
    PacketType type  ///< [in] type packet type (imu or lidar)
  );
...
PacketValidationFailure validate_packet(
  const SensorInfo & info, const PacketFormat & format, const uint8_t * buf, uint64_t buf_size,
...
struct OUSTER_API_CLASS LidarPacket : public Packet
{
  ...
  OUSTER_API_FUNCTION inline auto nth_col(
    int n  ///< [in] which column
  ) const
  {
  ...
  OUSTER_API_FUNCTION inline auto footer() const { return format->footer(buf.data()); }
  ...
  OUSTER_API_FUNCTION inline auto block_parsable() const { return format->block_parsable(); }
...
struct OUSTER_API_CLASS ImuPacket : public Packet
{
  ...
  OUSTER_API_FUNCTION inline auto sys_ts() const { return format->imu_sys_ts(buf.data()); }

  /// Read accelerometer timestamp from the packet.
  /// Only available in PROFILE_IMU_LEGACY, otherwise returns 0.
  /// @return accelerometer timestamp
  OUSTER_API_FUNCTION inline auto accel_ts() const { return format->imu_accel_ts(buf.data()); }

  /// Read gyroscope timestamp from the packet.
  /// Only available in PROFILE_IMU_LEGACY, otherwise returns 0.
  /// @return gyroscope timestamp
  OUSTER_API_FUNCTION inline auto gyro_ts() const { return format->imu_gyro_ts(buf.data()); }

  /// Read NMEA sentence from an IMU buffer.
  /// Only available in PROFILE_ACCEL32_GYRO32_NMEA.
  /// @return nmea sentence string
  OUSTER_API_FUNCTION
  std::string nmea_sentence() const { return format->imu_nmea_sentence(buf.data()); }
  ...
  [[deprecated("Use accel() instead")]] OUSTER_API_FUNCTION inline auto la_x() const
  {
  ...
  [[deprecated("Use accel() instead")]] OUSTER_API_FUNCTION inline auto la_y() const
  {
    if (format->udp_profile_imu == UDPProfileIMU::LEGACY) {
  ...
  [[deprecated("Use accel() instead")]] OUSTER_API_FUNCTION inline auto la_z() const
  {
    if (format->udp_profile_imu == UDPProfileIMU::LEGACY) {
  ...
  [[deprecated("Use gyro() instead")]] OUSTER_API_FUNCTION inline auto av_x() const
  {
    if (format->udp_profile_imu == UDPProfileIMU::LEGACY) {
  ...
  [[deprecated("Use gyro() instead")]] OUSTER_API_FUNCTION inline auto av_y() const
  {
    if (format->udp_profile_imu == UDPProfileIMU::LEGACY) {
  ...
  [[deprecated("Use gyro() instead")]] OUSTER_API_FUNCTION inline auto av_z() const
  {
    if (format->udp_profile_imu == UDPProfileIMU::LEGACY) {
...
struct OUSTER_API_CLASS ZonePacket : public Packet
{
  ...
  uint64_t OUSTER_API_FUNCTION timestamp() const { return format->zone_timestamp(buf.data()); }
