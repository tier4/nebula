...
void visit_field_2d(const FieldView & field, OP && op, Args &&... args)
{
  switch (field.tag()) {
    case ChanFieldType::UINT8:
      op.template operator()(Eigen::Ref<const img_t<uint8_t>>(field), std::forward<Args>(args)...);
      break;
    case ChanFieldType::UINT16:
      op.template operator()(Eigen::Ref<const img_t<uint16_t>>(field), std::forward<Args>(args)...);
      break;
    case ChanFieldType::UINT32:
      op.template operator()(Eigen::Ref<const img_t<uint32_t>>(field), std::forward<Args>(args)...);
      break;
    case ChanFieldType::UINT64:
      op.template operator()(Eigen::Ref<const img_t<uint64_t>>(field), std::forward<Args>(args)...);
      break;
    case ChanFieldType::INT8:
      op.template operator()(Eigen::Ref<const img_t<int8_t>>(field), std::forward<Args>(args)...);
      break;
    case ChanFieldType::INT16:
      op.template operator()(Eigen::Ref<const img_t<int16_t>>(field), std::forward<Args>(args)...);
      break;
    case ChanFieldType::INT32:
      op.template operator()(Eigen::Ref<const img_t<int32_t>>(field), std::forward<Args>(args)...);
      break;
    case ChanFieldType::INT64:
      op.template operator()(Eigen::Ref<const img_t<int64_t>>(field), std::forward<Args>(args)...);
      break;
    case ChanFieldType::FLOAT16:
      ...
      break;
    case ChanFieldType::FLOAT32:
      op.template operator()(Eigen::Ref<const img_t<float>>(field), std::forward<Args>(args)...);
      break;
    case ChanFieldType::FLOAT64:
      op.template operator()(Eigen::Ref<const img_t<double>>(field), std::forward<Args>(args)...);
      break;
    case ChanFieldType::ZONE_STATE:
    case ChanFieldType::CHAR:
    case ChanFieldType::VOID:
...
void visit_field_ndim(const FieldView & field, OP && op, Args &&... args)
{
  if (field.shape().size() == 3) {
    switch (field.tag()) {
      case ChanFieldType::FLOAT16:
        op.template operator()(ConstArrayView<float16_t, 3>(field), std::forward<Args>(args)...);
        return;
      default:
        throw std::invalid_argument("Invalid field for LidarScan");
    }
    ...
    switch (field.tag()) {
      case ChanFieldType::UINT8:
        op.template operator()(ConstArrayView<uint8_t, 2>(field), std::forward<Args>(args)...);
        break;
      case ChanFieldType::UINT16:
        op.template operator()(ConstArrayView<uint16_t, 2>(field), std::forward<Args>(args)...);
        break;
      case ChanFieldType::UINT32:
        op.template operator()(ConstArrayView<uint32_t, 2>(field), std::forward<Args>(args)...);
        break;
      case ChanFieldType::UINT64:
        op.template operator()(ConstArrayView<uint64_t, 2>(field), std::forward<Args>(args)...);
        break;
      case ChanFieldType::INT8:
        op.template operator()(ConstArrayView<int8_t, 2>(field), std::forward<Args>(args)...);
        break;
      case ChanFieldType::INT16:
        op.template operator()(ConstArrayView<int16_t, 2>(field), std::forward<Args>(args)...);
        break;
      case ChanFieldType::INT32:
        op.template operator()(ConstArrayView<int32_t, 2>(field), std::forward<Args>(args)...);
        break;
      case ChanFieldType::INT64:
        op.template operator()(ConstArrayView<int64_t, 2>(field), std::forward<Args>(args)...);
        break;
      case ChanFieldType::FLOAT16:
        op.template operator()(ConstArrayView<float16_t, 2>(field), std::forward<Args>(args)...);
        break;
      case ChanFieldType::FLOAT32:
        op.template operator()(ConstArrayView<float, 2>(field), std::forward<Args>(args)...);
        break;
      case ChanFieldType::FLOAT64:
        op.template operator()(ConstArrayView<double, 2>(field), std::forward<Args>(args)...);
        break;
      default:
        throw std::invalid_argument("Invalid field for LidarScan");
    }
...
void visit_field(SCAN && ls, const std::string & name, OP && op, Args &&... args)
{
  ...
  visit_field_2d(ls.field(name), std::forward<OP>(op), std::forward<Args>(args)...);
}
 ...
 */
template <typename SCAN, typename OP, typename... Args>
void visit_field_ndim(SCAN && ls, const std::string & name, OP && op, Args &&... args)
{
  // throw early as python downstream expects ValueError
  if (!ls.has_field(name)) {
    throw std::invalid_argument("Invalid field for LidarScan");
  ...
  visit_field_ndim(ls.field(name), std::forward<OP>(op), std::forward<Args>(args)...);
}
 ...
 */
template <typename SCAN, typename OP, typename... Args>
void foreach_channel_field(SCAN && ls, const PacketFormat & pf, OP && op, Args &&... args)
{
  for (const auto & ft : pf) {
    if (ls.has_field(ft.first)) {
      visit_field(ls, ft.first, std::forward<OP>(op), ft.first, std::forward<Args>(args)...);
    }
  }
}
 ...
 */
template <typename SCAN, typename OP, typename... Args>
void foreach_channel_field_ndim(SCAN && ls, const PacketFormat & pf, OP && op, Args &&... args)
{
  for (const auto & ft : pf) {
    if (ls.has_field(ft.first)) {
      visit_field_ndim(ls, ft.first, std::forward<OP>(op), ft.first, std::forward<Args>(args)...);
    }
  }
}
...
struct OUSTER_API_CLASS copy_and_cast
{
  template <typename T>
  void operator()(
    Eigen::Ref<img_t<T>> field_dest, const LidarScan & ls_source,
...
void scan_to_packets(
  const LidarScan & ls, const ouster::sdk::core::impl::PacketWriter & pw, OutputItT iter,
  uint32_t init_id, uint64_t prod_sn)
{
  ...
  auto emit_lidar_packet = [&pw, &ls, &set_header, &iter, &packet_format](size_t packet_id) {
    ...
    auto pack_field = [&pw](auto ref_field, const std::string & i, LidarPacket & packet) {
      if (sizeof(ref_field.shape) / sizeof(ref_field.shape[0]) != 2) {
        ...
        pw.set_block(ref_field.data(), ref_field.shape[1], i, packet.buf.data());
      }
    };
    ...
    if (raw_headers_enabled(pw, ls)) {
      ...
      pw.udp_profile_lidar != UDPProfileLidar::LEGACY && pw.header_type == HeaderType::STANDARD) {
      assert(lidar_packet.buf.size() > sizeof(uint64_t));
      uint64_t crc = pw.calculate_crc(lidar_packet.buf.data(), lidar_packet.buf.size());
      memcpy(lidar_packet.buf.data() + lidar_packet.buf.size() - sizeof(crc), &crc, sizeof(crc));
    }

    *iter++ = lidar_packet;
  };

  auto emit_imu_packet = [&pw, &ls, &set_header, &iter, &packet_format](size_t packet_id) {
    ...
    if (ls.has_field(POSITION_STRING)) {
      ...
      pw.set_imu_nmea_sentence(imu_buf, nmea_sentences.subview(packet_id).data());
    }
    ...
    const FieldView imu_ts_fview = ls.has_field(IMU_TIMESTAMP) ? ls.field(IMU_TIMESTAMP) : empty;
    ...
    if (pw.header_type == HeaderType::STANDARD) {
      uint64_t crc = pw.calculate_crc(imu_packet.buf.data(), imu_packet.buf.size());
  ...
  auto emit_zm_packet = [&pw, &ls, &set_header, &iter, &packet_format]() {
    ...
    if (pw.header_type == HeaderType::STANDARD) {
      uint64_t crc = pw.calculate_crc(zone_packet.buf.data(), zone_packet.buf.size());
      memcpy(zone_packet.buf.data() + zone_packet.buf.size() - sizeof(crc), &crc, sizeof(crc));
    }
...
inline void destagger_into(
  const Eigen::Ref<const img_t<T>> & img, const std::vector<int> & pixel_shift_by_row, bool inverse,
