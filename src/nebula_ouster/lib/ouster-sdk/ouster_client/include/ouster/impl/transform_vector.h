...
public:
   ...
   */
  template <typename OtherDerived>
  RotV(const Eigen::AngleAxis<OtherDerived> & other) : Eigen::Vector3d(other.axis() * other.angle())
  {
  }
   ...
   */
  template <typename OtherDerived>
  RotV(const Eigen::Quaternion<OtherDerived> & other) : RotV(Eigen::AngleAxis<OtherDerived>{other})
  {
  }
