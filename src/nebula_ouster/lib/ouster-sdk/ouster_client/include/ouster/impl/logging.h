...
private:
  ...
  std::string finalize_format_builder(std::shared_ptr<Logger::format_builder>);
...
private:
  // Base Case
  template <typename T>
  void process_args(std::shared_ptr<Logger::format_builder> builder, T next_arg)
  {
  ...
  void process_args(std::shared_ptr<Logger::format_builder> builder, T next_arg, Args &&... args)
  {
    process_arg(builder, next_arg);
    process_args(builder, std::forward<Args>(args)...);
  }
