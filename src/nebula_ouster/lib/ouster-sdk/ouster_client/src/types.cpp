...
extern const Table<MultipurposeIOMode, const char *, 6> MULTIPURPOSE_IO_MODE_STRINGS{
...
Table<BloomReductionOptimization, const char *, 2> bloom_reduction_optimization_strings{{
...
{
  return (
    lhs.reflectivity_status == rhs.reflectivity_status &&
...
bool operator==(const SensorConfig & lhs, const SensorConfig & rhs)
{
  return (
    ...
    lhs.azimuth_window == rhs.azimuth_window && lhs.signal_multiplier == rhs.signal_multiplier &&
    lhs.nmea_in_polarity == rhs.nmea_in_polarity &&
    lhs.nmea_ignore_valid_char == rhs.nmea_ignore_valid_char &&
    lhs.nmea_baud_rate == rhs.nmea_baud_rate && lhs.nmea_leap_seconds == rhs.nmea_leap_seconds &&
    ...
    lhs.udp_profile_lidar == rhs.udp_profile_lidar && lhs.udp_profile_imu == rhs.udp_profile_imu &&
...
static optional<V> lookup(const impl::Table<K, V, N> & table, const K & key)
{
  ...
  auto res = std::find_if(
    table.begin(), end, [&](const std::pair<K, V> & pair) { return pair.first == key; });

  return res == end ? nullopt : make_optional<V>(res->second);
}

template <typename K, size_t N>
static optional<K> rlookup(const impl::Table<K, const char *, N> & table, const char * value)
{
  auto end = table.end();
...
{
  auto res = lookup(impl::thermal_shutdown_status_strings, thermal_shutdown_status);
  return res ? res.value() : "UNKNOWN";
}
...
jsoncons::json config_to_json(const SensorConfig & config)
{
  ...
  if (config.bloom_reduction_optimization) {
    root["bloom_reduction_optimization"] = to_string(config.bloom_reduction_optimization.value());
  }
  ...
  for (const auto & kv : config.extra_options) {
    try {
      ...
      throw std::runtime_error(
        "Failed to parse config extra_options['" + kv.first + "'] as json: " + e.what());
    }
  }
...
{
  std::regex product_regex(R"(^(\w+)-(\d+|DOME)?(?:-(\d+))?(?:-((?!SR)\w+))?-?(SR)?)");
  std::smatch matches;
  if (!product_info_string.empty()) {
    if (regex_search(product_info_string, matches, product_regex)) {
      ...
      return ProductInfo(product_info_string, form_factor, short_range, beam_config, beam_count);
    } else {
      throw std::runtime_error(
        "Product Info \"" + product_info_string + "\" is not a recognized product info");
    }
  }
  return ProductInfo();
}
