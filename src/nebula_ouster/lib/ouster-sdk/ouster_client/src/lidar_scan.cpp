...
static LidarScanFieldTypes lookup_scan_fields(UDPProfileLidar profile)
{
  ...
  auto it = std::find_if(impl::default_scan_fields.begin(), end, [profile](const auto & key_value) {
  ...
  for (size_t i = 0; i < entry.n_fields; i++) {
    ...
    field_types.emplace_back(
      entry.fields[i].first, entry.fields[i].second, std::vector<size_t>(),
      FieldClass::PIXEL_FIELD);
  }
  return field_types;
}

bool raw_headers_enabled(const PacketFormat & packet_format, const LidarScan & lidar_scan)
{
  ...
  // column
  if (
    packet_format.pixels_per_column * field_type_size(raw_headers_ft) <
...
static FieldDescriptor get_field_type_descriptor(
  const LidarScan & scan, const FieldType & field_type)
{
...
: LidarScan{
    format.columns_per_frame, format.pixels_per_column, get_field_types(format, Version(0, 0, 0)),
...
: LidarScan{
    info.format.columns_per_frame, info.format.pixels_per_column, get_field_types(info),
...
LidarScan::LidarScan(std::shared_ptr<SensorInfo> info, const std::vector<FieldType> & field_types)
: LidarScan{
    info->format.columns_per_frame, info->format.pixels_per_column, field_types,
    info->format.columns_per_packet}
{
  sensor_info = info;
}
...
: packet_count_{
    (w + columns_per_packet - 1) / columns_per_packet},  // equivalent to
...
LidarScan::LidarScan(const LidarScan & ls_src, const LidarScanFieldTypes & field_types)
    ...
    if (ls_src.has_field(name)) {
      ...
      if (src_desc == dst_desc) {
        ...
        ouster::sdk::core::impl::visit_field(
          *this, name, ouster::sdk::core::impl::copy_and_cast(), ls_src, name);
      }
    } else {
      add_field(name, dst_desc, field_type.field_class);
    }
...
LidarScan::LidarScan(size_t w, size_t h, UDPProfileLidar profile, size_t columns_per_packet)
...
Field & LidarScan::add_field(const std::string & name, FieldDescriptor desc, FieldClass field_class)
{
  if (has_field(name)) {
  ...
  if (field_class == FieldClass::PACKET_FIELD) {
    if (desc.shape[0] != packet_count_) {
      throw std::invalid_argument(
        ...
        std::to_string(desc.shape[0]) + " vs required width of " + std::to_string(packet_count_));
    }
  }
...
static FieldType get_field_type(const std::string & name, const Field & field)
{
  ...
  if (field.field_class() == FieldClass::PIXEL_FIELD) {
    ...
    (field.field_class() == FieldClass::COLUMN_FIELD) ||
...
std::unordered_map<std::string, Field> & LidarScan::fields()
{
  return fields_;
}
...
Eigen::Ref<const LidarScan::Header<uint64_t>> LidarScan::packet_timestamp() const
{
  return packet_timestamp_;
}
...
Field & LidarScan::pose()
{
  return pose_;
}

const Field & LidarScan::pose() const
{
  return pose_;
}
...
void LidarScan::set_column_pose(int index, const Matrix4dR & pose)
{
  ...
  std::memcpy(
    static_cast<char *>(pose_.get()) + index * 16 * sizeof(double), pose.data(),
    sizeof(double) * 16);
}
...
Matrix4dR LidarScan::get_column_pose(int index) const
{
  ...
  Matrix4dR out;
  std::memcpy(
    out.data(), static_cast<const char *>(pose_.get()) + index * 16 * sizeof(double),
    sizeof(double) * 16);
  return out;
}
...
size_t LidarScan::packet_count() const
{
...
LidarScanFieldTypes get_field_types(const DataFormat & format, const Version & fw_version)
{
  LidarScanFieldTypes field_types = impl::lookup_scan_fields(format.udp_profile_lidar);
...
std::string to_string(const LidarScan & lidar_scan)
{
  ...
  string_stream << "LidarScan: {h = " << lidar_scan.h << ", w = " << lidar_scan.w
                << ", packets_per_frame = " << lidar_scan.packet_timestamp().size()
                << ", fid = " << lidar_scan.frame_id << "," << std::endl
                << " frame status = " << std::hex << lidar_scan.frame_status << std::dec
                << ", thermal_shutdown status = " << to_string(lidar_scan.thermal_shutdown())
                << ", shot_limiting status = " << to_string(lidar_scan.shot_limiting()) << ","
                << std::endl
  ...
  auto read_field = [&read_eigen](
                      const Field & field_val, const std::string & name,
                      std::stringstream & string_stream) {
...
: w_(info->format.columns_per_frame), h_(info->format.pixels_per_column), pf(get_format(*info))
{
...
ScanBatcher::ScanBatcher(const SensorInfo & info) : ScanBatcher(std::make_shared<SensorInfo>(info))
{
}
 ...
 */
void zero_header_cols(LidarScan & lidar_scan, std::ptrdiff_t start, std::ptrdiff_t end)
{
...
struct ParseFieldCol
{
  ...
  void operator()(
    ArrayView<T, NDim> field, const std::string & field_name, uint16_t m_id,
    const PacketFormat & packet_format, const uint8_t * col_buf) const
  {
    ...
    if (NDim == 3) {
      ...
      packet_format.col_field(
        col_buf, field_name, field.subview(keep(), m_id).data(), field.shape[1]);
    }
  }
};
...
struct PackRawHeadersCol
{
  template <typename T>
  void operator()(
    Eigen::Ref<img_t<T>> rh_field, const std::string & /*unused*/,
    const PacketFormat & packet_format, uint16_t col_idx, const uint8_t * packet_buf) const
  {
    ...
    const ColMajorView col_header_vec(
      reinterpret_cast<const T *>(col_buf), packet_format.col_header_size / sizeof(T));

    rh_field.block(0, m_id, col_header_vec.size(), 1) = col_header_vec;

    const ColMajorView col_footer_vec(
      reinterpret_cast<const T *>(col_buf + packet_format.col_size - packet_format.col_footer_size),
    ...
    const ColMajorView packet_header_vec(
      reinterpret_cast<const T *>(packet_buf), packet_format.packet_header_size / sizeof(T));

    rh_field.block(
      col_header_vec.size() + col_footer_vec.size(), m_id, packet_header_vec.size(), 1) =
      packet_header_vec;
    ...
    rh_field.block(
      col_header_vec.size() + col_footer_vec.size() + packet_header_vec.size(), m_id,
      packet_footer_vec.size(), 1) = packet_footer_vec;
  }
};

void zero_field(
  LidarScan & scan, const std::string & name, std::ptrdiff_t start, std::ptrdiff_t end,
...
void zero_fields(
  LidarScan & scan, const PacketFormat & packet_format, std::ptrdiff_t start, std::ptrdiff_t end)
{
...
void ScanBatcher::parse_by_col(const uint8_t * packet_buf, LidarScan & lidar_scan)
{
  ...
  for (int icol = 0; icol < pf.columns_per_packet; icol++) {
    ...
    if (raw_headers) {
      ...
      if (m_id >= next_headers_m_id_) {
        zero_field(lidar_scan, ChanField::RAW_HEADERS, next_headers_m_id_, m_id);
        next_headers_m_id_ = m_id + 1;
      }

      impl::visit_field(
        lidar_scan, ChanField::RAW_HEADERS, PackRawHeadersCol(), ChanField::RAW_HEADERS, pf, icol,
        packet_buf);
    }
    ...
    impl::foreach_channel_field_ndim(lidar_scan, pf, ParseFieldCol{}, m_id, pf, col_buf);
  }
}
...
struct ParseFieldBlock
{
  template <typename T, size_t NDim>
  void operator()(
    ArrayView<T, NDim> field, const std::string & field_name, const PacketFormat & packet_format,
    const uint8_t * packet_buf) const
  {
    ...
    if (NDim == 3) {
      ...
      packet_format.block_field<T, BlockDim>(field.data(), field.shape[1], field_name, packet_buf);
    }
  }
};

void ScanBatcher::parse_by_block(const uint8_t * packet_buf, LidarScan & lidar_scan)
{
  // zero out missing columns if we jumped forward
  ...
  switch (pf.block_parsable()) {
    case 16:
      impl::foreach_channel_field_ndim(lidar_scan, pf, ParseFieldBlock<16>{}, pf, packet_buf);
      break;
    case 8:
      impl::foreach_channel_field_ndim(lidar_scan, pf, ParseFieldBlock<8>{}, pf, packet_buf);
      break;
    case 4:
      impl::foreach_channel_field_ndim(lidar_scan, pf, ParseFieldBlock<4>{}, pf, packet_buf);
      break;
    default:
...
void ScanBatcher::batch_lidar_packet(const LidarPacket & packet, LidarScan & lidar_scan)
{
  ...
  if (block_parsable != 0) {
    for (int icol = 0; icol < pf.columns_per_packet; icol += block_parsable) {
      const uint8_t * col_buf = pf.nth_col(icol, packet_buf);
      const uint16_t m_id = pf.col_measurement_id(col_buf);
...
{
  if (
    packet.type() == PacketType::Imu && pf.udp_profile_imu != UDPProfileIMU::ACCEL32_GYRO32_NMEA) {
    return false;
  }
  ...
  if (
    static_cast<size_t>(lidar_scan.packet_timestamp().rows()) !=
    lidar_scan.w / pf.columns_per_packet) {
    throw std::invalid_argument(
      "unexpected scan columns_per_packet: " + std::to_string(pf.columns_per_packet));
  }
  ...
  if (lidar_scan.frame_id == -1 || finished_scan_id_ >= 0) {
    ...
    if (finished_scan_id_ >= 0) {
      ...
      if (finished_scan_id_ == f_id) {
        ...
        return false;
      } else if (finished_scan_id_ == ((f_id + 1) % (static_cast<int64_t>(pf.max_frame_id) + 1))) {
    ...
    lidar_scan.frame_status = frame_status(f_thermal_shutdown, f_shot_limiting);

    // The countdown values are supposed to be the same for all packets in a
    // given scan.
    lidar_scan.shutdown_countdown = pf.countdown_thermal_shutdown(packet_buf);
    lidar_scan.shot_limiting_countdown = pf.countdown_shot_limiting(packet_buf);
    lidar_scan.sensor_info = sensor_info_;
  } else if (lidar_scan.frame_id == ((f_id + 1) % (static_cast<int64_t>(pf.max_frame_id) + 1))) {
    // drop reordered packets from the previous frame
    return false;
  ...
  if (pf.udp_profile_lidar == UDPProfileLidar::LEGACY || packet.type() == PacketType::Lidar) {
    ...
    pf.udp_profile_imu == UDPProfileIMU::ACCEL32_GYRO32_NMEA && packet.type() == PacketType::Imu) {
...
bool ScanBatcher::check_scan_complete(const LidarScan & lidar_scan) const
{
  bool lidar_batching_finished =
    ...
    (batched_lidar_packets_ >= expected_lidar_packets_ &&
     static_cast<size_t>(lidar_scan.packet_timestamp().count()) == expected_lidar_packets_);
  ...
  return lidar_batching_finished && imu_batching_finished && zone_batching_finished;
}

void ScanBatcher::finalize_scan(LidarScan & lidar_scan)
{
  if (next_valid_m_id_ < w_) {
    zero_fields(lidar_scan, pf, next_valid_m_id_, static_cast<std::ptrdiff_t>(w_));
  }

  if (impl::raw_headers_enabled(pf, lidar_scan)) {
    zero_field(
      lidar_scan, ChanField::RAW_HEADERS, next_headers_m_id_, static_cast<std::ptrdiff_t>(w_));
  }
...
FieldType::FieldType(
  const std::string & name_in, ChanFieldType element_type_in,
  const std::vector<size_t> extra_dims_in, FieldClass class_in)
: name(name_in), element_type(element_type_in), extra_dims(extra_dims_in), field_class(class_in)
{
}
