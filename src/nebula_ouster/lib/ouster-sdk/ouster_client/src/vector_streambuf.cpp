...
: buffer_(const_cast<ByteVector *>(buffer))
{  // std::streambuf's setg and setp methods require
   // non-const pointer args.
  if (buffer_ == nullptr) {
    throw std::invalid_argument("VectorStreamBuf: buffer pointer is null");
  }
  setg(
    reinterpret_cast<char *>(buffer_->data()), reinterpret_cast<char *>(buffer_->data()),
