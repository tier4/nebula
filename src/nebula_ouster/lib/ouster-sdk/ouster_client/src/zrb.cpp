...
Zrb::Zrb(
  uint32_t n_rows, uint32_t n_cols, float m_per_zmbin, uint64_t serial_number_init,
  Sha256 stl_hash_init, const mat4d & beam_to_lidar_init, const mat4d & lidar_to_sensor_init,
...
void Zrb::save(std::ostream & out) const
{
  ...
  if (near_range_mm.cols() != far_range_mm.cols() || near_range_mm.rows() != far_range_mm.rows()) {
    throw std::logic_error(
      "Zrb::save: near_range_mm and far_range_mm images have "
      "different sizes");
  }
  if (
    beam_to_lidar_transform.isZero() || lidar_to_sensor_transform.isZero() ||
  ...
  auto bounds_hash = Sha256::hash_blobs({{zr_buffer.data(), zr_buffer.size() * sizeof(uint32_t)}});
...
bool Zrb::read_zone_cache(
  std::istream & file, bool load_bounds, std::vector<std::string> & mismatches)
{
  if (!file) {
  ...
  if (load_bounds) {
    ...
    for (uint32_t col_id = 0; col_id < header.meta.n_cols; col_id++) {
      ...
      if (!file.read(
            reinterpret_cast<char *>(col_buffer.data()),
