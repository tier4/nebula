...
Zip::Zip() : Zip(nullptr, 0, 0)
{  // NOLINT(cppcoreguidelines-pro-type-member-init)
}

Zip::Zip(const std::vector<uint8_t> & zip_bytes)
: Zip(zip_bytes.data(), zip_bytes.size(), ZIP_RDONLY)
{  // NOLINT(cppcoreguidelines-pro-type-member-init)
}
...
std::vector<uint8_t> Zip::get_file(const std::string & filename)
{
  ...
  if (index < 0) {
    throw std::invalid_argument("Error reading " + filename + " from zip archive: No such file");
  }
  ...
  if (zip_stat_index(archive_, u_index, 0, &info) != 0) {
    ...
    zip_fclose(fzip);
    throw std::invalid_argument("Error reading " + filename + " from zip archive: " + error_str);
  }
  ...
  if ((info.valid & ZIP_STAT_SIZE) != 0u) {
    ...
    while (true) {
      ...
      if (bytes < 0) {
        ...
        throw std::runtime_error("Error reading " + filename + " from zip archive: read error");
      }
