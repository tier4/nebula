...
void populate_extrinsics(
  std::string extrinsics_file, std::vector<Eigen::Matrix<double, 4, 4, Eigen::RowMajor>> extrinsics,
  ...
  if (!extrinsics_file.empty()) {
    ...
    if (data.contains("transforms")) {
      for (const auto & obj : data["transforms"].array_range()) {
        ...
        auto rot = Eigen::Quaternion<double>(qw, qx, qy, qz).normalized().toRotationMatrix();
...
std::vector<std::vector<FieldType>> resolve_field_types(
  const std::vector<std::shared_ptr<SensorInfo>> & metadata, bool raw_headers, bool raw_fields,
      ...
      for (const auto & name : field_names.value()) {
        ...
        if (!found) {
          throw std::runtime_error(
            "Requested field '" + name + "' does exist not in packet format.");  // todo print
                                                                                 // packet format
                                                                                 // name?
        }
      }
...
core::AnyScanSource open_source(
  const std::string & name, const std::function<void(ScanSourceOptions &)> & options, bool collate,
  int sensor_idx)
{
...
core::AnyScanSource open_source(
  const std::vector<std::string> & names, const std::function<void(ScanSourceOptions &)> & options,
  bool collate, int sensor_idx)
{
  ...
  if (impl::get_builders().find(type) != impl::get_builders().end()) {
    auto source = impl::get_builders()[type](names, opts, collate, sensor_idx);
    return core::AnyScanSource(std::move(source));
  } else {
    if (type == IoType::OSF || type == IoType::PCAP || type == IoType::SENSOR) {
      throw std::runtime_error(
        "Could not open scan source. Unhandled source type " + to_string(type) +
...
core::AnyPacketSource open_packet_source(
  const std::string & name, const std::function<void(PacketSourceOptions &)> & options)
{
  std::vector<std::string> list = {name};
  ...
  const std::function<void(PacketSourceOptions &)> & options)
{
  ...
  if (impl::get_packet_builders().find(type) != impl::get_packet_builders().end()) {
    return core::AnyPacketSource(impl::get_packet_builders()[type](names, opts));
  } else {
    if (type == IoType::PCAP || type == IoType::SENSOR) {
      throw std::runtime_error(
        "Could not open packet source. Unhandled source type " + to_string(type) +
        ". Perhaps you forgot to include the requisite source type or "
        "built the library without the correct build flags.");
    } else {
      throw std::runtime_error(
