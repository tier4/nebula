...
: scans_(), fields_(std::make_shared<std::unordered_map<std::string, Field>>())
{
}

LidarScanSet::LidarScanSet(const std::vector<std::shared_ptr<LidarScan>> & scans)
: scans_(scans), fields_(std::make_shared<std::unordered_map<std::string, Field>>())
{
}

LidarScanSet::LidarScanSet(std::vector<std::shared_ptr<LidarScan>> && scans) : LidarScanSet()
{
  scans_.swap(scans);
}

LidarScanSet::LidarScanSet(std::initializer_list<std::shared_ptr<LidarScan>> scans)
: scans_(scans), fields_(std::make_shared<std::unordered_map<std::string, Field>>())
{
}
...
std::vector<std::shared_ptr<LidarScan>>::const_iterator LidarScanSet::begin() const
{
  return scans_.cbegin();
}

std::vector<std::shared_ptr<LidarScan>>::const_iterator LidarScanSet::end() const
{
  return scans_.cend();
}
...
{
  return const_cast<std::shared_ptr<LidarScan> &>((*const_cast<const LidarScanSet *>(this))[index]);
}

size_t LidarScanSet::size() const
{
  return scans_.size();
}
...
const Field & LidarScanSet::field(const std::string & name) const
{
  try {
    ...
    throw std::out_of_range("Field '" + name + "' not found in LidarScanSet.");
  }
}

Field & LidarScanSet::field(const std::string & name)
{
  return const_cast<Field &>(const_cast<const LidarScanSet *>(this)->field(name));
}
