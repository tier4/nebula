...
bool Logger::configure_file_sink(const std::string & log_level)
{
                                 ...
                                 {
                                   try {
                                     std::shared_ptr<spdlog::sinks::base_sink<std::mutex>>
                                       file_sink;
                                     if (rotating) {
                                       file_sink =
                                     ...
                                     std::cerr << LOGGER_NAME
                                               << " init_logger failed: " << ex.what() << std::endl;
                                     return false;
                                   }
                                   return true;
                                 }
#else
bool Logger::configure_file_sink(
...
#else
std::string Logger::finalize_format_builder(std::shared_ptr<Logger::format_builder> builder)
{
...
#define LOGGER_PROCESS_ARG(SINGLE_LOGGER_PROCESS_ARG_type)                          \
                                 ...
                                 template <typename T>
                                 void Logger::process_arg(
                                   std::shared_ptr<Logger::format_builder> /*builder*/, T /*data*/)
                                 {
#if defined(ENABLE_SPDLOG)
