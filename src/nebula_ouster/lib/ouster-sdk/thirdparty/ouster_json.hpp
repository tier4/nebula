...
public:
  ...
  : root_(std::make_shared<nlohmann::json>(nlohmann::json::array())), ptr_(root_.get())
  {
  }

  // String value
  json(const std::string & s) : root_(std::make_shared<nlohmann::json>(s)), ptr_(root_.get()) {}

  json(std::string && s) : root_(std::make_shared<nlohmann::json>(std::move(s))), ptr_(root_.get())
  {
  }

  json(const char * s) : root_(std::make_shared<nlohmann::json>(s ? s : "")), ptr_(root_.get()) {}
  ...
  json(T && val) : root_(std::make_shared<nlohmann::json>(std::forward<T>(val))), ptr_(root_.get())
  {
  }

  // Copy constructor: always makes a deep independent copy
  json(const json & other) : root_(std::make_shared<nlohmann::json>(*other.ptr_)), ptr_(root_.get())
  {
  }

  // Move constructor: transfer ownership; leave other in a valid empty state
  json(json && other) noexcept : root_(std::move(other.root_)), ptr_(other.ptr_)
  {
  ...
  static json from_ptr(std::shared_ptr<nlohmann::json> root, nlohmann::json * ptr) noexcept
  {
  ...
  json at(const std::string & key) const { return json(root_, &ptr_->at(key)); }
  ...
  {
    if (ptr_->is_object() && ptr_->contains(key)) return json(root_, &(*ptr_)[key]);
    return json();
  }

  json operator[](std::size_t idx) { return json(root_, &(*ptr_)[idx]); }

  json operator[](std::size_t idx) const { return json(root_, &(*ptr_)[idx]); }
  ...
  T as() const
  {
    if constexpr (std::is_same_v<T, std::string> || std::is_same_v<T, string_view>) {
      ...
      if (ptr_->is_number_integer()) return static_cast<T>(ptr_->get<std::int64_t>());
      if (ptr_->is_number_unsigned()) return static_cast<T>(ptr_->get<std::uint64_t>());
      if (ptr_->is_number_float()) return static_cast<T>(ptr_->get<double>());
      if (ptr_->is_string()) {
        try {
  ...
  std::vector<json> array_range() const
  {
    ...
    if (ptr_->is_array()) {
      for (auto & elem : *ptr_) result.emplace_back(json(root_, &elem));
    }
    return result;
  }
  ...
  const std::shared_ptr<nlohmann::json> & nlohmann_root() const noexcept { return root_; }
};
...
{
public:
  JsonKeyValue(std::string key, json val) : key_(std::move(key)), value_(std::move(val)) {}
...
void encode_base64(It begin, It end, std::string & out)
{
  ...
  if (valb > -6) out.push_back(detail::b64_enc[((val << 8) >> (valb + 8)) & 0x3F]);
...
void decode_base64(It begin, It end, std::vector<std::uint8_t> & out)
{
  ...
  int val = 0, valb = -8;
  for (auto it = begin; it != end; ++it) {
    std::int8_t d = detail::b64_dec[static_cast<std::uint8_t>(static_cast<char>(*it))];
...
using MatchList = std::vector<std::pair<std::string, nlohmann::json *>>;

inline void evaluate(
  nlohmann::json * node, const std::vector<Segment> & segs, std::size_t si, const std::string & cur,
  MatchList & out)
{
  if (!node) return;
  if (si == segs.size()) {
  ...
  if (seg.type == Segment::Type::KEY) {
    ...
    if (node->is_array()) {
      for (std::size_t i = 0; i < node->size(); ++i)
        evaluate(&(*node)[i], segs, si + 1, cur + "[" + std::to_string(i) + "]", out);
...
inline json json_query(const json & root, const std::string & path)
{
  ...
  impl::evaluate(const_cast<nlohmann::json *>(root.nlohmann_ptr()), segs, 0, "$", matches);

  json result(json_array_arg);
  for (auto & [p, node] : matches) result.emplace_back(json::from_nlohmann(*node));
  return result;
}
...
void json_query(const json & root, const std::string & path, F callback)
{
  ...
  for (auto & [p, node] : matches) {
    json val = json::from_ptr(root.nlohmann_root(), const_cast<nlohmann::json *>(node));
...
inline json_schema make_schema(const json & /*schema_json*/)
{
  return {};
}
