...
def extract_seyond_payload(packet_data, link_type, fragments):
    """Strip Link/IP/UDP headers to find Seyond payload, reassembling if necessary."""
    offset = 0
...
def replay_seyond(
    pcap_file, target_ip, target_port, rate=1.0, loop=False, max_gap_ms=100.0, use_pcap_time=False
):
        ...
        while True:
            ...
            for pcap_ts_us, pkt_data in reader:
                ...
                if max_gap_ms > 0 and (ts_us - last_ts_us) > max_gap_ms * 1000:
                    start_wall_time -= (
                        ((ts_us - last_ts_us) - max_gap_ms * 1000) / 1_000_000.0 / rate
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--max-gap-ms", type=float, default=100.0, help="Maximum gap between packets in ms"
    )
    parser.add_argument(
        "--pcap-time",
        action="store_true",
        help="Use PCAP recorded timestamps instead of LiDAR internal timestamps",
    )

    args = parser.parse_args()
    replay_seyond(
        args.pcap, args.ip, args.port, args.rate, args.loop, args.max_gap_ms, args.pcap_time
    )
