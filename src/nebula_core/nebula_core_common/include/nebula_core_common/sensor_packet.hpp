...
// Addresses longer than 45 characters are rejected instead of truncated.
struct SensorEndpoint
{
  ...
  {
    if (addr.size() >= address.size()) {
      throw std::length_error("SensorEndpoint: address exceeds 45-char IPv6 maximum");
    }
    std::memcpy(address.data(), addr.data(), addr.size());
    address[addr.size()] = '\0';
  }
...
struct SensorCanMetadata
{
  ...
  {
    if (name.size() >= interface_name.size()) {
      throw std::length_error("SensorCanMetadata: interface name exceeds Linux IFNAMSIZ");
    }
    std::memcpy(interface_name.data(), name.data(), name.size());
    interface_name[name.size()] = '\0';
  }
...
struct SensorPacketView
{
  ...
  static SensorPacketView from(const SensorPacket &&) = delete;
};
